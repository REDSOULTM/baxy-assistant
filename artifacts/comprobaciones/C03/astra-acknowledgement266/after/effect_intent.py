...
def resolve_explicit_clarification_intent(
    ...
    if (
        ...
        and re.fullmatch(
            ...
            r"(?:\s+(?:del\s+(?:sistema|equipo)|of\s+the\s+system))?"
...
def _strip_request_envelope(text: str) -> str:
    ...
    for _ in range(6):
        ...
        found = _match(
            current,
            r"^[¿?¡!\s]*(?:s[ií]|yes|ok(?:ay)?)\s*[,;:.!]+\s*"
            r"(?P<body>.+)$",
        )
        if found is not None and not (
            _has(found.group("body"), rf"^[¿?¡!\s]*{_COVERAGE_ACTION_HEAD}\b")
...
def _machine_status_scopes(text: str) -> frozenset[str]:
    ...
    if _has(text, r"\bram\b") or (
        _has(text, r"\b(?:memoria|memory)\b") and _has(text, _MEMORY_SCOPE_MEASURE)
    ):
        named.add("memory")
...
_AUDIO_OBSERVATION_HEAD = (
    ...
    r"revisa|revisar|chequea|checa|verifica|mira|fijate|check)"
)
...
def _strict_catalog_request(
    ...
    request_observation = _has(
        ...
        r"dime|dame|cuentame|tell|show|display|muestra|muestrame|ensename|"
...
def _review_system_and_network_effects(
    ...
    if _head_is(
        head,
        r"(?:que|what|dime|show|muestra|tell|time|current|local|hora|fecha)",
    ) and _has(
        folded,
        (
