...
def test_correction_is_derived_from_this_subject_and_value(focus, name):
    from baxy_mind.window_prose_facts import window_focus_contradiction
    payload = _compose_situation_payload(situation(focus, name=name), "en")
    wrong = f"{name} is {'not ' if focus else ''}active."
    feedback = window_focus_contradiction(wrong, payload)
    ...
    assert window_focus_contradiction(f"{name} is {'not ' if not focus else ''}active.", payload) is None
