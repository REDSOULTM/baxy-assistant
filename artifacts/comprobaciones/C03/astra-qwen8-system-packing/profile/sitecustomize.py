...
def _profile_post(self, payload, *args, **kwargs):
    if Path(str(getattr(self, "_gguf", ""))).name.casefold() != "qwen_qwen3-8b-iq3_xxs.gguf":
        return _original_post(self, payload, *args, **kwargs)
    adjusted = dict(payload, temperature=0.7, top_p=0.8, top_k=20, min_p=0.0)
    packed = []
    for message in adjusted.get("messages", []):
        if (message.get("role") == "system" and isinstance(message.get("content"), str)
                and set(message) == {"role", "content"} and packed
                and packed[-1].get("role") == "system" and set(packed[-1]) == {"role", "content"}):
            packed[-1]["content"] += "\n\n" + message["content"]
        else:
            packed.append(dict(message))
    adjusted["messages"] = packed
