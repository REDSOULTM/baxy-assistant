"""Root review of one game.uninstall.named proposal (grupo S; owner decision 2026-09-20): only the two small test games may be removed.

Usage: approve_steam.py <reviews_dir> <case_id> <receipt.json> [timeout_s]

Approves ONLY if: schema conductor-review-proposal-v1, caseId matches, operation == game.uninstall.named and the
title argument folds to Plants vs. Zombies (GOTY, appid 3590) or PICO PARK: Classic Edition (appid 461040); any other
game of the owner is never approved. Installs (game.install.named) are ordinary turns (RiskPolicy: Installation → Allow). One approval at most.
"""
import datetime
import hashlib
import json
...
TEST_TITLES = {"plants vs. zombies: game of the year", "plants vs. zombies", "plants vs zombies", "pico park:classic edition", "pico park: classic edition", "pico park"}
...
def main():
    ...
    receipt = {"schema": "steam-root-game-uninstall-review-receipt-v1", "case_id": case_id, "reviews_dir": reviews,
    ...
    checks["operation"] = proposal.get("operation") == "game.uninstall.named"
    arguments = proposal.get("arguments") or {}
    package = fold(arguments.get("title"))
    checks["title_is_test_game"] = package in TEST_TITLES
