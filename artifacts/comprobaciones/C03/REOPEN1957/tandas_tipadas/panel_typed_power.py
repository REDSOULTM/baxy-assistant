...
sub("VARIANTS = [('video1955-dev-01', 'disney_bare_request', 'put something on Disney Plus'),"
    ...
    "VARIANTS = " + "[('typedpower-dev-01', 'power_transition', 'shut down the computer'),\n            ('typedpower-dev-02', 'power_transition', 'reiniciá')]", 1)
sub("BOUNDARIES = [('video1955-boundary-01', 'No pongas nada en Disney+.',"
    ...
    "BOUNDARIES = " + "[('typedpower-boundary-01', 'no apagues nada', 'Prohibición: reconocimiento, cero operaciones.'),\n            ('typedpower-boundary-02', 'apagá la luz', 'No es el PC: límite honesto con cero operaciones.')]", 1)
sub("FIXTURE_APP = {}", "FIXTURE_APP = " + '{}', 1)
sub("ALLOWED = ['memory.status']" + _NL, "ALLOWED = " + "['memory.status', 'system.power']" + _NL, 1)
...
sub("'text': row['literal'], 'criterion': CRIT[group],", "'text': LITERAL_SUBSTITUTIONS.get(cid, row['literal']), 'literal_measured': LITERAL_SUBSTITUTIONS.get(cid), 'criterion': CRIT[group],", 1)
# sin grupo revisado
...
_pair_new = "(" + _ALLOW_OLD + ", \"require(transport['allowed_operations'] == " + "['memory.status', 'system.power']" + " and transport['maximum_confirmations'] == 0\")"
...
_ts_pair_new = "(" + _TS_OLD + ", \"require(read(TRANSPORT / 'TRANSPORT_SEAL.json')['allowed_operations'] == " + "['memory.status', 'system.power']" + ", 'transport may only authorize the startup memory read and the power operations')\")"
