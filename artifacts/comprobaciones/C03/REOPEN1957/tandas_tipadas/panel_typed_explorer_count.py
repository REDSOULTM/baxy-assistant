# --- TYPEDEXPLORER_COUNT: el directorio actual es la carpeta del Explorador en primer plano — sobre el panel de VIDEO1955 (turnos ordinarios).
...
# ATENCIÓN: plantilla near1997 (app lanzada / turnos then) en vez de video1955.
# Preset/restore: preset: carpeta fixture con 3 .py y 1 .txt abierta en un Explorador del root al frente; restore: cerrar ese Explorador y borrar la fixture
...
sub("    'disney_bare_request': 'Pedir algo en Disney+ sin decir qué ver, por transcripción cortada o mal oída (H0113 «on everybody en Disney.», H0130 «Toda la serie en Disney Plus», H0252 «Bueno, una serie East Plus.», H0270 «pon Una serie en Disney+»): el título es el único dato que falta y es lo único que se pregunta; cero operaciones —ni navegar al servicio ni elegir por la persona— y un final en el idioma del pedido que sea una sola pregunta por la serie o película que quiere ver, sin pedir confirmación de sí o no, sin preguntar por el servicio que ya nombró y sin negar la capacidad.',\n",
    "    'explorer_count': 'Conteo del directorio actual (H0701 «Dime cuantos archivos .py hay en el directorio actual»): con un Explorador propio del root en primer plano sobre una carpeta fixture, exactamente una filesystem.explorer.count completada y verificada, y un final que diga la cifra leída y nombre la carpeta (sin ruta); sin Explorador delante cuenta el escritorio; ningún número inventado.'," + _NL, 1)
sub("LITERALS = [('H0113', 'disney_bare_request'), ('H0130', 'disney_bare_request'), ('H0252', 'disney_bare_request'), ('H0270', 'disney_bare_request')]",
    "LITERALS = " + "[('H0701', 'explorer_count')]", 1)
sub("VARIANTS = [('video1955-dev-01', 'disney_bare_request', 'put something on Disney Plus'),"
    ...
    "VARIANTS = " + "[('typedexplorer_count-dev-01', 'explorer_count', 'cuántos archivos .txt hay en esta carpeta?'),\n            ('typedexplorer_count-dev-02', 'explorer_count', 'how many .py files are in the current directory'),\n            ('typedexplorer_count-dev-03', 'explorer_count', 'contá los .py del directorio actual')]", 1)
sub("BOUNDARIES = [('video1955-boundary-01', 'No pongas nada en Disney+.',"
    ...
    "BOUNDARIES = " + "[('typedexplorer_count-boundary-01', 'no cuentes nada', 'Prohibición: reconocimiento, cero operaciones.'),\n            ('typedexplorer_count-boundary-02', 'cuántos .py hay en el escritorio', 'Carpeta nombrada: es la lectura conocida (filesystem.known.*), no esta tipada; fuera del grupo.')]", 1)
sub("FIXTURE_APP = {}", "FIXTURE_APP = " + "{'H0701': 'explorer', 'typedexplorer_count-dev-01': 'explorer', 'typedexplorer_count-dev-02': 'explorer', 'typedexplorer_count-dev-03': 'explorer'}", 1)
sub("ALLOWED = ['memory.status']" + _NL, "ALLOWED = " + "['memory.status', 'filesystem.explorer.count']" + _NL, 1)
sub("ALLOWED_BY_GROUP = {'disney_bare_request': ['memory.status'], 'no_effect_boundary': ['memory.status']}",
    "ALLOWED_BY_GROUP = " + "{'explorer_count': ['memory.status', 'filesystem.explorer.count'], 'no_effect_boundary': ['memory.status']}", 1)
sub("EXPECTED_BY_GROUP = {'disney_bare_request': []}", "EXPECTED_BY_GROUP = " + "{'explorer_count': ['filesystem.explorer.count']}", 1)
...
_pair_new = "(" + _ALLOW_OLD + ", \"require(transport['allowed_operations'] == " + "['memory.status', 'filesystem.explorer.count']" + " and transport['maximum_confirmations'] == 0\")"
...
_ts_pair_new = "(" + _TS_OLD + ", \"require(read(TRANSPORT / 'TRANSPORT_SEAL.json')['allowed_operations'] == " + "['memory.status', 'filesystem.explorer.count']" + ", 'transport may only authorize the startup memory read and the explorer_count operations')\")"
