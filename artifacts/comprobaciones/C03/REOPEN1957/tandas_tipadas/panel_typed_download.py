# --- TYPEDDOWNLOAD: descargar un archivo o la portada de una página a una carpeta conocida — sobre el panel de VIDEO1955 (turnos ordinarios).
...
# Preset/restore: restore: borrar el archivo descargado
...
sub("VARIANTS = [('video1955-dev-01', 'disney_bare_request', 'put something on Disney Plus'),"
    ...
    "VARIANTS = " + "[('typeddownload-dev-01', 'web_download', 'download https://www.python.org/static/img/python-logo.png to downloads'),\n            ('typeddownload-dev-02', 'web_download', 'bajá la portada de es.wikipedia.org a imágenes'),\n            ('typeddownload-dev-03', 'web_download', 'descargá https://example.com/index.html en descargas')]", 1)
sub("BOUNDARIES = [('video1955-boundary-01', 'No pongas nada en Disney+.',"
    ...
    "BOUNDARIES = " + "[('typeddownload-boundary-01', 'no descargues nada', 'Prohibición: reconocimiento, cero operaciones.'),\n            ('typeddownload-boundary-02', 'descargá todo internet', 'Sin dirección ni archivo: límite honesto, cero operaciones.')]", 1)
sub("FIXTURE_APP = {}", "FIXTURE_APP = " + '{}', 1)
sub("ALLOWED = ['memory.status']" + _NL, "ALLOWED = " + "['memory.status', 'web.download']" + _NL, 1)
sub("ALLOWED_BY_GROUP = {'disney_bare_request': ['memory.status'], 'no_effect_boundary': ['memory.status']}",
    "ALLOWED_BY_GROUP = " + "{'web_download': ['memory.status', 'web.download'], 'no_effect_boundary': ['memory.status']}", 1)
sub("EXPECTED_BY_GROUP = {'disney_bare_request': []}", "EXPECTED_BY_GROUP = " + "{'web_download': ['web.download']}", 1)
...
_pair_new = "(" + _ALLOW_OLD + ", \"require(transport['allowed_operations'] == " + "['memory.status', 'web.download']" + " and transport['maximum_confirmations'] == 0\")"
...
_ts_pair_new = "(" + _TS_OLD + ", \"require(read(TRANSPORT / 'TRANSPORT_SEAL.json')['allowed_operations'] == " + "['memory.status', 'web.download']" + ", 'transport may only authorize the startup memory read and the download operations')\")"
