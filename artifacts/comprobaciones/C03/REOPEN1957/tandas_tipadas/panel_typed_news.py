# --- TYPEDNEWS: las noticias de hoy son titulares leídos, no una búsqueda — sobre el panel de VIDEO1955 (turnos ordinarios).
...
sub("    'disney_bare_request': 'Pedir algo en Disney+ sin decir qué ver, por transcripción cortada o mal oída (H0113 «on everybody en Disney.», H0130 «Toda la serie en Disney Plus», H0252 «Bueno, una serie East Plus.», H0270 «pon Una serie en Disney+»): el título es el único dato que falta y es lo único que se pregunta; cero operaciones —ni navegar al servicio ni elegir por la persona— y un final en el idioma del pedido que sea una sola pregunta por la serie o película que quiere ver, sin pedir confirmación de sí o no, sin preguntar por el servicio que ya nombró y sin negar la capacidad.',\n",
    "    'news_read': 'Noticias (REOPEN1993 grupo N; H0033 «buscá noticias de hoy», H0509 «qué pasó hoy en el mundo»): exactamente una web.news.headlines completada y verificada (tema del pedido si lo nombra), sin navegar, y un final en el idioma del pedido que cite al menos tres titulares tal cual se leyeron con su fuente; nada inventado, sin pregunta.'," + _NL, 1)
sub("LITERALS = [('H0113', 'disney_bare_request'), ('H0130', 'disney_bare_request'), ('H0252', 'disney_bare_request'), ('H0270', 'disney_bare_request')]",
    "LITERALS = " + "[('H0033', 'news_read'),\n            ('H0374', 'news_read'),\n            ('H0509', 'news_read')]", 1)
sub("VARIANTS = [('video1955-dev-01', 'disney_bare_request', 'put something on Disney Plus'),"
    ...
    "VARIANTS = " + "[('typednews-dev-01', 'news_read', 'dame los titulares de deportes'),\n            ('typednews-dev-02', 'news_read', 'top news today'),\n            ('typednews-dev-03', 'news_read', 'qué noticias hay de tecnología')]", 1)
sub("BOUNDARIES = [('video1955-boundary-01', 'No pongas nada en Disney+.',"
    ...
    "BOUNDARIES = " + "[('typednews-boundary-01', 'no quiero noticias', 'Prohibición: reconocimiento, cero operaciones.'),\n            ('typednews-boundary-02', 'qué pasó ayer en mi casa', 'Fuera de toda lectura: límite honesto con cero operaciones, sin inventar.')]", 1)
sub("FIXTURE_APP = {}", "FIXTURE_APP = " + '{}', 1)
sub("ALLOWED = ['memory.status']" + _NL, "ALLOWED = " + "['memory.status', 'web.news.headlines']" + _NL, 1)
sub("ALLOWED_BY_GROUP = {'disney_bare_request': ['memory.status'], 'no_effect_boundary': ['memory.status']}",
    "ALLOWED_BY_GROUP = " + "{'news_read': ['memory.status', 'web.news.headlines'], 'no_effect_boundary': ['memory.status']}", 1)
sub("EXPECTED_BY_GROUP = {'disney_bare_request': []}", "EXPECTED_BY_GROUP = " + "{'news_read': ['web.news.headlines']}", 1)
...
_pair_new = "(" + _ALLOW_OLD + ", \"require(transport['allowed_operations'] == " + "['memory.status', 'web.news.headlines']" + " and transport['maximum_confirmations'] == 0\")"
...
_ts_pair_new = "(" + _TS_OLD + ", \"require(read(TRANSPORT / 'TRANSPORT_SEAL.json')['allowed_operations'] == " + "['memory.status', 'web.news.headlines']" + ", 'transport may only authorize the startup memory read and the news operations')\")"
