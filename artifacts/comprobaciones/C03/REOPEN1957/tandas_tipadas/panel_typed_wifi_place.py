# --- TYPEDWIFI_PLACE: «casa» no es un SSID: preguntar cuál, recordarlo y conectar directo después — sobre el panel de VIDEO1955 (turnos ordinarios).
...
# ATENCIÓN: plantilla then2001 (app lanzada / turnos then) en vez de video1955.
# Preset/restore: preset: borrar `<perfil>/scans/wifi-places.v1.json`; restore: reconectar la red original del dueño
...
sub("VARIANTS = [('video1955-dev-01', 'disney_bare_request', 'put something on Disney Plus'),"
    ...
    "VARIANTS = " + "[('typedwifi_place-dev-01', 'wifi_place', 'connect to my home wifi'),\n            ('typedwifi_place-dev-02', 'wifi_place', 'cambia el wifi al de casa')]", 1)
sub("BOUNDARIES = [('video1955-boundary-01', 'No pongas nada en Disney+.',"
    ...
    "BOUNDARIES = " + "[('typedwifi_place-boundary-01', 'conectate al wifi de la luna', 'Nombre inexistente: fallo honesto wifi_profile_not_found, sin pregunta de lugar.'),\n            ('typedwifi_place-boundary-02', 'no te conectes al wifi', 'Prohibición: reconocimiento, cero operaciones.')]", 1)
sub("FIXTURE_APP = {}", "FIXTURE_APP = " + '{}', 1)
sub("ALLOWED = ['memory.status']" + _NL, "ALLOWED = " + "['memory.status', 'wifi.profile.list', 'wifi.connect.named']" + _NL, 1)
sub("ALLOWED_BY_GROUP = {'disney_bare_request': ['memory.status'], 'no_effect_boundary': ['memory.status']}",
    "ALLOWED_BY_GROUP = " + "{'wifi_place': ['memory.status', 'wifi.profile.list', 'wifi.connect.named'], 'no_effect_boundary': ['memory.status']}", 1)
sub("EXPECTED_BY_GROUP = {'disney_bare_request': []}", "EXPECTED_BY_GROUP = " + "{'wifi_place': ['wifi.profile.list', 'wifi.connect.named']}", 1)
...
_pair_new = "(" + _ALLOW_OLD + ", \"require(transport['allowed_operations'] == " + "['memory.status', 'wifi.profile.list', 'wifi.connect.named']" + " and transport['maximum_confirmations'] == 0\")"
...
_ts_pair_new = "(" + _TS_OLD + ", \"require(read(TRANSPORT / 'TRANSPORT_SEAL.json')['allowed_operations'] == " + "['memory.status', 'wifi.profile.list', 'wifi.connect.named']" + ", 'transport may only authorize the startup memory read and the wifi_place operations')\")"
