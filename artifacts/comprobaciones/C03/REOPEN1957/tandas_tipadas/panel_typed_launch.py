# --- TYPEDLAUNCH: lanzar un juego instalado nombrado sin edición, con «ve a» o «lanzá» — sobre el panel de VIDEO1955 (turnos ordinarios).
...
# Preset/restore: launch/before (PvZ y PICO PARK presentes); steam/pvz-present antes de H0083; launch/close antes de cada caso siguiente a un lanzamiento y launch/after al final (cierra PlantsVsZombies/popcapgame1/PICO PARK); el Steam del dueño se reutiliza
...
sub("    'disney_bare_request': 'Pedir algo en Disney+ sin decir qué ver, por transcripción cortada o mal oída (H0113 «on everybody en Disney.», H0130 «Toda la serie en Disney Plus», H0252 «Bueno, una serie East Plus.», H0270 «pon Una serie en Disney+»): el título es el único dato que falta y es lo único que se pregunta; cero operaciones —ni navegar al servicio ni elegir por la persona— y un final en el idioma del pedido que sea una sola pregunta por la serie o película que quiere ver, sin pedir confirmación de sí o no, sin preguntar por el servicio que ya nombró y sin negar la capacidad.',\n",
    "    'game_launch': 'Lanzar un juego instalado (D11; H0083 «lanzá Mortal Kombat en Steam», H0608 «lanzá Mortal Kombat»; medidos con Plants vs. Zombies instalado, D13): exactamente una game.launch completada y verificada (el proceso del juego o su ventana aparece) y un final que diga que lo lanzó; con el título ausente de la biblioteca, la lectura honesta (game.entitlement.named) y ninguna invención.'," + _NL, 1)
sub("LITERALS = [('H0113', 'disney_bare_request'), ('H0130', 'disney_bare_request'), ('H0252', 'disney_bare_request'), ('H0270', 'disney_bare_request')]",
    "LITERALS = " + "[('H0083', 'game_launch'),\n            ('H0608', 'game_launch')]", 1)
sub("VARIANTS = [('video1955-dev-01', 'disney_bare_request', 'put something on Disney Plus'),"
    ...
    "VARIANTS = " + "[('typedlaunch-dev-01', 'game_launch', 'abrí PICO PARK'),\n            ('typedlaunch-dev-02', 'game_launch', 've a Plants vs. Zombies')]", 1)
sub("BOUNDARIES = [('video1955-boundary-01', 'No pongas nada en Disney+.',"
    ...
    "BOUNDARIES = " + "[('typedlaunch-boundary-01', 'no lances nada', 'Prohibición: reconocimiento, cero operaciones.'),\n            ('typedlaunch-boundary-02', 'gracias, ya lo abrí yo', 'Aviso hecho: reconocimiento, cero operaciones.')]", 1)
sub("FIXTURE_APP = {}", "FIXTURE_APP = " + '{}', 1)
sub("ALLOWED = ['memory.status']" + _NL, "ALLOWED = " + "['memory.status', 'game.launch', 'game.entitlement.named']" + _NL, 1)
sub("ALLOWED_BY_GROUP = {'disney_bare_request': ['memory.status'], 'no_effect_boundary': ['memory.status']}",
    "ALLOWED_BY_GROUP = " + "{'game_launch': ['memory.status', 'game.launch', 'game.entitlement.named'], 'no_effect_boundary': ['memory.status']}", 1)
sub("EXPECTED_BY_GROUP = {'disney_bare_request': []}", "EXPECTED_BY_GROUP = " + "{'game_launch': ['game.launch']}", 1)
...
sub("LITERALS = ", "LITERAL_SUBSTITUTIONS = " + "{'H0083': 'lanzá Plants vs. Zombies en Steam', 'H0608': 'lanzá Plants vs. Zombies'}" + _NL + "CASE_FIXTURE_STEPS = " + "{'H0083': 'steam/pvz-present', 'H0608': 'launch/close', 'typedlaunch-dev-01': 'launch/close', 'typedlaunch-dev-02': 'launch/close', 'typedlaunch-boundary-01': 'launch/close'}" + _NL + "LITERALS = ", 1)
...
sub("counts = {'cases': N, 'historical_literals': 4, 'original_development_variants': 2, 'boundaries': 2, 'wire_lines': 2 * N,",
    "counts = {'cases': N, 'historical_literals': 2, 'original_development_variants': 2, 'boundaries': 2, 'wire_lines': 2 * N,", 1)
...
sub("{'historical_literal': 4, 'original_development_variant': 2, 'boundary': 2}, 'kind counts')",
    "{'historical_literal': 2, 'original_development_variant': 2, 'boundary': 2}, 'kind counts')", 1)
sub("panel[0:4]] == ids, 'literal order')", "panel[0:2]] == ids, 'literal order')", 1)
sub("and seal['kind_counts'] == {'historical_literal': 4, 'original_development_variant': 2, 'boundary': 2}, 'Material cardinalities changed')",
    "and seal['kind_counts'] == {'historical_literal': 2, 'original_development_variant': 2, 'boundary': 2}, 'Material cardinalities changed')", 1)
sub("need(seal['kind_counts'] == {'historical_literal':4, 'original_development_variant':2, 'boundary':2}, 'Wrong material kinds')",
    "need(seal['kind_counts'] == {'historical_literal':2, 'original_development_variant':2, 'boundary':2}, 'Wrong material kinds')", 1)
...
_pair_new = "(" + _ALLOW_OLD + ", \"require(transport['allowed_operations'] == " + "['memory.status', 'game.launch', 'game.entitlement.named']" + " and transport['maximum_confirmations'] == 0\")"
...
_ts_pair_new = "(" + _TS_OLD + ", \"require(read(TRANSPORT / 'TRANSPORT_SEAL.json')['allowed_operations'] == " + "['memory.status', 'game.launch', 'game.entitlement.named']" + ", 'transport may only authorize the startup memory read and the launch operations')\")"
