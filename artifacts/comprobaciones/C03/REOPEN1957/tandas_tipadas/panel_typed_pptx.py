# --- TYPEDPPTX: el powerpoint se escribe como paquete Open XML y se abre — sobre el panel de VIDEO1955 (turnos ordinarios).
...
# Preset/restore: restore: borrar el .pptx de Documentos; cerrar PowerPoint
...
sub("VARIANTS = [('video1955-dev-01', 'disney_bare_request', 'put something on Disney Plus'),"
    ...
    "VARIANTS = " + "[('typedpptx-dev-01', 'presentation_create', 'armá una presentación sobre el sistema solar de 4 diapositivas'),\n            ('typedpptx-dev-02', 'presentation_create', 'make a presentation about dogs with 5 slides'),\n            ('typedpptx-dev-03', 'presentation_create', 'creá un powerpoint sobre gatos')]", 1)
sub("BOUNDARIES = [('video1955-boundary-01', 'No pongas nada en Disney+.',"
    ...
    "BOUNDARIES = " + "[('typedpptx-boundary-01', 'no hagas ningún powerpoint', 'Prohibición: reconocimiento, cero operaciones.'),\n            ('typedpptx-boundary-02', 'crea un powerpoint', 'Sin tema: pregunta de qué, cero operaciones.')]", 1)
sub("FIXTURE_APP = {}", "FIXTURE_APP = " + '{}', 1)
sub("ALLOWED = ['memory.status']" + _NL, "ALLOWED = " + "['memory.status', 'document.presentation.create', 'file.open']" + _NL, 1)
sub("ALLOWED_BY_GROUP = {'disney_bare_request': ['memory.status'], 'no_effect_boundary': ['memory.status']}",
    "ALLOWED_BY_GROUP = " + "{'presentation_create': ['memory.status', 'document.presentation.create', 'file.open'], 'no_effect_boundary': ['memory.status']}", 1)
sub("EXPECTED_BY_GROUP = {'disney_bare_request': []}", "EXPECTED_BY_GROUP = " + "{'presentation_create': ['document.presentation.create', 'file.open']}", 1)
...
_pair_new = "(" + _ALLOW_OLD + ", \"require(transport['allowed_operations'] == " + "['memory.status', 'document.presentation.create', 'file.open']" + " and transport['maximum_confirmations'] == 0\")"
...
_ts_pair_new = "(" + _TS_OLD + ", \"require(read(TRANSPORT / 'TRANSPORT_SEAL.json')['allowed_operations'] == " + "['memory.status', 'document.presentation.create', 'file.open']" + ", 'transport may only authorize the startup memory read and the pptx operations')\")"
