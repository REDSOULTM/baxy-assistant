# --- TYPEDAPPVOLUME: Spotify: nivel absoluto por app, ajuste en pausa y la pregunta por la cantidad — sobre el panel de VIDEO1955 (turnos ordinarios).
# Generado por make_typed_panels.py; se ejecuta dentro de derive_<new>.py (la variable s es el build).
# Cardinalidades: 1 literales, 4 variantes, 2 límites (N = 7).
# Preset/restore: appvolume/before (Spotify abierto con sesión de audio: reproduce 3 s por SMTC y pausa; registra el nivel) y appvolume/after (pausa y restaura el nivel) de typed_fixtures.ps1 con app_volume.ps1; el Spotify del dueño no se cierra
...
sub("VARIANTS = [('video1955-dev-01', 'disney_bare_request', 'put something on Disney Plus'),"
    ...
    "VARIANTS = " + "[('typedappvolume-dev-01', 'app_volume', 'poné el volumen de Spotify al 40'),\n            ('typedappvolume-dev-02', 'app_volume', 'dejá spotify a la mitad'),\n            ('typedappvolume-dev-03', 'app_volume', 'set spotify volume to 60'),\n            ('typedappvolume-dev-04', 'app_volume', 'bajá el volumen de spotify en 20')]", 1)
sub("BOUNDARIES = [('video1955-boundary-01', 'No pongas nada en Disney+.',"
    ...
    "BOUNDARIES = " + "[('typedappvolume-boundary-01', 'no toques el volumen de spotify', 'Prohibición: reconocimiento, cero operaciones.'),\n            ('typedappvolume-boundary-02', '¿a cuánto está el volumen de spotify?', 'Pregunta de estado: lectura (audio.status) o límite honesto, ningún cambio.')]", 1)
sub("FIXTURE_APP = {}", "FIXTURE_APP = " + '{}', 1)
sub("ALLOWED = ['memory.status']" + _NL, "ALLOWED = " + "['memory.status', 'audio.app.volume.set', 'audio.app.volume.adjust', 'audio.status']" + _NL, 1)
sub("ALLOWED_BY_GROUP = {'disney_bare_request': ['memory.status'], 'no_effect_boundary': ['memory.status']}",
    "ALLOWED_BY_GROUP = " + "{'app_volume': ['memory.status', 'audio.app.volume.set', 'audio.app.volume.adjust', 'audio.status'], 'no_effect_boundary': ['memory.status']}", 1)
sub("EXPECTED_BY_GROUP = {'disney_bare_request': []}", "EXPECTED_BY_GROUP = " + "{'app_volume': ['audio.app.volume.set']}", 1)
...
# ------------------------------------------------ cardinalidades: 1 literales, 4 variantes, 2 límites
sub("counts = {'cases': N, 'historical_literals': 4, 'original_development_variants': 2, 'boundaries': 2, 'wire_lines': 2 * N,",
    "counts = {'cases': N, 'historical_literals': 1, 'original_development_variants': 4, 'boundaries': 2, 'wire_lines': 2 * N,", 1)
sub("kind_counts = {'historical_literal': 4, 'original_development_variant': 2, 'boundary': 2}",
    "kind_counts = {'historical_literal': 1, 'original_development_variant': 4, 'boundary': 2}", 1)
sub("'historical_literals': 4, 'original_development_variants': 2, 'boundaries': 2, 'wire_lines': 16, 'session_controls': 8, 'ordinary_turns': 8, 'reviewed_turns': 0, 'maximum_reportable_terminals': 8, 'maximum_internal_confirmations': 0}\"",
    "'historical_literals': 1, 'original_development_variants': 4, 'boundaries': 2, 'wire_lines': 14, 'session_controls': 7, 'ordinary_turns': 7, 'reviewed_turns': 0, 'maximum_reportable_terminals': 7, 'maximum_internal_confirmations': 0}\"", 1)
# turnos ordinarios

sub("== ['historical_literal'] * 4 + ['original_development_variant'] * 2 + ['boundary'] * 2,",
    "== ['historical_literal'] * 1 + ['original_development_variant'] * 4 + ['boundary'] * 2,", 1)
sub("{'historical_literal': 4, 'original_development_variant': 2, 'boundary': 2}, 'kind counts')",
    "{'historical_literal': 1, 'original_development_variant': 4, 'boundary': 2}, 'kind counts')", 1)
...
sub("and seal['kind_counts'] == {'historical_literal': 4, 'original_development_variant': 2, 'boundary': 2}, 'Material cardinalities changed')",
    "and seal['kind_counts'] == {'historical_literal': 1, 'original_development_variant': 4, 'boundary': 2}, 'Material cardinalities changed')", 1)
sub("need(seal['kind_counts'] == {'historical_literal':4, 'original_development_variant':2, 'boundary':2}, 'Wrong material kinds')",
    "need(seal['kind_counts'] == {'historical_literal':1, 'original_development_variant':4, 'boundary':2}, 'Wrong material kinds')", 1)
...
_pair_new = "(" + _ALLOW_OLD + ", \"require(transport['allowed_operations'] == " + "['memory.status', 'audio.app.volume.set', 'audio.app.volume.adjust', 'audio.status']" + " and transport['maximum_confirmations'] == 0\")"
...
_ts_pair_new = "(" + _TS_OLD + ", \"require(read(TRANSPORT / 'TRANSPORT_SEAL.json')['allowed_operations'] == " + "['memory.status', 'audio.app.volume.set', 'audio.app.volume.adjust', 'audio.status']" + ", 'transport may only authorize the startup memory read and the appvolume operations')\")"
