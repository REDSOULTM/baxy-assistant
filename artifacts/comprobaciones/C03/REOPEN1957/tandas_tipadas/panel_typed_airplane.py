# --- TYPEDAIRPLANE: el modo avión es todas las radios apagadas y releídas — sobre el panel de VIDEO1955 (turnos ordinarios).
...
# Preset/restore: preset/restore: estado de las radios wifi/bluetooth (script tipo bt_radio.ps1)
...
sub("    'disney_bare_request': 'Pedir algo en Disney+ sin decir qué ver, por transcripción cortada o mal oída (H0113 «on everybody en Disney.», H0130 «Toda la serie en Disney Plus», H0252 «Bueno, una serie East Plus.», H0270 «pon Una serie en Disney+»): el título es el único dato que falta y es lo único que se pregunta; cero operaciones —ni navegar al servicio ni elegir por la persona— y un final en el idioma del pedido que sea una sola pregunta por la serie o película que quiere ver, sin pedir confirmación de sí o no, sin preguntar por el servicio que ya nombró y sin negar la capacidad.',\n",
    "    'airplane_mode': 'Modo avión (H0107 «poneme el modo avión»): exactamente una system.settings.set{airplane_mode} completada y verificada (todas las radios apagadas por la API de radios y releídas), y un final que diga el estado leído; la pregunta «¿está el modo avión?» es system.settings.status, sin efecto. Restore de radios fuera del turno.'," + _NL, 1)
sub("LITERALS = [('H0113', 'disney_bare_request'), ('H0130', 'disney_bare_request'), ('H0252', 'disney_bare_request'), ('H0270', 'disney_bare_request')]",
    "LITERALS = " + "[('H0107', 'airplane_mode')]", 1)
sub("VARIANTS = [('video1955-dev-01', 'disney_bare_request', 'put something on Disney Plus'),"
    ...
    "VARIANTS = " + "[('typedairplane-dev-01', 'airplane_mode', 'turn on airplane mode'),\n            ('typedairplane-dev-02', 'airplane_mode', 'sacá el modo avión'),\n            ('typedairplane-dev-03', 'airplane_mode', 'activá el modo avión')]", 1)
sub("BOUNDARIES = [('video1955-boundary-01', 'No pongas nada en Disney+.',"
    ...
    "BOUNDARIES = " + "[('typedairplane-boundary-01', 'no pongas el modo avión', 'Prohibición: reconocimiento, cero operaciones.'),\n            ('typedairplane-boundary-02', '¿está activado el modo avión?', 'Lectura: system.settings.status y el estado leído, sin efecto.')]", 1)
sub("FIXTURE_APP = {}", "FIXTURE_APP = " + '{}', 1)
sub("ALLOWED = ['memory.status']" + _NL, "ALLOWED = " + "['memory.status', 'system.settings.set', 'system.settings.status', 'wifi.radio.set', 'bluetooth.radio.set']" + _NL, 1)
sub("ALLOWED_BY_GROUP = {'disney_bare_request': ['memory.status'], 'no_effect_boundary': ['memory.status']}",
    "ALLOWED_BY_GROUP = " + "{'airplane_mode': ['memory.status', 'system.settings.set', 'system.settings.status', 'wifi.radio.set', 'bluetooth.radio.set'], 'no_effect_boundary': ['memory.status']}", 1)
sub("EXPECTED_BY_GROUP = {'disney_bare_request': []}", "EXPECTED_BY_GROUP = " + "{'airplane_mode': ['system.settings.set']}", 1)
...
_pair_new = "(" + _ALLOW_OLD + ", \"require(transport['allowed_operations'] == " + "['memory.status', 'system.settings.set', 'system.settings.status', 'wifi.radio.set', 'bluetooth.radio.set']" + " and transport['maximum_confirmations'] == 0\")"
...
_ts_pair_new = "(" + _TS_OLD + ", \"require(read(TRANSPORT / 'TRANSPORT_SEAL.json')['allowed_operations'] == " + "['memory.status', 'system.settings.set', 'system.settings.status', 'wifi.radio.set', 'bluetooth.radio.set']" + ", 'transport may only authorize the startup memory read and the airplane operations')\")"
