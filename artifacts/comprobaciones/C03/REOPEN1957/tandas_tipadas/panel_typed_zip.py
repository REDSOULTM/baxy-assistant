# --- TYPEDZIP: carpeta, txt, zip y abrir: una misión de cuatro tipadas — sobre el panel de VIDEO1955 (turnos ordinarios).
...
# Preset/restore: restore: borrar «Nueva carpeta*» y «Nueva carpeta.zip» del escritorio; cerrar el Explorador del zip
...
sub("VARIANTS = [('video1955-dev-01', 'disney_bare_request', 'put something on Disney Plus'),"
    ...
    "VARIANTS = " + "[('typedzip-dev-01', 'folder_zip_mission', 'make a folder on the desktop with a txt inside, zip it and open the zip'),\n            ('typedzip-dev-02', 'folder_zip_mission', 'armá una carpeta en el escritorio con un txt, comprimila y abrí el zip'),\n            ('typedzip-dev-03', 'folder_zip_mission', 'creá una carpeta en documentos, poné un txt, comprimila y abrila')]", 1)
sub("BOUNDARIES = [('video1955-boundary-01', 'No pongas nada en Disney+.',"
    ...
    "BOUNDARIES = " + "[('typedzip-boundary-01', 'no crees nada en el escritorio', 'Prohibición: reconocimiento, cero operaciones.'),\n            ('typedzip-boundary-02', 'comprimí el escritorio entero', 'Sin carpeta nombrada ni creada: límite honesto, cero operaciones.')]", 1)
sub("FIXTURE_APP = {}", "FIXTURE_APP = " + '{}', 1)
sub("ALLOWED = ['memory.status']" + _NL, "ALLOWED = " + "['memory.status', 'filesystem.create.directory', 'filesystem.write.text', 'file.compress', 'file.open']" + _NL, 1)
sub("ALLOWED_BY_GROUP = {'disney_bare_request': ['memory.status'], 'no_effect_boundary': ['memory.status']}",
    "ALLOWED_BY_GROUP = " + "{'folder_zip_mission': ['memory.status', 'filesystem.create.directory', 'filesystem.write.text', 'file.compress', 'file.open'], 'no_effect_boundary': ['memory.status']}", 1)
sub("EXPECTED_BY_GROUP = {'disney_bare_request': []}", "EXPECTED_BY_GROUP = " + "{'folder_zip_mission': ['filesystem.create.directory', 'filesystem.write.text', 'file.compress', 'file.open']}", 1)
...
_pair_new = "(" + _ALLOW_OLD + ", \"require(transport['allowed_operations'] == " + "['memory.status', 'filesystem.create.directory', 'filesystem.write.text', 'file.compress', 'file.open']" + " and transport['maximum_confirmations'] == 0\")"
...
_ts_pair_new = "(" + _TS_OLD + ", \"require(read(TRANSPORT / 'TRANSPORT_SEAL.json')['allowed_operations'] == " + "['memory.status', 'filesystem.create.directory', 'filesystem.write.text', 'file.compress', 'file.open']" + ", 'transport may only authorize the startup memory read and the zip operations')\")"
