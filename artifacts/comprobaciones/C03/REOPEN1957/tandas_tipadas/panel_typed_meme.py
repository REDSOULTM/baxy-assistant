# --- TYPEDMEME: un meme se busca, se descarga y se abre con el visor — sobre el panel de VIDEO1955 (turnos ordinarios).
...
# Preset/restore: restore: borrar la imagen de Imágenes; cerrar el visor
...
sub("VARIANTS = [('video1955-dev-01', 'disney_bare_request', 'put something on Disney Plus'),"
    ...
    "VARIANTS = " + "[('typedmeme-dev-01', 'web_image', 'mandame un meme de gatos'),\n            ('typedmeme-dev-02', 'web_image', 'show me a meme'),\n            ('typedmeme-dev-03', 'web_image', 'pasame una foto de un gato')]", 1)
sub("BOUNDARIES = [('video1955-boundary-01', 'No pongas nada en Disney+.',"
    ...
    "BOUNDARIES = " + "[('typedmeme-boundary-01', 'tienes alguna foto?', 'Imagen sin sujeto: pregunta de qué, cero operaciones.'),\n            ('typedmeme-boundary-02', 'no me mandes memes', 'Prohibición: reconocimiento, cero operaciones.')]", 1)
sub("FIXTURE_APP = {}", "FIXTURE_APP = " + '{}', 1)
sub("ALLOWED = ['memory.status']" + _NL, "ALLOWED = " + "['memory.status', 'web.download', 'file.open']" + _NL, 1)
sub("ALLOWED_BY_GROUP = {'disney_bare_request': ['memory.status'], 'no_effect_boundary': ['memory.status']}",
    "ALLOWED_BY_GROUP = " + "{'web_image': ['memory.status', 'web.download', 'file.open'], 'no_effect_boundary': ['memory.status']}", 1)
sub("EXPECTED_BY_GROUP = {'disney_bare_request': []}", "EXPECTED_BY_GROUP = " + "{'web_image': ['web.download', 'file.open']}", 1)
...
_pair_new = "(" + _ALLOW_OLD + ", \"require(transport['allowed_operations'] == " + "['memory.status', 'web.download', 'file.open']" + " and transport['maximum_confirmations'] == 0\")"
...
_ts_pair_new = "(" + _TS_OLD + ", \"require(read(TRANSPORT / 'TRANSPORT_SEAL.json')['allowed_operations'] == " + "['memory.status', 'web.download', 'file.open']" + ", 'transport may only authorize the startup memory read and the meme operations')\")"
