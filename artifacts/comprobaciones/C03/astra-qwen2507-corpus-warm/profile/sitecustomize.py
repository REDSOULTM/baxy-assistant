...
def _profile_post(self, payload, *args, **kwargs):
    ...
    try:
        ...
        if prompt.startswith("Extrae en una sola respuesta los argumentos"):
            with _audit.with_name("argument-batches.jsonl").open("a", encoding="utf-8") as stream:
                stream.write(json.dumps({"requestId": request_id, "payload": adjusted, "response": response}) + "\n")
        return response
...
def _observe_skills(self, *args, **kwargs):
    ...
    try:
        ...
        _record_resource({"component":"skill_registry", "encoderSupplied":kwargs.get("encoder") is not None, "seconds":time.monotonic()-started, "error":error})
