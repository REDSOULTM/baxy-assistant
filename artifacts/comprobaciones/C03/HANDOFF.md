...
- **Permiso (BLOQUEO ACTUAL)**: la sesión raíz auto-modo auto-deniega operar WhatsApp/Discord reales (el clasificador bloqueó hasta un commit que describía envíos). El `.claude/settings.local.json` del dueño tiene `defaultMode: bypassPermissions` pero NO está activo en la sesión en curso; se activa al REINICIAR la sesión. La raíz no evita el clasificador para enviar mensajes reales. **Al reanudar en una sesión con bypassPermissions activo**: (1) prueba supervisada — un envío real a «Música» (WhatsApp) verificado por OCR, que el dueño ve; (2) si verifica, construir la tanda estilo MSG1837 (reviewed) adaptada a message.send.test (base `BASE/C03-msg1837-tooling`, driver que llama message.send.test) y correrla (escritor único de C03); (3) adjudicar: acreditan sólo los literales cuyo destinatario ES Música (H0005, H0013, H0198, H0231, H0369, H0395, H0425, H0533, H0536, H0719) o Violeta; los de terceros se miden pero quedan sin crédito con la causa.
