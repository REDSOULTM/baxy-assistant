...
- **Falta construir el camino de envío real forzado** (el mecanismo MSG1837 actual es borrador-sin-enviar): el adaptador debe enviar de verdad al destino forzado y el compositor decirlo. Base a adaptar: `BASE/C03-msg1837-tooling` (msgdraft_case.sh, approve_message_draft.py, clear_composer.ps1) + `src/Baxy.Providers.Windows/External/DesktopMessagingAdapter.cs`.
- **Permiso**: la sesión raíz auto-deniega operar WhatsApp/Discord reales. El dueño gestiona `.claude/settings.local.json` (gitignored; la raíz NUNCA lo edita) con reglas allow. Al reanudar con permiso: (1) construir el envío forzado, (2) prueba supervisada a un canal del dueño y verificar por OCR, (3) sólo entonces sellar y correr la tanda completa (escritor único de C03). No rebajar guardas ni evitar la auto-denegación. Si un comando se deniega, dar al peer/dueño el comando exacto para la regla allow.
