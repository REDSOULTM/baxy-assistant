...
def _compose_situation_payload(
    ...
    *,
    ...
    if cause_key == "remaining_steps_cancelled":
    ...
    elif cause_key == "remaining_steps_cancelled":
...
def _compose_user_content(
    ...
    *,
    ...
    parts = [f"Texto original de la persona: {user_text}"] if include_request else []
...
def compose_visible_defect(
    ...
    if intent == "welcome" or kind == "welcome":
        ...
        if _looks_like_knowledge_question(user_text) and reply_reading.greets:
            ...
            if body.endswith("?") and not re.search(r"[.!]\s", body):
        ...
        payload = {
            "messages": [
                {"role": "system", "content": NATIVE_TOOL_POLICY_PROMPT},
                {
                    ...
                    "content": json.dumps(
                        {
                            "previous_dialogue_for_references_only": prior_messages[-6:],
                            "current_request_to_interpret": text,
                        },
                        ensure_ascii=False,
                    ),
                },
    ...
    def compose_user_message(
        ...
        *,
        ...
        if previous_answer and (
            ...
            # A prior reply as an assistant turn replayed the old topic in
            # Granite panels4/5. Keep the bounded published context as data:
            # recovery still needs it to explain a preceding observed failure.
            visible_situation["previousResponse"] = previous_answer
        prompt_facts = {
            key: value
