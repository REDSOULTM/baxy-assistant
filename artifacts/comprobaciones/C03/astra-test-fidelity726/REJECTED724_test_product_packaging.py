...
class ProductPackagingTests(unittest.TestCase):
    ...
    def test_detached_head_snapshot_is_exact_and_cleanup_removes_it(self) -> None:
        ...
        byte_checks = "\n".join(
            f"    if ((Get-BaxySha256 -Path (Join-Path $path '{relative}')) -cne '{sha256_bytes(content)}') {{ throw 'snapshot bytes mismatch' }}"
            for relative, content in expected_files.items()
        )
        command = f"""
...
"""
        ...
        self.assertEqual(source_git("rev-parse", "HEAD"), commit)
        self.assertEqual(
            source_git("status", "--porcelain=v1", "--untracked-files=all"),
            status_before,
        )
        worktrees = source_git("worktree", "list", "--porcelain")
        self.assertEqual(worktrees.count("worktree "), 1, worktrees)
        self.assertNotIn("head-snapshot", worktrees)
