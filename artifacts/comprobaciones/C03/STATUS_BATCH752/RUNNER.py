"""Frozen 73-case status regression through the registered, unmodified product.
...
OUT = ROOT / 'artifacts/comprobaciones/C03/STATUS_BATCH752'
PRIVATE = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-status-batch752-private'
PROFILE = PRIVATE.parent / 'C03-status-profile752'
...
pins751 = read(ROOT / 'artifacts/comprobaciones/C03/WINDOW_INVENTORY751/SOURCE_PINS.json')
assert all(sha(ROOT / path) == digest for path, digest in pins751.items())
...
prereg = {
    'utc': utc(), 'method': 'Registered product regression after published750/751; not a model comparison.',
    ...
    'validation': 'Published751 owner tests and Fast, plus historical Full7; no new source edit.',
    'source751_pins': pins751, 'sources': sources, 'runner_sha256': sha(__file__),
...
with (PRIVATE / 'launch.log').open('w', encoding='utf-8') as log:
    ...
    state.update(confirmedAtUtc=utc(), workStatus='status752_running', activeValidation={
        'name': 'registered-product752', 'launcherPid': process.pid, 'runnerPid': os.getpid(),
        'path': str(OUT), 'registered_turns': 73},
        continuation='Collect the existing752 process; do not restart. Adjudicate fresh observations against frozen73 criteria.')
    write(state_path, state)
    print(json.dumps({'started': True, 'launcher': process.pid, 'registered_turns': 73}), flush=True)
...
outcome = {'utc': utc(), 'exit_code': code, 'manifest_unchanged': sha(manifest) == prereg['manifest_sha256'],
           ...
           'source751_unchanged': all(sha(ROOT / path) == digest for path, digest in pins751.items()),
