...
OUT = ROOT / 'artifacts/comprobaciones/C03/STATUS_BATCH767'
PRIVATE = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-status-batch767-private'
PROFILE = PRIVATE.parent / 'C03-status-profile767'
...
prereg = {
    'utc': utc(), 'method': 'Registered73 product regression after truncation766 with dense764; original panel/criteria. Build prepared before sealing DLL; no model comparison.',
    ...
    'validation': 'Published766 owners/current pin integrity and Fast, dense764 owners/Fast retained. Full7 historical, no final acceptance.',
    'source766_pins': pins766, 'source764_pins': pins764, 'sources': sources, 'runner_sha256': sha(__file__),
...
outcome = {'utc': utc(), 'exit_code': code, 'manifest_unchanged': sha(manifest) == prereg['manifest_sha256'],
           ...
           'source766_unchanged': all(sha(ROOT / path) == digest for path, digest in pins766.items()),
