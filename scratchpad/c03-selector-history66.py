...
out = base / 'astra-selector-history66'
...
prereg = {'method': 'Four exact source63 native selector payloads recorded in files64-wire; clock uses its existing deterministic route. '
          ...
          'comparison66': 'Compare unchanged captured JSON context with original dialogue as native role messages and current request as the last user message; separately omit only previous assistant messages inside the original JSON as a diagnostic. Native roles retain all references; user-only is not proposed as product behavior. No other sampling, tools or system policy change.',
...
try:
    ...
    for index, packet in enumerate(selected, 1):
        for variant in ('captured', 'native_roles', 'user_history_only'):
            payload = copy.deepcopy(packet['payload'])
            context = json.loads(payload['messages'][-1]['content'])
            if variant == 'native_roles':
