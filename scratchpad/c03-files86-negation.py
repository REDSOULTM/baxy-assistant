...
files = ['tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-files86-negation.turns.jsonl',
         'scratchpad/c03-files86-negation.py']
files.append('src/Baxy.App/UserMessagePolicy.cs')
files.append('src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.dll')
files.append('artifacts/comprobaciones/C03/astra-files86-negation/CASES.json')
...
prereg['method'] = 'Source84 plus86: the existing Python/C# failure-polarity checks recognize Spanish negative result forms with omitted/impersonal subjects, while removing only negated failures before testing independent failure assertions. No new prompt, model, selector, history or provider change. Same four readonly requests/fixtures and Qwen3.5 isolated override as84 (2/4 useful; faithful empty-search drafts vetoed, next why answer invented encryption). Record all outcomes. No UI/audio/human reserve or model promotion.'
...
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-files86-negation'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
        ...
        while process.poll() is None:
            if not released:
                ...
                if time.monotonic()-started > 50:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-files86-negation.turns.jsonl').read_bytes()
