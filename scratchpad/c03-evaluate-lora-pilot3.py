"""Base vs pilot LoRA on the frozen 30 holdout and 9 historical development cases."""
...
OUT=ROOT/'artifacts/comprobaciones/C03/astra-lora-pilot3-evaluation'
assert not OUT.exists()
BASE=Path('D:/BAXYRuntime/experiments/models/qwen3-4b-instruct-2507-training-cdbee75f')
ADAPTER=BASE/'c03-pilot-lora-v3-f32.gguf'
PREVIOUS=BASE/'c03-pilot-lora-v2-f32.gguf'
MODEL=Path('D:/BAXYRuntime/experiments/models/qwen3-4b-instruct-2507-a06e946b/Qwen3-4B-Instruct-2507-Q4_K_M.gguf')
assert ADAPTER.is_file()
training=json.loads((ROOT/'artifacts/comprobaciones/C03/astra-lora-pilot3-training/RESULT.json').read_text())
assert training['status']=='trained-not-promoted' and training['gpuPeakMiB']<=4096
def digest(path):
    with path.open('rb') as stream:return hashlib.file_digest(stream,'sha256').hexdigest()
holdout=ROOT/'artifacts/comprobaciones/C03/astra-lora-pilot3-data/HOLDOUT.jsonl'
...
OUT.mkdir()
prereg={'kind':'direct-model-pilot-evaluation-not-product-acceptance','cases':cases,'variants':['pilot2','pilot3'],
...
results=[]
for variant in ['pilot2','pilot3']:
    client=Candidate(variant=='pilot3');gpu=ProcessTreeGpuSampler(os.getpid());ram=RamSampler(os.getpid())
