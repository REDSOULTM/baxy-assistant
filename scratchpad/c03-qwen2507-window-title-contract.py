...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen2507-window-title-contract'
...
files = ['src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen2507-window-title-contract.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen2507-window-title-contract/profile/sitecustomize.py',
         'scratchpad/c03-qwen2507-window-title-contract.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Named window recognition now preserves app.close, but batch argument extraction omits canonical operation description and requests a process name. Preserve description in extraction and ground byTitle against the supplied title relation instead of on/off words. Same six development cases. Audit exact extraction payload and result; no model or sampling changes.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen2507-window-title-contract'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen2507-window-title-contract.turns.jsonl')]
