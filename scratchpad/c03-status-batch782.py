"""Complete50 clock panel plus24 chained context turns through the registered, unmodified product.
...
OUT = ROOT / 'artifacts/comprobaciones/C03/STATUS_BATCH782'
PRIVATE = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-status-batch782-private'
PROFILE = PRIVATE.parent / 'C03-status-profile782'
...
# All fifteen existing clock cases, in the same order, then35 development variants.
panel = [c for c in panel if c['group'] == 'clock']
assert len(panel) == 15
for index, text in enumerate(['Muéstrame la fecha actual.', 'Pásame la fecha de hoy.', 'Enséñame la hora local.', 'Indícame la fecha de hoy.', 'Comprueba la hora local.', 'Baxy, mostrame la hora.', 'Por favor, mostrame la fecha.', '¿Puedes pasarme la hora?', 'Baxy, dame la fecha.', 'Decime la hora actual.', 'Show me the current date.', 'Give me the local time.', 'Check the current local time.', "Tell me today's date.", 'What is the local time?', 'Baxy, show me the current date.', 'Please give me the local time.', 'Could you show me the current date?', 'Tell me the current local time.', 'What date is it today?', 'Mostrame the current date.', 'Show me la hora actual.', 'Baxy, give me la fecha de hoy.', 'Por favor, show me the local time.', 'Dime qué hora es.', 'Hola, dime qué fecha es hoy.', 'Ahora mismo, indícame la hora local.', 'Responde en inglés: mostrame la fecha.', 'Answer in Spanish: show me the current date.', 'What time does this computer show?', '¿Qué hora marca mi PC?', 'Necesito saber la hora actual.', 'I need to know the time right now, what is it?', '¿Y la fecha?', '¿Y la hora?'], 1):
...
pins781 = read(ROOT / 'artifacts/comprobaciones/C03/CLOCK_CONTEXT781/SOURCE_PINS.json')
pins764 = read(ROOT / 'artifacts/comprobaciones/C03/DENSE_INVENTORY764/SOURCE_PINS.json')
assert all(sha(ROOT / path) == digest for path, digest in {**pins781, **pins764}.items())
...
prereg = {
    'utc': utc(), 'method': 'Complete50-case779 clock panel plus24 declared chained follow-ups after human context781; original clock criterion unchanged. Build prepared before sealing DLL; no model comparison.',
    'head': subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip(),
    'private': str(PRIVATE), 'parent_panel_sha256': sha(panel_path), 'panel_sha256': sha(PRIVATE / 'panel.json'), 'turns_sha256': sha(turns),
    ...
    'validation': 'Candidate781 owners/current pin integrity and Fast; not adopted yet, dense764 owners/Fast retained. Full7 historical, no final acceptance.',
    'source781_pins': pins781, 'source764_pins': pins764, 'sources': sources, 'runner_sha256': sha(__file__),
...
with (PRIVATE / 'launch.log').open('w', encoding='utf-8') as log:
    ...
    print(json.dumps({'started': True, 'launcher': process.pid, 'registered_turns': 74}), flush=True)
...
outcome = {'utc': utc(), 'exit_code': code, 'manifest_unchanged': sha(manifest) == prereg['manifest_sha256'],
           ...
           'source781_unchanged': all(sha(ROOT / path) == digest for path, digest in pins781.items()),
