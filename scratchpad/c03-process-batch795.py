"""Frozen 50-case process-read category through the registered, unmodified product.
...
OUT = ROOT / 'artifacts/comprobaciones/C03/PROCESS_BATCH795'
PRIVATE = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-process-batch795-private'
PROFILE = PRIVATE.parent / 'C03-process-profile795'
...
pins793 = read(ROOT / 'artifacts/comprobaciones/C03/INVENTORY_VETO793/SOURCE_PINS.json')
pins764 = read(ROOT / 'artifacts/comprobaciones/C03/DENSE_INVENTORY764/SOURCE_PINS.json')
assert all(sha(ROOT / path) == digest for path, digest in {**pins793, **pins764}.items())
...
prereg = {
    'utc': utc(), 'method': 'Registered50 process-read category with published793; nine historical requests and41 variants. Build prepared before sealing DLL; model selection is closed.',
    ...
    'validation': 'Published793 owners/current pin integrity and Fast, dense764 unchanged. Full7 historical; not final acceptance.',
    'source793_pins': pins793, 'source764_pins': pins764, 'sources': sources, 'runner_sha256': sha(__file__),
...
outcome = {'utc': utc(), 'exit_code': code, 'manifest_unchanged': sha(manifest) == prereg['manifest_sha256'],
           ...
           'source793_unchanged': all(sha(ROOT / path) == digest for path, digest in pins793.items()),
