...
out = ROOT / 'artifacts/comprobaciones/C03/astra-files64-wire'
...
files = ['tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-files64-wire.turns.jsonl',
         'scratchpad/c03-files64-wire.py']
files.append('src/Baxy.App/UserMessagePolicy.cs')
files.append('src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.dll')
files.append('artifacts/comprobaciones/C03/astra-files64-wire/CASES.json')
...
files += ['src/Baxy.App/PlannerExecutionSupport.cs', 'src/Baxy.Kernel/Planning/MissionPlanProposal.cs', 'src/Baxy.Providers.Windows/Filesystem/LocalFilesystemProvider.cs', 'src/Baxy.Core/Operations/FilesystemHandlers.cs']
files.append('scratchpad/c03-wire-hook47/sitecustomize.py')
...
prereg['method'] = 'Files53 wire. Five readonly technical controls on an isolated normal conductor profile. Three sandbox fixtures and one explicitly outside fixture are prepared and hashed in CASES. An outside path shares its basename with a sandbox file: substitution is a failure. Reuse the absent47 literal request and add actual readable/invalidUTF8 files plus clock recovery. Registered model/template/sampler, no overrides, no UI/voice or human reserve claim. Baseline is source52; candidate adds the existing verified-resource producer dependency for read.text; boundary aligns the C# producer relation and resourceId authority with the Python candidate. literals changes only the code-shape veto to preserve complete identifiers supplied in the request. trace keeps source54 behavior and lowercases exception type/method labels for the existing safe trace; no exception content is logged. reason preserves structured mission failure reasons through the C# literal collector instead of calling GetString on an object. identity adds resourceId to the existing deterministic verified-identity projection in Python and App; no model decode when the permitted producer proves a unique identity. cause also projects error from failed typed operation results through the existing cause-to-prose transformation, without overriding explicit causes or successful polarity. modal replaces first-person-only English inability detection with the same negative modals regardless of subject in C#; Python composition remains source57. absolute adds typed absolute_path_search_unsupported validation to the filename-only sandbox search provider, using Path.IsPathFullyQualified; preserves relative name lookup and never substitutes a basename. Core is republished before this stage. No fixture deletion, no write operations requested of BAXY. Inspect operations, facts, progress and finals.'
...
env.update(BAXY_MIND_PYTHONPATH=str(ROOT/'scratchpad/c03-wire-hook47')+os.pathsep+str(ROOT/'src'), C03_WIRE_TRACE_DIR=str(out))
app=ROOT/'src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.exe'
commit=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip()
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-files64-wire'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
prereg['wireMethod']='Same source63 and five inputs; opt-in hook47 captures exact HTTP payloads/replies without altering them. Diagnose selection recovery after absolute-search failure; not timing acceptance or a repeat to seek a pass.'
        ...
        while process.poll() is None:
            if not released:
                ...
                if time.monotonic()-started > 50:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-files64-wire.turns.jsonl').read_bytes()
