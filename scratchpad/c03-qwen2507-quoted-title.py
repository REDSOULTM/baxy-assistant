...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen2507-quoted-title'
...
files = ['src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen2507-quoted-title.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen2507-quoted-title/profile/sitecustomize.py',
         'scratchpad/c03-qwen2507-quoted-title.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Literal target now resolves and closes after confirmation. The final reply was rejected as internal_code because the C# stutter detector erased quotation boundaries. Preserve punctuation boundaries when detecting adjacent repeated words, without changing facts or admitting actual stutters. Same six development cases and model profile.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen2507-quoted-title'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen2507-quoted-title.turns.jsonl')]
