...
out = root / 'artifacts/comprobaciones/C03/astra-cancel-scope378'
private = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-cancel-scope378-private'
...
for text in ['cancel', 'cancelar']:
    ...
    if text == 'cancel':
        ...
        reference['messages'][-1]['content'] = reference['messages'][-1]['content'].replace(old, new)
