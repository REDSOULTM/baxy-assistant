...
OUT = ROOT / 'artifacts/comprobaciones/C03/STATUS_BATCH772'
PRIVATE = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-status-batch772-private'
PROFILE = PRIVATE.parent / 'C03-status-profile772'
...
pins771 = read(ROOT / 'artifacts/comprobaciones/C03/INVENTORY_SCOPE771/SOURCE_PINS.json')
pins764 = read(ROOT / 'artifacts/comprobaciones/C03/DENSE_INVENTORY764/SOURCE_PINS.json')
assert all(sha(ROOT / path) == digest for path, digest in {**pins771, **pins764}.items())
...
prereg = {
    'utc': utc(), 'method': 'Registered73 product regression after inventory scope and correction771 with dense764; original panel/criteria. Build prepared before sealing DLL; no model comparison.',
    ...
    'validation': 'Published771 owners/current pin integrity and Fast, dense764 owners/Fast retained. Full7 historical, no final acceptance.',
    'source771_pins': pins771, 'source764_pins': pins764, 'sources': sources, 'runner_sha256': sha(__file__),
...
outcome = {'utc': utc(), 'exit_code': code, 'manifest_unchanged': sha(manifest) == prereg['manifest_sha256'],
           ...
           'source771_unchanged': all(sha(ROOT / path) == digest for path, digest in pins771.items()),
