...
out = root / 'artifacts/comprobaciones/C03/astra-read-recovery412'
private = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-read-recovery412-private'
...
cases_path = Path('C:\\Users\\emman\\AppData\\Local\\BAXY\\C03-read-recovery412-cases-private\\cases.json')
...
hook = private / 'hook'
hook.mkdir()
hook_source = (root / 'scratchpad/c03-owner370-hook/sitecustomize.py').read_text(encoding='utf-8').replace('C03-memory-product370-private', 'C03-read-recovery412-private')
...
hook_source += (root / 'scratchpad/c03-early-read412-hook.py').read_text(encoding='utf-8')
(hook / 'sitecustomize.py').write_text(hook_source, encoding='utf-8')
prereg = {'utc': datetime.now(timezone.utc).isoformat(), 'case_ids': [c['id'] for c in cases], 'private_case_file': str(cases_path),
    'method': 'Fourteen fixed development cases: actual411T3 with exact last12role history plus13controls409. Source410 and current authenticated catalog, same model/template/sampler, two sequential sidecars: actual411T3 first immediately after catalog.ready, then the14cases after semantic readiness (15requests per arm). Audit must confirm lexical mode for the initial target before attributing its comparison to the real411 boundary. Diagnostic-only variant runs a second existing native decision over the first4 offered candidates when the primary is knowledge/social without effects, and retains it only if all proposed operations have authenticated read_only risk. The normal information/domain/compound/action/grounding/confirmation guards still run. No injected answers, fixed identity operation, effect execution, source change or promotion. Original late fallback remains in this diagnostic; successful adoption must move the existing repair rather than add duplicate passes.',
...
for variant in ['baseline', 'early-read']:
    ...
    try:
        ...
        configure = {'id': 'catalog412', 'type': 'catalog.configure', 'capabilities': current_capabilities}
        ...
        client.close(graceful_message={'id': 'close412', 'type': 'shutdown'}, timeout=15)
