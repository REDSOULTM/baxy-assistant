...
out = root/'artifacts/comprobaciones/C03/astra-memory-product366'
private = Path(os.environ['LOCALAPPDATA'])/'BAXY/C03-memory-product366-private'
...
prereg = {
    ...
    'method':'Ten explicitly synthetic development controls in one continuous private profile: namesJordan/Álvaro, ES/EN, ask what confirmation would authorize, cancel, then reference the session name without having enabled persistence. Same modelQwen3.5 and ten controls364; source365 joins explicit name declaration and name-save authority, including compound names, without changing model/prompt or public response guards; no runtime promotion, no classifier or response injection. These are not historical human messages or fresh acceptance. No graphical UI or physical voice evidence.',
    'profile_inheritance':'Dedicated LOCALAPPDATA/BAXY child, starts with persistence disabled. Only the typed product pipeline may change it. Requests expressly ask to remember a synthetic name but later cancel both enable challenges. No authorisation to affect owner memory; all journal/state are in the diagnostic profile. Model/sampling/template unchanged364.',
...
for name in list(env):
    if name.startswith(('BAXY_MIND_','BAXY_VOICE_','BAXY_C03_','BAXY_FIELD_')) or name in {'PYTHONPATH','BAXY_DATA_DIR','BAXY_ASSET_DESCRIPTOR','BAXY_APP_TRACE'}:
        env.pop(name,None)
env.update(BAXY_MIND_LLM_GGUF=str(model), BAXY_MIND_PYTHONPATH=str(root/'scratchpad/c03-owner366-hook')+os.pathsep+str(root/'src'), BAXY_VOICE_WAKE_ON_START='0',
...
command=['py','main.py','--conductor','--profile',str(private.parent/'C03-memory-profile366'),
