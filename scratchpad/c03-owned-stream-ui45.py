...
OUT=ROOT/'artifacts/comprobaciones/C03/astra-owned-stream-ui45'
...
for key in list(env):
    if key.startswith(('BAXY_MIND_','BAXY_VOICE_','BAXY_C03_','BAXY_FIELD_')) or key in {'BAXY_ASSET_DESCRIPTOR','PYTHONPATH','BAXY_DATA_DIR'}:
        del env[key]
env.update(BAXY_MIND_PYTHONPATH=str(ROOT/'scratchpad/c03-native-hook45')+os.pathsep+str(ROOT/'src'), C03_NATIVE_TRACE_DIR=str(OUT), BAXY_APP_TRACE=str(OUT/'shell-trace.jsonl'),BAXY_MIND_MESSAGE_COMPOSE_AUDIT_PATH=str(OUT/'compose-audit.jsonl'),BAXY_MIND_MESSAGE_COMPOSE_AUDIT_CONTENT='1',BAXY_MIND_TURN_AUDIT_PATH=str(OUT/'turn-audit.jsonl'),BAXY_MIND_RAW_REPLY_AUDIT_PATH=str(OUT/'raw-replies.jsonl'))
(OUT/'PREREG.json').write_text(json.dumps({'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'command':['py','main.py'],'sourcePins':json.loads((ROOT/'artifacts/comprobaciones/C03/TRAMO44_PINS.json').read_text(encoding='utf-8')),'registrationSha256':digest,'method':'Owned blocking output stream candidate, same native diagnostic real-window launch. Registered model/KV/voice/Python; the sole diagnostic override adds scratchpad/c03-native-hook45 to PythonPath for sitecustomize faulthandler files. No source/runtime installation changed; not acceptance. Manual Windows skill input and inspection. Observe current real UI with the source validated in tranche44. Request the consumed negative-state question, both clock/independent-prohibition controls and conversation. If an old recovery appears, inspect it; do not assume prior pending state. No data deletion or external messaging. Development only, not fresh acceptance. Existing audit sinks only; resource sampler attaches exclusively to the newly launched exact Baxy.exe.'},indent=2),encoding='utf-8')
