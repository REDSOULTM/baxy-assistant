...
private = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-voice154-private'
...
try:
    ...
    for phase in prereg['phases']:
        ...
        assert engine.speak(prereg['texts'][phase])
...
np.savez(private/'tap154.npz', microphone=np.asarray(tap_mic), reference=np.asarray(tap_ref), clean=np.asarray(tap_clean))
(private/'tap154.json').write_text(json.dumps({'anchor': tap_anchor, 'frames': tap_rows}, indent=2), encoding='utf-8')
index = [{'privatePath': str(private/name), 'sha256': sha(private/name)} for name in ['tap154.json', 'tap154.npz']]
