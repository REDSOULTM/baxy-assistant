...
out = root/'artifacts/comprobaciones/C03/astra-memory-product370'
private = Path(os.environ['LOCALAPPDATA'])/'BAXY/C03-memory-product370-private'
...
prereg = {
    ...
    'method':'Ten explicitly synthetic development controls in one continuous private profile: namesJordan/Álvaro, ES/EN, ask what confirmation would authorize, cancel, then reference the session name without having enabled persistence. Same modelQwen3.5 and ten controls366; source369 preserves the complete initial confirmation facts for invalid replies and the public pending action during reconciliation, without changing model/prompt, name parser or public response guards; no runtime promotion, no classifier or response injection. These are not historical human messages or fresh acceptance. No graphical UI or physical voice evidence.',
...
for name in list(env):
    if name.startswith(('BAXY_MIND_','BAXY_VOICE_','BAXY_C03_','BAXY_FIELD_')) or name in {'PYTHONPATH','BAXY_DATA_DIR','BAXY_ASSET_DESCRIPTOR','BAXY_APP_TRACE'}:
        env.pop(name,None)
env.update(BAXY_MIND_LLM_GGUF=str(model), BAXY_MIND_PYTHONPATH=str(root/'scratchpad/c03-owner370-hook')+os.pathsep+str(root/'src'), BAXY_VOICE_WAKE_ON_START='0',
...
command=['py','main.py','--conductor','--profile',str(private.parent/'C03-memory-profile370'),
