"""Bounded physical product voice session: RAW, integrated DTLN512, VAD, Piper and ASR."""
...
from webrtc_aligned254 import EchoCanceller as CandidateCanceller
voice.EchoCanceller = CandidateCanceller
voice.resolve_echo_canceller_directory = lambda: Path("D:/BAXYRuntime/experiments/voice/webrtc176/python")
capture_copy = ROOT / "scratchpad/capture_split252.py"
exec(compile(capture_copy.read_text(encoding="utf-8"), str(capture_copy), "exec"), vars(voice))
voice.VoiceEngine._capture_loop = voice._capture_loop
candidate_session = None
original_candidate_init = CandidateCanceller.__init__
def initialize_candidate(self):
    global candidate_session
    original_candidate_init(self)
    candidate_session = self
CandidateCanceller.__init__ = initialize_candidate

OUT = ROOT / "artifacts/comprobaciones/C03/astra-voice254"
PRIVATE = Path(os.environ["LOCALAPPDATA"]) / "BAXY/C03-voice254-private"
...
def main():
    ...
    assert voice.EchoCanceller.__module__ == "webrtc_aligned254"
    assert candidate_session is None
    ...
    save(OUT / "PREREG.json", {
        ...
        "sourceFiles": {str(p): sha(ROOT / p) for p in [
            ...
            "src/baxy_mind/dtln_aec.py", "src/baxy_mind/voice_output.py", "src/baxy_mind/piper_tts.py"]},
        "probeSha256": sha(helper), "lockSha256": sha(ROOT / "pylock.runtime-win-x64.toml"), "captureCopySha256": sha(capture_copy), "adapterSha256": sha(ROOT / "scratchpad/webrtc_aligned254.py"), "aecDirectory": str(voice.resolve_echo_canceller_directory()),
    })
    capacity = 2000
    near = np.empty((capacity, 512), np.float32)
    reference = np.empty((capacity, 4512), np.int16)
    clean = np.empty((capacity, 512), np.float32)
    observations = np.full((capacity, 7), np.nan, np.float64)
    confirmation = np.empty((capacity, 512), np.float32)
    count = 0
    owner = None
    effects = []
    generated = []
    ...
    def observe_process(self, microphone, history):
        ...
        confirmation[i] = self.last_final
        observations[i] = [time.monotonic(), (after - begin) * 1000, 0, float(engine.speaking), np.nan, -1, np.nan]
        count += 1
    ...
    def observe_vad(self, frame):
        ...
        if count and threading.get_ident() == owner:
            observations[count - 1, 6 if self is engine._confirmation_vad else 4] = result
        return result
    ...
    with _com_apartment():
        ...
        try:
            ...
            engine.load()
            engine._confirmation_vad = voice.SileroVad()
            ...
            print("Physical RAW/experimental split AEC3 direct session ready; four fixed TTS controls.", flush=True)
            ...
            np.savez(PRIVATE / "tap254.npz", microphone=near[:count], reference=reference[:count],
                clean=clean[:count], confirmation=confirmation[:count], observations=observations[:count])
            np.savez(PRIVATE / "generated254.npz", **{f"audio{i}": item[0] for i, item in enumerate(generated)})
            save(PRIVATE / "generated254.json", [item[1] for item in generated])
            ...
            result = {"frames": count, "audioSeconds": count * .032, "effects": effects,
                ...
                "columns": ["monotonic", "dspMs", "copyMs", "speaking", "linearVadProbability", "echoGuard", "confirmationVadProbability"],
