...
OUT = ROOT / "artifacts/comprobaciones/C03/astra-voice243"
PRIVATE = Path(os.environ["LOCALAPPDATA"]) / "BAXY/C03-voice243-private"
...
def main():
    ...
    assert voice.EchoCanceller.__module__ == "baxy_mind.dtln_aec"
    ...
    save(OUT / "PREREG.json", {
        ...
        "method": "Source242 VoiceEngine with integrated DTLN512 from declared assets and installed locked LiteRT2.2.0, actual RAW capture/Silero/Piper/baxy.2. Four fixed187 texts at the same schedule as240. No replacement AEC factory, no experimental imports, no LLM/App/UI/wake seam, no decoder or threshold changes. Bounded observers record actual processed audio and decisions.",
        ...
        "probeSha256": sha(helper), "lockSha256": sha(ROOT / "pylock.runtime-win-x64.toml"), "aecDirectory": str(voice.resolve_echo_canceller_directory()),
    })
    capacity = 2000
    ...
    with _com_apartment():
        ...
        try:
            ...
            print("Physical RAW/integrated DTLN512 direct session ready; four fixed TTS controls.", flush=True)
            ...
            np.savez(PRIVATE / "tap243.npz", microphone=near[:count], reference=reference[:count],
                clean=clean[:count], observations=observations[:count])
            np.savez(PRIVATE / "generated243.npz", **{f"audio{i}": item[0] for i, item in enumerate(generated)})
            save(PRIVATE / "generated243.json", [item[1] for item in generated])
