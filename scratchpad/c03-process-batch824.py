"""Frozen 50-case process-read category through the registered product with narrator823.

Inherits conductor, resource guards and frozen panel from batch822.
Python-only prompt823 requests the observed accessible process count in natural language; budgets and facts unchanged.
...
assert Path(__file__).resolve().parent.name == 'scratchpad', 'Copy proposal into canonical scratchpad before execution'
...
PRIVATE = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-process-batch824-private'
PROFILE = PRIVATE.parent / 'C03-process-profile824'
...
plan = read(plan_path)
# Python-only narrator823 requires its own owners and Fast; Full818 is inherited baseline evidence.
...
assert read(validation_root / 'OWNERS_REFERENCE_EXIT.json')['exit_code'] == 0
for filename in ('OWNERS_PYTHON_EXIT.json', 'OWNERS_REFERENCE_EXIT.json'):
    receipt = read(validation_root / filename)
    assert receipt['source_pins_unchanged'] is True
    assert receipt['source_pins_sha256'] == sha(validation_root / 'SOURCE_PINS.json')
    assert receipt['passed'] > 0
    if filename == 'OWNERS_PYTHON_EXIT.json':
        assert receipt['skipped'] == 0
baseline_validation_root = ROOT / 'artifacts/comprobaciones/C03/PROCESS_REPAIRS818/FULL_RETRY1'
full_validation_root = baseline_validation_root
assert read(baseline_validation_root / 'OWNERS_DOTNET_EXIT.json')['exit_code'] == 0
...
pins823 = read(validation_root / 'SOURCE_PINS.json')
assert len(pins823) == 33, 'narrator823 source pin set'
assert all(sha(ROOT / path) == digest for path, digest in pins823.items())
...
prereg = {
    'utc': utc(), 'method': 'Registered50 process-read category with sealed candidate823; nine historical requests and41 variants. Build prepared before sealing DLL; model selection is closed.',
    ...
    'validation': 'Python-only narrator823 requires its own 33 source pins, Python/reference owners and Fast exit0. Full and .NET owners are inherited from PROCESS_REPAIRS818/FULL_RETRY1 only; baseline pins are separate and are not asserted equal to candidate823. Adoption and final Full remain governed by the goal; no coverage credit.',
    ...
    'owners_dotnet_path': str(baseline_validation_root / 'OWNERS_DOTNET_EXIT.json'),
    'owners_dotnet_sha256': sha(baseline_validation_root / 'OWNERS_DOTNET_EXIT.json'),
    'owners_reference_path': str(validation_root / 'OWNERS_REFERENCE_EXIT.json'),
    'owners_reference_sha256': sha(validation_root / 'OWNERS_REFERENCE_EXIT.json'),
    'source823_pins_sha256': sha(validation_root / 'SOURCE_PINS.json'),
    ...
    'source823_pins': pins823, 'sources': sources, 'runner_sha256': sha(__file__),
    'reviewer_sha256': sha(ROOT / 'scratchpad/c03-review-process824.py'),
...
outcome = {'utc': utc(), 'exit_code': code, 'manifest_unchanged': sha(manifest) == prereg['manifest_sha256'],
           ...
           'source823_unchanged': all(sha(ROOT / path) == digest for path, digest in pins823.items()),
