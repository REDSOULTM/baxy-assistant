...
out=base/'astra-audio-mind504';private=local/'C03-audio-mind504-private'
...
write(out/'PREREG.json',{'utc':datetime.now(timezone.utc).isoformat(),'method':'Full production mind sidecar with current catalogue/real app snapshots, source503 explicit need/acknowledgement and unmute family repair; exact14cases/binding protocol comparison against502/source501; eight exact baseline488 cases plus explicit positive-but and the actual C# ResumeObjective string and current registered model/runtime. Actual recovered owner264 indexes44/46/51 with exact immediately relevant pairs, plus five synthetic development controls. catalog.configure, turn.decide and actual arguments/plan requests matching the shell: no provider invocations, volume/mute changes, UI or speech. Transmit only actual selected operations to later protocol stages, never the expected list from the test case. Follow the current C# shape: plan gets the real history and expectedOperations from turn.result; direct arguments get selected operation and current text. Additional development controls cover the prior missing-level request, a contextual negative clause, a new time request and English numeric continuation. Observe native requests/replies and route through actual lexical/semantic retrieval/argument guards; no forced selection/reply injection. Wait for actual E5 resources readiness as409; no fake readiness.','profile_reason':'Current registered runtime baseline for source regression, not a model-capability ranking or claim that defaults are optimal. Model-specific profile research448-484 remains; a bad result here is inspected at its first boundary, not a new default-based model rejection.','cases':cases,'criteria':'Useful operation-level proposal preserving both compound effects and order; correct grounded level and unmute=false where returned; negative/knowledge controls must not unmute. A plan with arguments to be bound later only proves selection, not provider success. Unresolved owner46/51 remain open until whole product is demonstrated. The resumed case uses the literal string built by current C# but does not run its UI policy or prove that the shell reaches that branch. state is the actual mute argument; muted in old expectation notes was an observation field, never a returned or verified argument. No reserve, acceptance or physical-resource credit.','model':config['gguf'],'model_sha256':sha(config['gguf']),'server':config['llama_server'],'server_sha256':sha(config['llama_server']),'manifest_sha256':sha(manifest),'core':str(core),'core_sha256':sha(core),'source_sha256':{str(p):sha(p) for p in [root/'src/baxy_mind/effect_intent.py',root/'src/baxy_mind/llm.py',root/'src/baxy_mind/__main__.py']},'private':str(private),'limits':{'gpu_stop_mib':3800,'minimum_free_ram_mib':768,'request_seconds':90}})
hook=private/'hook';hook.mkdir()
hook_source=(root/'scratchpad/c03-owner370-hook/sitecustomize.py').read_text(encoding='utf-8-sig').replace('C03-memory-product370-private','C03-audio-mind504-private')
hook_source+=(root/'scratchpad/c03-observe407-hook.py').read_text(encoding='utf-8-sig').replace("Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-retrieval-startup407-private/startup-observer.jsonl'","Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-audio-mind504-private/startup.jsonl'")
...
try:
 ...
 config_req={'id':'catalog504','type':'catalog.configure','capabilities':capabilities}
 ...
 stop.set();client.close(graceful_message={'id':'close504','type':'shutdown'},timeout=15);guard.join(timeout=5);gpu.stop();ram.stop()
