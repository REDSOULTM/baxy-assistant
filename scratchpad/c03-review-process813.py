"""Correlate frozen813 outputs; no verdict inferred from terminal status."""
...
public = root / 'artifacts/comprobaciones/C03/PROCESS_BATCH813'
truncated = []
...
assert hashlib.sha256(scope_pins_path.read_bytes()).hexdigest() == prereg['source812_pins_sha256']
assert read(scope_pins_path) == prereg['source812_pins']
assert len(prereg['source812_pins']) == 31
assert read(validation_root / 'VALIDATION_EXIT.json')['source_pins_unchanged'] is True
assert read(validation_root / 'VALIDATION_EXIT.json')['source_pins_sha256'] == prereg['source812_pins_sha256']
...
if not args.partial:
    ...
    assert all(outcome[k] for k in ['manifest_unchanged', 'sources_unchanged',
                                  'source812_unchanged', 'runner_unchanged', 'app_dll_unchanged'])
...
if not args.partial:
    ...
    markdown = ['# Respuestas privadas813 — pendientes de adjudicación',
