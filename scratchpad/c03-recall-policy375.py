...
out = root / 'artifacts/comprobaciones/C03/astra-recall-policy375'
private = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-recall-policy375-private'
...
hook = private / 'hook'
hook.mkdir()
hook_source = (root / 'scratchpad/c03-owner370-hook/sitecustomize.py').read_text(encoding='utf-8').replace('C03-memory-product370-private', 'C03-recall-policy375-private')
(hook / 'sitecustomize.py').write_text(hook_source, encoding='utf-8')
prereg = {'utc': datetime.now(timezone.utc).isoformat(), 'cases': cases,
    'method': 'Same five development inputs/history371 through actual sidecar turn.decide with authenticated current Core catalog. Core starts only for its read-only catalog hello, no operation request. Mind can propose actions but this diagnostic never executes them. Same five cases and authenticated catalog as373; source374 narrows the observation guard to machine subjects, without changing first model payload or conversation history. No private dispatch/parser change, runtime promotion, fresh human acceptance, UI or voice.',
...
try:
    ...
    configure = {'id': 'catalog375', 'type': 'catalog.configure', 'capabilities': capabilities}
