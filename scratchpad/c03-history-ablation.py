...
out=root/'artifacts/comprobaciones/C03/astra-history-ablation'
...
for target in ['t8','t9','t10','t13','t14','t15']:
    ...
    for row in rows:
        if row['turnId']==target:
            ...
            candidate['messages']=[m for m in candidate['messages'] if m['role']!='assistant']
            panels.append({'turnId':target,'request':row['request'],'full':original,'user_only':candidate})
            break
...
os.environ.update(BAXY_MIND_LLM_GGUF=str(model),BAXY_MIND_NGL='99',BAXY_MIND_KV_CACHE_TYPE='q8_0')
prereg={'kind':'direct-model-context-ablation-not-product-acceptance','source':str(source),'sourceSha256':hashlib.sha256(source.read_bytes()).hexdigest(),'history':'Reconstructed from published paired turns using MainWindowViewModel.BuildMindHistory 12-message rule. Not a captured original HTTP prompt. Same reconstructed history on both sides.','hypothesis':'Prior assistant prose contaminates new knowledge answers. Removing assistant messages retains earlier user preferences and requests. No production source changes.','cases':panels,'sampling':{'temperature':.7,'top_p':.8,'top_k':20,'min_p':0,'seed':0},'llmSha256':hashlib.sha256((root/'src/baxy_mind/llm.py').read_bytes()).hexdigest()}
...
try:
    ...
    with (out/'replies.jsonl').open('w',encoding='utf-8') as stream:
        for i,panel in enumerate(panels):
            for variant in (['full','user_only'] if i%2==0 else ['user_only','full']):
