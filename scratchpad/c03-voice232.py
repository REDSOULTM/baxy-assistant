...
OUT = ROOT / "artifacts/comprobaciones/C03/astra-voice232"
PRIVATE = Path(os.environ["LOCALAPPDATA"]) / "BAXY/C03-voice232-private"
...
def main():
    ...
    save(OUT / "PREREG.json", {
        ...
        "method": "Actual source230 VoiceEngine direct session with provisional barge admission, production RAW capture/Speex/Silero/Piper/installed baxy.2; four fixed187 TTS texts, one per10seconds after2s warmup. No LLM/App/UI/wake seam, no alternative decoder or thresholds. Record exact microphone/reference/clean with bounded observer after actual Speex process, and VAD/guard decisions.",
    ...
    with _com_apartment():
        ...
        try:
            ...
            print("Physical RAW/Speex direct session ready; four fixed TTS controls.", flush=True)
            ...
            np.savez(PRIVATE / "tap232.npz", microphone=near[:count], reference=reference[:count],
                clean=clean[:count], observations=observations[:count])
            np.savez(PRIVATE / "generated232.npz", **{f"audio{i}": item[0] for i, item in enumerate(generated)})
            save(PRIVATE / "generated232.json", [item[1] for item in generated])
