...
BASE=ROOT/'artifacts/comprobaciones/C03';OUT=BASE/'astra-real-dialogue-system-layers';OUT.mkdir(exist_ok=False)
...
prereg={'cases':cases,'variants':['bare','identity','baseline'],'change':{'before':old,'after':new},'method':'Six consumed real-log cases, standalone chat knowledge/es, same model/cache/sampling and normal guards. Bare removes system messages; identity retains one minimal BAXY identity plus no-unverified-actions constraint; baseline retains existing system/policies. No PC effects or fresh acceptance. Tests whether system/policy layers degrade the actual negative request and knowledge controls.', 'registrationSha256':hashlib.sha256(register.read_bytes()).hexdigest()}
...
class Client(LlmRuntime):
    ...
    def _post(self,payload,*args,**kwargs):
        ...
        if self.variant in {'bare','identity'}:
            payload['messages']=[m for m in payload['messages'] if m['role']!='system']
            if self.variant=='identity':payload['messages'].insert(0,{'role':'system','content':'Eres BAXY, un compañero que vive en el PC. Eres un él. Tuteas. Responde al mensaje de la persona en su idioma. No afirmes haber ejecutado acciones.'})
