...
out = ROOT / 'artifacts/comprobaciones/C03/astra-files75-http'
...
files = ['tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-files75-http.turns.jsonl',
         'scratchpad/c03-files75-http.py']
files.append('src/Baxy.App/UserMessagePolicy.cs')
files.append('src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.dll')
files.append('artifacts/comprobaciones/C03/astra-files75-http/CASES.json')
...
files.append('scratchpad/c03-wire-hook75/sitecustomize.py')
...
prereg['method'] = 'Same source74/model override/ten requests and fixtures. Diagnostic hook75 additionally records HTTPError status and existing io.BytesIO body by getvalue, without consuming it or altering the request/response/error. 72/74 t6/t7 fail at conversation_reply with HTTPError; t10 fails at turn_preparation. The prior hook logged only successes, so no rejected payload was captured. Find the first server rejection cause. No new source change, registry promotion, UI/audio or human acceptance. No volume level supplied; no mutation requested.'
...
env.update(BAXY_MIND_PYTHONPATH=str(ROOT/'scratchpad/c03-wire-hook75')+os.pathsep+str(ROOT/'src'), C03_WIRE_TRACE_DIR=str(out))
app=ROOT/'src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.exe'
commit=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip()
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-files75-http'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
        ...
        while process.poll() is None:
            if not released:
                ...
                if time.monotonic()-started > 50:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-files75-http.turns.jsonl').read_bytes()
