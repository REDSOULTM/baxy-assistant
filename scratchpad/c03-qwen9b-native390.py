...
out = root / 'artifacts/comprobaciones/C03/astra-qwen9b-native390'
private = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-qwen9b-native390-private'
...
prereg = {'utc': datetime.now(timezone.utc).isoformat(), 'cases': prior['cases'],
    ...
    'ngl': 14, 'mmap': False, 'method': '389 stopped before the first answer because the conservative free-RAM768MiB bound fired; GPU peak2905.57MiB, RAM5770.91MiB.390 keeps all criteria, sampler, cases, ngl14, and resource watchdogs; the sole loading difference is --no-mmap. Official b9980 server documentation says disabling mapping can reduce pageouts (slower load), not that it guarantees less RAM. Do not relax resource bounds. Four identical original-description native controls387. Only GGUF4B→9B and mandatory partial offload99→14 change. Same captured BAXY native prompt, tools, history, sampler, token allowance and transport/template handling. No account-scope description variant, App, guarded mind, operations, UI, voice or fresh-human acceptance. This is a capability/resource diagnostic, not model-alone or latency acceptance.',
...
class LoggedRuntime(LlmRuntime):
    def _server_command(self):
        return [*super()._server_command(), '--no-mmap', '--log-file', str(private / 'server.log')]
