...
out = ROOT / 'artifacts/comprobaciones/C03/astra-files53-candidate'
...
files = ['tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-files53-candidate.turns.jsonl',
         'scratchpad/c03-files53-candidate.py']
files.append('src/Baxy.App/UserMessagePolicy.cs')
files.append('src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.dll')
files.append('artifacts/comprobaciones/C03/astra-files53-candidate/CASES.json')
...
prereg['method'] = 'Files53 candidate. Five readonly technical controls on an isolated normal conductor profile. Three sandbox fixtures and one explicitly outside fixture are prepared and hashed in CASES. An outside path shares its basename with a sandbox file: substitution is a failure. Reuse the absent47 literal request and add actual readable/invalidUTF8 files plus clock recovery. Registered model/template/sampler, no overrides, no UI/voice or human reserve claim. Baseline is source52; candidate adds the existing verified-resource producer dependency for read.text. No fixture deletion, no write operations requested of BAXY. Inspect operations, facts, progress and finals.'
...
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-files53-candidate'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
        ...
        while process.poll() is None:
            if not released:
                ...
                if time.monotonic()-started > 50:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-files53-candidate.turns.jsonl').read_bytes()
