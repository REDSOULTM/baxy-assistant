"""Refresh470 cumulative335 exposure; no case is executed, displayed or frozen."""
...
prior = base / 'C03-exposure335-private/review.jsonl'
...
out = base / 'C03-exposure470-private'
...
with (out/'review.jsonl').open('x', encoding='utf-8') as stream:
    for row in rows:
        ...
        row['current_exposure470'] = sorted(matches.get(row['id'], []))
        if row['current_exposure470']:
...
summary = dict(prior_review=str(prior), cumulative_prior_exposure_preserved=True, joined=len(rows), source_count=len(sources), matched_current=len(matches),
    newly_exposed_since335=len(newly_exposed), both_positive_still_requires_audit=len(remaining),
...
public = root/'artifacts/comprobaciones/C03/astra-exposure470'
