...
out = ROOT / 'artifacts/comprobaciones/C03/astra-polarity52'
...
files = ['tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-polarity52.turns.jsonl',
         'scratchpad/c03-polarity52.py']
files.append('src/Baxy.App/UserMessagePolicy.cs')
files.append('src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.dll')
files.append('artifacts/comprobaciones/C03/astra-polarity52/CASES.json')
...
prereg['method'] = 'Same seven technical inputs, registered runtime and source51 except C# failure-polarity detection. Negated failure assertions no longer count as failed outcomes; independent affirmative failures remain visible to the same validator. Native generation, Python progress, catalog and providers unchanged. The hour/CPU case failed identically in48/49/51 despite faithful drafts because No hay fallos matched Contains(fallo). New owner tests reproduce both reversal directions and contrast independent failures. Read-only product conductor; no human reserve, UI or voice claim. Judge whole turns including progress.'
...
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-polarity52'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
        ...
        while process.poll() is None:
            if not released:
                ...
                if time.monotonic()-started > 50:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-polarity52.turns.jsonl').read_bytes()
