...
out = ROOT / 'artifacts/comprobaciones/C03/astra-topic-context47'
...
files = ['tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-topic-context47.turns.jsonl',
         'scratchpad/c03-topic-context47.py']
files.append('src/Baxy.App/UserMessagePolicy.cs')
files.append('src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.dll')
files.append('artifacts/comprobaciones/C03/astra-topic-context47/CASES.json')
...
prereg['method'] = 'Same10 consumed technical prefix and order as wire-context47. Registered model/backend/sampler unchanged; same opt-in exact-payload logging hook. Only request_reading terminal style modifier changes. Tests preserve existing subjects, reference-dependent explanations and caller history. The raw failed-panel reconstruction reproduced the Moon phrase with the identical captured policies; no-context control removed it. Product run checks that this conditional scope reaches the real generation. Not acceptance, not UI/audio, not repeat-until-green. Known CPU omission and literal-path resolution failures remain separately recorded and are not retried in this prefix.'
...
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-topic-context47'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
        ...
        while process.poll() is None:
            if not released:
                ...
                if time.monotonic()-started > 50:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-topic-context47.turns.jsonl').read_bytes()
