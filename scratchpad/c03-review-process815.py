"""Correlate frozen815 outputs; no verdict inferred from terminal status."""
...
public = root / 'artifacts/comprobaciones/C03/PROCESS_BATCH815'
truncated = []
...
assert hashlib.sha256(scope_pins_path.read_bytes()).hexdigest() == prereg['source814_pins_sha256']
assert read(scope_pins_path) == prereg['source814_pins']
assert len(prereg['source814_pins']) == 31
assert read(validation_root / 'VALIDATION_EXIT.json')['source_pins_unchanged'] is True
assert read(validation_root / 'VALIDATION_EXIT.json')['source_pins_sha256'] == prereg['source814_pins_sha256']
...
if not args.partial:
    ...
    assert all(outcome[k] for k in ['manifest_unchanged', 'sources_unchanged',
                                  'source814_unchanged', 'runner_unchanged', 'app_dll_unchanged'])
...
if not args.partial:
    ...
    markdown = ['# Respuestas privadas815 — pendientes de adjudicación',
