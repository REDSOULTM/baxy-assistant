...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen2507-mixed-evidence'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen2507-mixed-evidence.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen2507-mixed-evidence/profile/sitecustomize.py',
         'scratchpad/c03-qwen2507-mixed-evidence.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'The finite lexical mixed-language guard falsely rejects meaningful code-switching outside its vocabulary. For suspected monolingual replies only, reuse the existing semantic language classifier; keep rejecting genuinely monolingual output and keep fact/polarity/authority checks. Same model, sampling, six controls and budgets as qwen2507-explanation. No model promotion.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen2507-mixed-evidence'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen2507-mixed-evidence.turns.jsonl')]
