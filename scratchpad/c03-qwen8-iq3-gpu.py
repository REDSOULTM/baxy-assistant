...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen8-iq3-gpu'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen8-iq3-gpu.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen8-iq3-gpu/profile/sitecustomize.py',
         'scratchpad/c03-qwen8-iq3-gpu.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Same single IQ3_XXS quant, now full GPU: ngl30 measured only3093.57MiB total BAXY but still hit inference deadlines with remaining CPU layers/output. Use measured 1002MiB headroom to offload all tensors; q4_0 KV, corpus, sampler and GPU deadlines unchanged. Stop above4096MiB. No quant sweep.',
          'expected': 'Remove remaining CPU inference bottleneck without changing quality criteria or increasing deadlines. Adjudicate all12; no automatic acceptance or promotion.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen8-iq3-gpu'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen8-iq3-gpu.turns.jsonl')]
