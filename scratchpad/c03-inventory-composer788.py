...
validation = read(BASE / 'INVENTORY_BUDGET787/VALIDATION.json')
...
prereg = {'utc': utc(), 'head': subprocess.check_output(['git', 'rev-parse', 'HEAD'], text=True).strip(),
    'candidate': 'INVENTORY_BUDGET787, validated but not adopted yet', 'cases': len(cases),
    ...
    'template_source': 'Identical50 cases, IDs, observed values, questions and criteria from785. Candidate787 changes dense inventory cap and quantity interpretation only; original model/sampler/prompt remain fixed.',
...
try:
    ...
    for case in cases:
        ...
        try:
            ...
            client.end_request()
        row = {'id': case['id'], 'request_budget_seconds': request_budget, 'answer': answer, 'error': error, 'seconds': time.monotonic() - case_started}
        replies.append(row)
