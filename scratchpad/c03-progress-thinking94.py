...
out = base / 'astra-progress-thinking94'
...
prereg = {'method': 'Single first93 case to diagnose censoring, not a six-case acceptance or progress candidate. '
...
client = ThinkingRuntime(); gpu = ProcessTreeGpuSampler(os.getpid()); ram = RamSampler(os.getpid())
gpu.start(); ram.start(); started = time.monotonic()
try:
    client.start_warmup(); assert client.wait_warmup(90)
    for case in cases:
        for variant in ('thinking',):
            ...
            try:
                request = urllib.request.Request(client._endpoint + '/v1/chat/completions',
