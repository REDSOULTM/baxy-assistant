...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen2507-step-failure-reason'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'src/Baxy.App/MainWindowViewModel.cs', 'src/Baxy.App/MissionNarration.cs', 'src/Baxy.App/MindPlanSession.cs',
         'artifacts/comprobaciones/C03/astra-qwen2507-step-failure-reason.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen2507-step-failure-reason/profile/sitecustomize.py',
         'scratchpad/c03-qwen2507-step-failure-reason.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Reproduce the app.close step failure with original runtime prompts and preserved response.Message plus structured mission reason. No seeded calculator window and no confirmation acceptance: this is a failure-reporting diagnostic.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen2507-step-failure-reason'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen2507-step-failure-reason.turns.jsonl')]
