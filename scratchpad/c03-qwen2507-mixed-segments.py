...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen2507-mixed-segments'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen2507-mixed-segments.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen2507-mixed-segments/profile/sitecustomize.py',
         'scratchpad/c03-qwen2507-mixed-segments.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Replace unconstrained mixed prose with two model-authored complementary Spanish/English parts in one structured decode, joined by a space. Diagnostic override only. Keep all existing quality/fact/authority guards, model/profile/token limits. Different representation, no extra classifier/decode. Also contrast repaired stale boot greeting.',
          'expected': '12 useful faithful replies; mixed contains complementary ES/EN prose without repetition, no synthetic clock crop, no stale welcome after first result. Inspect raw segments and public text.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen2507-mixed-segments'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen2507-mixed-segments.turns.jsonl')]
