"""Correlate frozen824 outputs; no verdict inferred from terminal status."""
...
public = root / 'artifacts/comprobaciones/C03/PROCESS_BATCH824'
truncated = []
...
for path_key, sha_key, filename in [
    ...
    if filename != 'OWNERS_DOTNET_EXIT.json':
        ...
        assert read(validation_path)['source_pins_sha256'] == prereg['source823_pins_sha256']
scope_pins_path = validation_root / 'SOURCE_PINS.json'
assert hashlib.sha256(scope_pins_path.read_bytes()).hexdigest() == prereg['source823_pins_sha256']
assert read(scope_pins_path) == prereg['source823_pins']
assert len(prereg['source823_pins']) == 33
assert read(validation_root / 'VALIDATION_EXIT.json')['source_pins_unchanged'] is True
assert read(validation_root / 'VALIDATION_EXIT.json')['source_pins_sha256'] == prereg['source823_pins_sha256']
...
if not args.partial:
    ...
    assert all(outcome[k] for k in ['manifest_unchanged', 'sources_unchanged',
                                  'source823_unchanged', 'runner_unchanged', 'app_dll_unchanged'])
...
if not args.partial:
    ...
    markdown = ['# Respuestas privadas824 — pendientes de adjudicación',
