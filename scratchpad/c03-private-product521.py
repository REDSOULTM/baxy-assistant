...
out = root/'artifacts/comprobaciones/C03/astra-private-product521'
private = Path(os.environ['LOCALAPPDATA'])/'BAXY/C03-private-product521-private'
...
assert not (private.parent/'C03-private-profile521').exists()
prereg = {
    'utc':datetime.now(timezone.utc).isoformat(),
    'production_verification': '521 real product: source520 shape hint removal, C#516 qualified memory configuration, registered Qwen/b9980. Same11cases as519; observation only.',
...
for name in list(env):
    if name.startswith(('BAXY_MIND_','BAXY_VOICE_','BAXY_C03_','BAXY_FIELD_')) or name in {'PYTHONPATH','BAXY_DATA_DIR','BAXY_ASSET_DESCRIPTOR','BAXY_APP_TRACE'}:
        env.pop(name,None)
env.update(BAXY_MIND_LLM_GGUF=str(model), BAXY_MIND_LLAMA_SERVER=config['llama_server'], BAXY_MIND_PYTHONPATH=str(root/'scratchpad/c03-owner521-hook')+os.pathsep+str(root/'src'), BAXY_VOICE_WAKE_ON_START='0',
...
command=['py','main.py','--conductor','--profile',str(private.parent/'C03-private-profile521'),
