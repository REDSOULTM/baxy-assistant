...
OUT=ROOT/'artifacts/comprobaciones/C03/astra-constraint-sampling';OUT.mkdir(exist_ok=False)
...
prereg={'cases':cases,'stages':['constraint_purpose_seed0','constraint_purpose_seed17','constraint_purpose_seed42'],'prompt':prompt,
    ...
    'sampling':{'temperature':0.7,'top_p':0.8,'top_k':20,'min_p':0.0,'seeds':[0,17,42]},'sourceSha256':hashlib.sha256((ROOT/'src/baxy_mind/llm.py').read_bytes()).hexdigest(),'registrationSha256':hashlib.sha256(reg.read_bytes()).hexdigest()}
            ...
            try:
                if stage=='existing_composer':
                    ...
                    payload={'messages':[{'role':'system','content':prompt},{'role':'user','content':json.dumps({'response_language':case['language'],'user_constraint':case['text']},ensure_ascii=False)}],
                        'temperature':0.7,'top_p':0.8,'top_k':20,'min_p':0.0,'seed':int(stage.rsplit('seed',1)[1]),'max_tokens':128,'chat_template_kwargs':{'enable_thinking':False}}
