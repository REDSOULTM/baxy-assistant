...
out = root/'artifacts/comprobaciones/C03/astra-private-product422'
...
prereg = {
    ...
    'attribution_only': 'Same421 with per-process RSS/private commit and system available memory every guard tick. This repeat diagnoses allocation ownership, not a new quality comparison or acceptance. No thresholds/source/loading change.',
...
for name in list(env):
    if name.startswith(('BAXY_MIND_','BAXY_VOICE_','BAXY_C03_','BAXY_FIELD_')) or name in {'PYTHONPATH','BAXY_DATA_DIR','BAXY_ASSET_DESCRIPTOR','BAXY_APP_TRACE'}:
        env.pop(name,None)
env.update(BAXY_MIND_LLM_GGUF=str(model), BAXY_MIND_PYTHONPATH=str(root/'scratchpad/c03-owner422-hook')+os.pathsep+str(root/'src'), BAXY_VOICE_WAKE_ON_START='0',
...
command=['py','main.py','--conductor','--profile',str(private.parent/'C03-private-profile422'),
