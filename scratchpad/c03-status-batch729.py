...
target_hook = root / "scratchpad/c03-status729-hook"
target_hook.mkdir(exist_ok=False)
hook_source = hook.read_text(encoding="utf-8")
assert hook_source.count("BAXY/C03-status-batch706-private") == 1
corrected_hook = hook_source.replace("BAXY/C03-status-batch706-private", "BAXY/C03-status-batch729-private")
(target_hook / "sitecustomize.py").write_text(corrected_hook, encoding="utf-8", newline="\n")
assert (target_hook / "sitecustomize.py").read_text(encoding="utf-8").replace("BAXY/C03-status-batch729-private", "BAXY/C03-status-batch706-private") == hook_source
verify_hook = runpy.run_path(str(root / "scratchpad/c03-observer-preflight729.py"))["verify_hook"]
sequence = {
    ...
    "differences_from_prepared706": "Correct the observer output directory after invalid728; same73 panel, criteria, source712, model/backend/presets and resource limits. Hook normalized text unchanged except destination; preflight before launch.",
    "scope": "Read-only hidden product diagnosis; no UI/voice/survey completion credit",
    "replaces_invalid_diagnostic": 728,
    "observer_sha256": sha(target_hook / "sitecustomize.py"),
}
(base / "STATUS_BATCH729_SEQUENCE.json").write_text(json.dumps(sequence, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
source = (root / "scratchpad/c03-status-batch694.py").read_text(encoding="utf-8").replace("694", "729")
...
source = source.replace(gate, "# Diagnostic sequencing explicitly declared in STATUS_BATCH729_SEQUENCE.json; adoption remains blocked.\n")
private_marker = "private.mkdir(exist_ok=False)"
assert source.count(private_marker) == 1
source = source.replace(private_marker, private_marker + "\nobserver_preflight = verify_hook(target_hook / 'sitecustomize.py', private)\n(base / 'STATUS_BATCH729_OBSERVER_PREFLIGHT.json').write_text(json.dumps(observer_preflight, indent=2) + '\\n', encoding='utf-8')")
...
marker = "(base/'STATUS_BATCH729_PLAN.json').write_text"
assert source.count(marker) == 1
source = source.replace(marker,
    "plan['acceptance_gate'] = {'full_passed': False, 'adopted': False, 'sequence_record': 'STATUS_BATCH729_SEQUENCE.json'}\n" + marker)
