...
private = base / "C03-process-batch819-private"
out = root / "artifacts/comprobaciones/C03/PROCESS_BATCH819"
...
seal_flags = ("manifest_unchanged", "sources_unchanged", "source818_unchanged",
...
assert (private / "root-adjudication.json").is_file(), "Root must save 50 judgments after panel819; no automatic adjudication"
...
# Validate the scripts, source818 and DLL against this run's own preregistration.
runner = root / "scratchpad/c03-process-batch819.py"
reviewer = root / "scratchpad/c03-review-process819.py"
assert sha(runner) == prereg["runner_sha256"]
assert sha(reviewer) == prereg["reviewer_sha256"]
validation_root = root / "artifacts/comprobaciones/C03/PROCESS_REPAIRS818/FULL_RETRY1"
pins_path = validation_root / "SOURCE_PINS.json"
assert sha(pins_path) == prereg["source818_pins_sha256"]
assert read(pins_path) == prereg["source818_pins"]
assert len(prereg["source818_pins"]) == 33
assert all(sha(root / path) == digest for path, digest in prereg["source818_pins"].items())
...
for path_key, hash_key, filename in (
    ...
    assert receipt_path == validation_root / filename
    assert sha(receipt_path) == prereg[hash_key]
    receipt = read(receipt_path)
    assert type(receipt["exit_code"]) is int and receipt["exit_code"] == 0
    if filename in ("VALIDATION_EXIT.json", "FULL_EXIT.json"):
        assert receipt["source_pins_unchanged"] is True
        assert receipt["source_pins_sha256"] == prereg["source818_pins_sha256"]
assert Path(prereg["full_baseline_source_pins_path"]) == pins_path
assert prereg["full_baseline_source_pins_sha256"] == prereg["source818_pins_sha256"]
...
old815 = {row["case_id"]: row["passed"] for row in read(
    root / "artifacts/comprobaciones/C03/PROCESS_BATCH815/ROOT_ADJUDICATION.json")["cases"]}
assert set(old801) == set(old815) == set(judgments)
assert len(old801) == len(old815) == 50 and sum(old801.values()) == 35 and sum(old815.values()) == 39
...
comparison = {str(batch): {"previous_valid": sum(old.values()),
              ...
              for batch, old in ((801, old801), (815, old815))}
previous_rows = read(base / "C03-process-batch815-private/review.json")
assert len(previous_rows) == 50
assert [{"case_id": row["case_id"], "group": row["group"]} for row in previous_rows] == prereg["cases"]
latency = {"metric": "Difference between first and last recorded turn-scoped shell event; all 50 completed terminals regardless of terminal kind, explicitly including composition_failed. Not acoustic latency.819 and815 each use their complete continuous50 review.json directly; no interrupted or resumed profiles are combined.",
           "819": {"all": times(review), "by_group": {group: times([row for row in review if row["group"] == group]) for group in groups}},
           "815": {"all": times(previous_rows), "by_group": {group: times([row for row in previous_rows if row["group"] == group]) for group in groups}}}
...
result = {"method": "Root read every terminal, including composition_failed, against its newly observed facts and the sealed criterion; this script assembles only those saved judgments.",
          "candidate": 818, "adopted": False, "continuous_50": True, "valid": valid, "failed": 50 - valid,
          ...
          "cases": cases, "evidence_sha256": {str(path): sha(path) for path in (
              ...
              validation_root / "OWNERS_PYTHON_EXIT.json", validation_root / "OWNERS_DOTNET_EXIT.json",
              validation_root / "VALIDATION_EXIT.json", validation_root / "FULL_EXIT.json",
              ...
              root / "artifacts/comprobaciones/C03/PROCESS_BATCH815/ROOT_ADJUDICATION.json",
              base / "C03-process-batch815-private/review.json")},
          "survey_counts": {"covered": 28, "open": 714, "not_applicable": 0}}
write(out / "ROOT_ADJUDICATION.json", result)
write(out / "VERIFICATION_STATUS.json", {"source_candidate": 818, "source_adopted": False,
...
markdown = ["# Respuestas819 — adjudicación raíz", f"{valid}/50 válidas;{50-valid}/50 fallidas. La decisión de adopción se registra aparte."]
for row in review:
