...
out = root / 'artifacts/comprobaciones/C03/astra-recall-dialogue371'
private = base / 'C03-recall-dialogue371-private'
...
prereg = {'utc': datetime.now(timezone.utc).isoformat(), 'cases': cases,
    'method': 'Five development cases: two exact366 error requests plus three explicitly synthetic provenance/persistence controls. Same Qwen3.5 and exact first compositor payload, baseline versus inserting the existing bounded user/assistant dialogue before the current user request. No new prose, cause, language, operation result, sampler, source, effect or runtime promotion. First native completions only; guards and App not tested by this replay.',
...
try:
    ...
    for case in cases:
        for variant in ['baseline', 'native-dialogue']:
            payload = copy.deepcopy(case['reference'])
            if variant == 'native-dialogue':
                payload['messages'][-1:-1] = copy.deepcopy(case['history'])
