...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen8-purpose'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen8-purpose.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen8-purpose/profile/sitecustomize.py',
         'scratchpad/c03-qwen8-purpose.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Preserving message kind and in-progress state prevents empty composition payloads from becoming instruction acknowledgements, greetings during progress, and preparation timeouts. Same model, sampling, corpus and budgets as astra-qwen8-product.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen8-purpose'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen8-purpose.turns.jsonl')]
