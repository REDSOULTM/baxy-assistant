...
snapshot = json.loads((BASE/'astra-source257-snapshot/FILES.json').read_text(encoding='utf-8'))
...
save('PREREG.json', {'utc':datetime.now(timezone.utc).isoformat(), 'model':str(model),
    'method':'Actual py main.py, source257 and installed lock60; Qwen3.5 override remains unpromoted. All user input through desktop Computer Use. Read-only voice/Piper/AEC observers; no injected inputs/responses, no replaced algorithms. Owned process-tree GPU/RAM sampling and volume restoration.',
    ...
    'plannedDevelopmentInputs':['¿Qué hora es?','What time is it?','Explícame brevemente qué es la RAM.'],
    ...
    'sourceFiles':snapshot, 'hookSha256':sha(ROOT/'scratchpad/c03-ui260-hook/sitecustomize.py'),
...
env.update(BAXY_MIND_LLM_GGUF=str(model), BAXY_VOICE_WAKE_ON_START='0',
    ...
    BAXY_MIND_PYTHONPATH=str(ROOT/'scratchpad/c03-ui260-hook')+os.pathsep+str(ROOT/'src'),
    BAXY_DATA_DIR=str(Path(env['LOCALAPPDATA'])/'BAXY/C03-ui260-profile'))
