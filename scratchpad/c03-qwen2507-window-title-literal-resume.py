...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen2507-window-title-literal-resume'
...
files = ['src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen2507-window-title-literal-resume.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen2507-window-title-literal-resume/profile/sitecustomize.py',
         'scratchpad/c03-qwen2507-window-title-literal-resume.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Inherit the historical close-target literal path in the existing typed argument extractor. Copy the explicit window title without asking the model to infer a process. Preserve spelling, quotes and accents; require the title-capable catalog schema and verified provider identity before exact confirmation. Same six development turns. Previous capture interrupted by verified Windows reboot at 2026-09-06 11:32:23 local before a complete terminal; preserve partial evidence. No model/sampler/source change.',
          'expected': 'Six focused development turns: natural title differs from executable name C03TitleFixture. Resolve the actual titled window, confirm exact close, cancel without effect, confirm second close verified and preserve subsequent clock/tasks. Only own empty fixture. No acceptance or model promotion.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen2507-window-title-literal-resume'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen2507-window-title-literal-resume.turns.jsonl')]
