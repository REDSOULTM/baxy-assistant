...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen8-confirmation-contract'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen8-confirmation-contract.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen8-confirmation-contract/profile/sitecustomize.py',
         'scratchpad/c03-qwen8-confirmation-contract.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Second causal repair: replace confirmation instruction that requested only choices with action/target authorization; mark readOnly using typed catalogue in ordered mission results; exempt exact observed names from generic glued-window token veto. Other prompts and sampler unchanged.',
          'expected': 'Same21 inputs and own empty C03WindowFixture process as step-order. Confirmations name pending action/target; no invented maximize action; exact observed process name accepted; successful cancellation and close verified in journal. No acceptance claim for mixed-language failures.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen8-confirmation-contract'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen8-confirmation-contract.turns.jsonl')]
