...
out = root / 'artifacts/comprobaciones/C03/astra-speex129'
out.mkdir(exist_ok=False)
private = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-speex129-private'
private.mkdir(exist_ok=False)
source = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-voice127-private'
...
capture = json.loads((root / 'artifacts/comprobaciones/C03/astra-voice127/AUDIO_RESULT.json').read_text(encoding='utf-8'))
...
events = [json.loads(line) for line in (root / 'artifacts/comprobaciones/C03/astra-voice127/EVENTS.jsonl').read_text(encoding='utf-8').splitlines()]
started = next(e for e in events if e.get('speaking'))
...
prereg = {'source': 'Existing physical127 microphone/loopback; no playback',
          ...
          'methods': ['raw', 'aec', 'aec_residual'],
          'preprocessor': 'Echo state linked; denoise and AGC disabled, echo suppression defaults unchanged',
...
def process(audio, playback, residual):
    ...
    if pre:
        zero = c.c_int(0)
        assert lib.speex_preprocess_ctl(pre, 0, c.byref(zero)) == 0
...
for name, audio, playback in [('physical_echo', mic, reference), ('near_only', near, np.zeros_like(near)), ('physical_echo_plus_synthetic_near', mic + near, reference)]:
    for method in ['raw', 'aec', 'aec_residual']:
