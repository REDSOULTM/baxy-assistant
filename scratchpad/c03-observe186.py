"""Adjudicate preserved physical185 audio locally; no expected-text hints."""
...
for i, event in enumerate(events):
    if event['speaking']:
        ...
        intervals.append((dt.datetime.fromisoformat(event['utc']).timestamp(), dt.datetime.fromisoformat(end['utc']).timestamp()))
...
prereg = {'method': 'Preserved physical185; registered Parakeet CPU6 beam8, no hints. All four intervals, microphone and loopback, raw and peak0.8 normalization. Mono mean then scipy resample_poly48000->16000 for loopback.',
          'limitation': '185 speaking state has paired UTC recorded in the process. Map directly to capture streamReadyUtc with one second margin each side; neither API time is exact first audible sample. Recognition and normalization are diagnostic, not proof of human input or complete fidelity.',
