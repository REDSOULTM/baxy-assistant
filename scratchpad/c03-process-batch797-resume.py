...
OUT = ROOT / 'artifacts/comprobaciones/C03/PROCESS_BATCH797_RESUME'
PRIVATE = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-process-batch797-resume-private'
PROFILE = PRIVATE.parent / 'C03-process-profile797-resume'
...
panel = original_panel[42:]
assert len(panel) == 8 and len({c['case_id'] for c in panel}) == 8
...
prereg = {
    'utc': utc(), 'method': 'Complete remaining8 independent frozen process requests after797 RAM guard ended at42 terminals. Same17source pins/model/profile settings/DLL; fresh private profile means this is a second segment, not an uninterrupted50-case run. No aggregate latency comparison with795.',
    ...
    'effect_scope': plan['effect_scope'], 'profile': 'Fresh isolated continuation profile after resource guard; conversation reset disclosed, wake disabled.', 'continuation_of':797, 'original_case_offset':42, 'case_count':8,
...
with (PRIVATE / 'launch.log').open('w', encoding='utf-8') as log:
    ...
    print(json.dumps({'started': True, 'launcher': process.pid, 'registered_turns': 8}), flush=True)
