...
out = ROOT / 'artifacts/comprobaciones/C03/astra-closed-prohibition7'
...
files = ['tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-closed-prohibition7.turns.jsonl',
         'scratchpad/c03-closed-prohibition7.py']
files.append('src/Baxy.App/UserMessagePolicy.cs')
files.append('src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.dll')
files.append('artifacts/comprobaciones/C03/astra-closed-prohibition7/CASES.json')
...
prereg={'kind':'qwen-base-integrated-diagnostic-not-acceptance','utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
 'method':'Five literal corpus inputs in their former local order, then the same standalone English and Spanish prohibition controls. Only intended mutation is setting audio100, followed by verified state reads. No app launching or external messaging. Current Python source preserves inherited closed prohibitions before catalog reclassification and presents only the literal constraint to the acknowledgement writer. Identical first seven inputs/order from closed-prohibition11; sole change is no previous device reports in constraint generation. Registered runtime; no overrides. Product conductor, not UI acceptance.',
...
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-closed-prohibition7'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
        ...
        while process.poll() is None:
            if not released:
                ...
                if time.monotonic()-started > 50:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-closed-prohibition7.turns.jsonl').read_bytes()
