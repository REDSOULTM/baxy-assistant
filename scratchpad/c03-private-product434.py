...
out = root/'artifacts/comprobaciones/C03/astra-private-product434'
private = Path(os.environ['LOCALAPPDATA'])/'BAXY/C03-private-product434-private'
...
prereg = {
    ...
    'production_verification': 'Source433 whole self-introduction uses existing social-conversation path; source431 generic app clarification and425 cache-ram0/GPU no-mmap retained. Hook observes unchanged HTTP and only adds logging. No source changes while running.',
...
for name in list(env):
    if name.startswith(('BAXY_MIND_','BAXY_VOICE_','BAXY_C03_','BAXY_FIELD_')) or name in {'PYTHONPATH','BAXY_DATA_DIR','BAXY_ASSET_DESCRIPTOR','BAXY_APP_TRACE'}:
        env.pop(name,None)
env.update(BAXY_MIND_LLM_GGUF=str(model), BAXY_MIND_PYTHONPATH=str(root/'scratchpad/c03-owner434-hook')+os.pathsep+str(root/'src'), BAXY_VOICE_WAKE_ON_START='0',
...
command=['py','main.py','--conductor','--profile',str(private.parent/'C03-private-profile434'),
