...
OUT=ROOT/'artifacts/comprobaciones/C03/astra-constraint-scope-data';OUT.mkdir(exist_ok=False)
        ...
        for case in cases:
            ...
            try:
                if stage=='existing_composer':
                    ...
                    payload={'messages':[{'role':'system','content':prompt},{'role':'user','content':json.dumps({'response_language':case['language'],'user_constraint':case['text'],'scope':('ongoing constraint' if case['text'].startswith('nunca ') else 'this instruction only'),'permanent_rule':case['text'].startswith('nunca ')},ensure_ascii=False)}],
