...
out = ROOT / 'artifacts/comprobaciones/C03/astra-presentation-product-qwen'
...
files = ['tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-presentation-product-qwen.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-presentation-product-qwen/profile/sitecustomize.py',
         'scratchpad/c03-presentation-product-qwen.py']
...
prereg={'kind':'qwen-base-integrated-diagnostic-not-acceptance','utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
 'method':'Inherited conductor and 21 requests, Qwen3 4B Instruct base without LoRA or sampling overrides; GGUF override and q8 KV are explicit diagnostic settings. Read-only audit captures actual payloads and replies; kernel/providers/composition and app publication run normally. Own empty fixture used for exact close/confirmation; current-state facts are observed, not replayed. Different live history from synthetic ablation: do not attribute every difference to one layer. No graphical UI observation from conductor.',
...
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-presentation-product-qwen'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
        ...
        while process.poll() is None:
            if not released:
                ...
                if {'planner_catalog','skill_registry'} <= ready_components or time.monotonic()-started > 150:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-presentation-product-qwen.turns.jsonl').read_bytes()
