...
out = ROOT / 'artifacts/comprobaciones/C03/astra-wire-context47'
...
files = ['tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-wire-context47.turns.jsonl',
         'scratchpad/c03-wire-context47.py']
files.append('src/Baxy.App/UserMessagePolicy.cs')
files.append('src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.dll')
files.append('artifacts/comprobaciones/C03/astra-wire-context47/CASES.json')
...
files += ['src/baxy_mind/voice_output.py', 'scratchpad/c03-wire-hook47/sitecustomize.py', 'artifacts/comprobaciones/C03/TRAMO46_PINS.json']
...
prereg['method'] = 'Capture exact product payloads and replies through an opt-in read-only sitecustomize wrapper of LlmRuntime._post. Same registered model/backend/template/sampler and product source46, PythonPath adds only diagnostic hook. First10 consumed technical turns reproduce the history leading to gravity; not a repeat33 to seek a pass. Then3 separate read-only diagnostics for compound progress, missing-file error and a normal clock recovery. No fresh acceptance or UI/audio claim. Hook adds I/O overhead, so not a performance benchmark. No captured packet is modified. Existing16-call context comparison did not reproduce the Moon phrase; C# last12messages includes current input, leaving an older orphan assistant that the five-pair reconstruction omitted.'
...
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-wire-context47'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
prereg['preparation']='Hold stdin for 50 seconds to allow normal warmup, payload logging hook enabled; model/backend/sampler unchanged. Warm development and recovery, not cold-start latency or fresh acceptance.'
(out/'PREREG.json').write_text(json.dumps(prereg,indent=2),encoding='utf-8')
released=False
        ...
        while process.poll() is None:
            if not released:
                ...
                if time.monotonic()-started > 50:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-wire-context47.turns.jsonl').read_bytes()
