...
out = ROOT / 'artifacts/comprobaciones/C03/astra-files77-template'
...
files = ['tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-files77-template.turns.jsonl',
         'scratchpad/c03-files77-template.py']
files.append('src/Baxy.App/UserMessagePolicy.cs')
files.append('src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.dll')
files.append('artifacts/comprobaciones/C03/astra-files77-template/CASES.json')
...
prereg['method'] = 'Source74 plus runtime77 serializes only leading system messages into one prefix, retaining text/order, dialogue and every other payload field. No model/prompt/history/selector changes. Same ten technical inputs and Qwen3.5 isolated override as74/75. Native76 fixes9/9 server rejections; registered2507 main replies preserved. Hook77 observes post_chat_completion after serialization, including HTTP errors without consuming bodies. Record useful final/progress, failures and resources. No UI/audio/human reserve or model promotion.'
...
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-files77-template'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
        ...
        while process.poll() is None:
            if not released:
                ...
                if time.monotonic()-started > 50:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-files77-template.turns.jsonl').read_bytes()
