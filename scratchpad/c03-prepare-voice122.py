...
out = root / 'artifacts/comprobaciones/C03/astra-voice122'
out.mkdir(exist_ok=False)
capture = (root / 'scratchpad/c03-capture-audio121.py').read_text(encoding='utf-8')
capture = capture.replace('astra-audio121', 'astra-voice122').replace('C03-audio121-private', 'C03-voice122-private')
with (root / 'scratchpad/c03-capture-voice122.py').open('x', encoding='utf-8', newline='\n') as f:
    f.write(capture)
def sha(path):
...
prereg = {'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'method': 'Source119 unchanged. Actual VoiceEngine/Piper and physical speakers, default '
                    ...
                    'wake, off-after phases, one VoiceEngine with normal start/stop. No App/LLM/Core '
          ...
          'phases': ['off-before', 'wake', 'off-after'],
