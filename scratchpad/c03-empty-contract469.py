...
out = base / 'astra-empty-contract469'
private = local / 'C03-empty-contract469-private'
...
write(out / 'PREREG.json', {
    ...
    'method': 'Paired against468 documentedQwen2507 .7/.8/k20/min0/no-thinking/neutralpenalties, seeds0/17, source466unchanged. Only confirmation payload transformation: omit empty literal headings Acciones:(ninguna) and Hechos:(ninguno), whose none assertion may obscure available situation facts. No replacement prompt, fixed visible string or new guard. Six confirmations plus one unchanged memory recall control perseed,14compositions. Inheritance367/368 improved pendingAction transport but repeating cause failedES;369 preserves boundprepared invocation. Do not reopen those edits. Current465/467/468 have actualfacts and profile audits. Source4661404owners/Fastgreen. If no useful improvement, abandon this formatting hypothesis.',
    'criteria': 'Same465/468 sixconfirmation rubric and allseeds. Explain exactpendingaction, target, irreversible deletion and export destination/syncrisk; nofalseeffect, namebothchoices. Memorycontrol unchanged. Count all attempts/failures. Diagnostic only, no source adoption/product/freshacceptance credit.',
    ...
    def _post(self, payload, timeout=None, **kwargs):
        ...
        if self.variant == 'confirmation':
            for message in payload['messages']:
                message['content'] = '\n'.join(line for line in message['content'].splitlines() if line not in {'Acciones: (ninguna)', 'Hechos: (ninguno)'})
