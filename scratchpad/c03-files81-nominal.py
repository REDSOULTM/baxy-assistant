...
out = ROOT / 'artifacts/comprobaciones/C03/astra-files81-nominal'
...
files = ['tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-files81-nominal.turns.jsonl',
         'scratchpad/c03-files81-nominal.py']
files.append('src/Baxy.App/UserMessagePolicy.cs')
files.append('src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.dll')
files.append('artifacts/comprobaciones/C03/astra-files81-nominal/CASES.json')
...
files.append('scratchpad/c03-wire-hook77/sitecustomize.py')
...
prereg['method'] = 'Source78 plus81: extend the existing whole-clause coordinated status grammar to bare domain nouns and CPU usage/uso de CPU, and split all comma/conjunction items only after that complete grammar matches. Preserve the governing observation verb, requested order and catalog completeness. No new selector/prompt/model or history layer. Same ten technical controls and isolated Qwen3.5 override as78 (9/10 useful, t9 partial audio only). 79 used an inapplicable file shortlist;80 isolated the three actual observation leaves and reproduced the early strict return. Unit controls cover missing leaves and an unknown extra segment. No UI/audio/human reserve or promotion.'
...
env.update(BAXY_MIND_PYTHONPATH=str(ROOT/'scratchpad/c03-wire-hook77')+os.pathsep+str(ROOT/'src'), C03_WIRE_TRACE_DIR=str(out))
app=ROOT/'src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.exe'
commit=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip()
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-files81-nominal'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
        ...
        while process.poll() is None:
            if not released:
                ...
                if time.monotonic()-started > 50:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-files81-nominal.turns.jsonl').read_bytes()
