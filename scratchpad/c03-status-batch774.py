...
OUT = ROOT / 'artifacts/comprobaciones/C03/STATUS_BATCH774'
PRIVATE = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-status-batch774-private'
PROFILE = PRIVATE.parent / 'C03-status-profile774'
...
pins773 = read(ROOT / 'artifacts/comprobaciones/C03/MACHINE_SCOPE773/SOURCE_PINS.json')
pins764 = read(ROOT / 'artifacts/comprobaciones/C03/DENSE_INVENTORY764/SOURCE_PINS.json')
assert all(sha(ROOT / path) == digest for path, digest in {**pins773, **pins764}.items())
...
prereg = {
    'utc': utc(), 'method': 'Complete seven-case battery category plus ten declared development variants after input-scope773; original battery criterion unchanged. Build prepared before sealing DLL; no model comparison.',
    ...
    'validation': 'Published773 owners/current pin integrity and Fast, dense764 owners/Fast retained. Full7 historical, no final acceptance.',
    'source773_pins': pins773, 'source764_pins': pins764, 'sources': sources, 'runner_sha256': sha(__file__),
...
outcome = {'utc': utc(), 'exit_code': code, 'manifest_unchanged': sha(manifest) == prereg['manifest_sha256'],
           ...
           'source773_unchanged': all(sha(ROOT / path) == digest for path, digest in pins773.items()),
