...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen8-knowledge-probe'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen8-knowledge-probe.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen8-knowledge-probe/profile/sitecustomize.py',
         'scratchpad/c03-qwen8-knowledge-probe.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Keep the resource-viable IQ3 full-GPU profile. Knowledge recovery now inherits the strict compatibility verifier and read-only catalog risk instead of permissive effect identity. Preserve real PC reads and compound effects. No prompt or model changes.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen8-knowledge-probe'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen8-knowledge-probe.turns.jsonl')]
