...
out = ROOT / 'artifacts/comprobaciones/C03/astra-files83-empty'
...
files = ['tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-files83-empty.turns.jsonl',
         'scratchpad/c03-files83-empty.py']
files.append('src/Baxy.App/UserMessagePolicy.cs')
files.append('src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.dll')
files.append('artifacts/comprobaciones/C03/astra-files83-empty/CASES.json')
...
prereg['method'] = 'Source81 unchanged; isolated Qwen3.5 override remains unpromoted. Four technical readonly controls: missing named file inside the sandbox, why question, a real readable file and clock recovery. A filename search returning0 differs from the unsupported absolute path already solved. No files are deleted to manufacture absence; this isolated profile contains only the listed hashed fixtures. No fabricated provider answers, model/prompt/history changes, UI/audio or human reserve claim. Judge facts and complete turns.'
...
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-files83-empty'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
        ...
        while process.poll() is None:
            if not released:
                ...
                if time.monotonic()-started > 50:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-files83-empty.turns.jsonl').read_bytes()
