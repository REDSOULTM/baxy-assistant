...
out = root/'artifacts/comprobaciones/C03/astra-private-product437'
private = Path(os.environ['LOCALAPPDATA'])/'BAXY/C03-private-product437-private'
...
prereg = {
    ...
    'production_verification': 'Source436 scopes generation history for explicit closed social turns, preserving the real dialogue. Source433 social names,431 missing app and425 cache-ram0/GPU no-mmap retained. Observation hook only logs; no source edits during run.',
    'method': 'Same eight synthetic development turns as434 in a new private profile; one source change436. T5 must acknowledge Alvaro without false memory save/Windows action and T6 must ask which app. T8 must still remember the current human declaration despite scoped generation history. T1-T4 remain useful, T7 wrong-speaker defect stays separately judged. Same model/context/precision/sampler/limits, no promotion/UI/physical voice/fresh acceptance.',
...
for name in list(env):
    if name.startswith(('BAXY_MIND_','BAXY_VOICE_','BAXY_C03_','BAXY_FIELD_')) or name in {'PYTHONPATH','BAXY_DATA_DIR','BAXY_ASSET_DESCRIPTOR','BAXY_APP_TRACE'}:
        env.pop(name,None)
env.update(BAXY_MIND_LLM_GGUF=str(model), BAXY_MIND_PYTHONPATH=str(root/'scratchpad/c03-owner437-hook')+os.pathsep+str(root/'src'), BAXY_VOICE_WAKE_ON_START='0',
...
command=['py','main.py','--conductor','--profile',str(private.parent/'C03-private-profile437'),
