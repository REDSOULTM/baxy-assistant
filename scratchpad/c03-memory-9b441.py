...
out=base/'astra-memory-9b441';private=local/'C03-memory-9b441-private'
...
model=Path('D:/BAXYRuntime/experiments/models/qwen35-9b-03b74727/Qwen3.5-9B-Q4_K_M.gguf')
def sha(p):
...
prereg={'utc':datetime.now(timezone.utc).isoformat(),
 ...
 'limits':{'gpu_stop_mib':3800,'minimum_free_ram_mib':768,'request_seconds':120},'private':str(private)}
assert prereg['model_sha256']=='03b74727a860a56338e042c4420bb3f04b2fec5734175f4cb9fa853daf52b7e8'
...
try:
    ...
    for case in cases:
        for variant in ['9b-ngl14']:
            payload=copy.deepcopy(case['payload'])
            assert payload==case['payload']
            before=time.monotonic();client.begin_request(120)
            try:
