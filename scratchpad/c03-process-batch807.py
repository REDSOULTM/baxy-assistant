"""Frozen 50-case process-read category through the registered product with sealed process scope806.
...
PRIVATE = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-process-batch807-private'
PROFILE = PRIVATE.parent / 'C03-process-profile807'
...
plan = read(plan_path)
# Full804 is the baseline for cumulative Python802 plus C#804, not Full806.
# Python-only scope806 requires its own owners and Fast before product measurement.
validation_root = ROOT / 'artifacts/comprobaciones/C03/PROCESS_SCOPE806'
...
pins806 = read(validation_root / 'SOURCE_PINS.json')
assert len(pins806) == 29, 'scope806 source pin set'
assert all(sha(ROOT / path) == digest for path, digest in pins806.items())
...
prereg = {
    'utc': utc(), 'method': 'Registered50 process-read category with sealed candidate806; nine historical requests and41 variants. Build prepared before sealing DLL; model selection is closed.',
    ...
    'validation': 'Full804 retry1 exit0 is the baseline for Python802 plus C#804, verified against SOURCE_PINS804; this is not Full806. PROCESS_SCOPE806 owners and Fast exit0 are required before launch. Original Full804 seal failure preserved. New real product confirmation; final cumulative Full/adoption remains governed by the goal. No native model comparison or automated coverage credit.',
    ...
    'source806_pins_sha256': sha(validation_root / 'SOURCE_PINS.json'),
    ...
    'source806_pins': pins806, 'sources': sources, 'runner_sha256': sha(__file__),
...
outcome = {'utc': utc(), 'exit_code': code, 'manifest_unchanged': sha(manifest) == prereg['manifest_sha256'],
           ...
           'source806_unchanged': all(sha(ROOT / path) == digest for path, digest in pins806.items()),
