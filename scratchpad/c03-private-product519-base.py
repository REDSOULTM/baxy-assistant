...
out = root/'artifacts/comprobaciones/C03/astra-private-product519-base'
private = Path(os.environ['LOCALAPPDATA'])/'BAXY/C03-private-product519-base-private'
out.mkdir(exist_ok=True)
private.mkdir(exist_ok=True)
assert not (out/'PREREG.json').exists() and not (out/'PROCESS.json').exists()
assert not (private/'http-posts.jsonl').exists()
cases = ['My name is Jordan. Remember my name.', 'confirm', 'Abre una aplicación.', 'What name have you saved in private memory?', 'Me llamo Álvaro.', 'Abre una aplicación.', '¿Qué nombre tienes guardado en tu memoria privada?', '¿Cómo me llamo?', 'Desactiva la memoria privada.', 'Dime la hora.', '¿Tienes memoria local?']
...
assert not (private.parent/'C03-private-profile519-base').exists()
prereg = {
    'utc':datetime.now(timezone.utc).isoformat(),
    'production_verification': '519 base: source516 full product, same11 cases; see astra-status-hint519/PREREG.json. Only treatment changes shape hint privately.',
    ...
    'cases':cases,
    'source_sha256':{name:sha(root/name) for name in ['src/baxy_mind/effect_intent.py','src/baxy_mind/__main__.py','src/baxy_mind/llm.py','src/Baxy.App/NaturalMemoryRequestParser.cs']},
...
for name in list(env):
    if name.startswith(('BAXY_MIND_','BAXY_VOICE_','BAXY_C03_','BAXY_FIELD_')) or name in {'PYTHONPATH','BAXY_DATA_DIR','BAXY_ASSET_DESCRIPTOR','BAXY_APP_TRACE'}:
        env.pop(name,None)
env.update(BAXY_MIND_LLM_GGUF=str(model), BAXY_MIND_LLAMA_SERVER=config['llama_server'], BAXY_MIND_PYTHONPATH=str(root/'scratchpad/c03-owner519-base-hook')+os.pathsep+str(root/'src'), BAXY_VOICE_WAKE_ON_START='0',
...
command=['py','main.py','--conductor','--profile',str(private.parent/'C03-private-profile519-base'),
