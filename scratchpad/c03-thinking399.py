...
out = root / 'artifacts/comprobaciones/C03/astra-thinking399'
private = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-thinking399-private'
...
prereg = {'utc': datetime.now(timezone.utc).isoformat(), 'cases': cases,
    'method': 'Two sequential profiles on the same4B quant/backend. Eight exact captured native/compose payloads, no prompt/tool/history edits. Both get max_tokens1024 instead of256 so output allowance is shared and not an accidental thinking truncation. Paired baseline disables reasoning with budget0; variant enables thinking with budget512. Both parse reasoning separately via deepseek and retain original temperature0/seed and remaining sampler. This isolates bounded reasoning, not the official sampling recommendation. No rerun to search for a favorable seed.',
...
for variant in ['baseline', 'thinking512']:
