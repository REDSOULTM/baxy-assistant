"""Eight final brief framing controls over the sealed physical OCR832 payloads.

Root must supply the collected Full and final source-pin byte hashes. No grading.
Prepared without inference; native836 framing plus only system 'Answer briefly.'.
"""
...
OUT = BASE / 'OCR_NATIVE838'
PRIVATE = LOCAL / 'C03-ocr-native838-private'
PLAN838 = LOCAL / 'C03-ocr838-brief-proposal/PLAN.json'
WIRE832 = LOCAL / 'C03-ocr-composer832-private/physical-http.jsonl'
WIRE832_SHA = '6d2823bc16a8ce5d1afdf4bdc34b49f732a541be2a07b41e72a4291d4f4d28ac'
PREREG832 = BASE / 'OCR_COMPOSER832/PREREG.json'
PREREG832_SHA = '8031be57c7c7c031c102fd4fec8cd1fa7e1798736e2e38ae3814344f4f320bdf'
FULL_SHA = 'd7858325857a250d1a5aeafaebe327cd6aa8aad3b4d26fa1ff5f9997f832c9ed'
PINS_SHA = '1e552541c0a18d816026c9139e82263f173ff29184f91d4a84aabce6cfb205ac'
...
def framing_payloads(cases):
    with WIRE832.open(encoding='utf-8-sig') as stream:
        events = [json.loads(line) for line in stream if line.strip()]
    merged = [event for event in events if event['state'] == 'merged_payload']
    sends = [event for event in events if event['state'] == 'send_started']
    require(len(merged) == len(sends) == 8, 'Expected eight first physical832 payloads')
    result = {}
    for case, event, sent in zip(cases, merged, sends):
        logical = case['id'] + ':1'
        require(event['logical_post_id'] == sent['logical_post_id'] == logical, '832 payload order changed')
        payload = event['payload']
        require(json.loads(base64.b64decode(sent['body_base64'])) == payload, 'Merged832 differs from physical body')
        require([m['role'] for m in payload['messages']] == ['system', 'user'], 'Unexpected832 framing')
        content = payload['messages'][1]['content']
        prefix = case['user_message'] + '\nsituation: '
        require(content.startswith(prefix), '832 literal request changed')
        situation_text = content[len(prefix):]
        _, json_end = json.JSONDecoder().raw_decode(situation_text)
        suffix = situation_text[json_end:]
        require(suffix in {
        ...
        }, 'Unexpected832 language suffix')
        native = copy.deepcopy(payload)
        native['messages'] = [{'role': 'system', 'content': 'Answer briefly.'}, {'role': 'user', 'content': content[:len(prefix) + json_end]}]
        result[case['id']] = native
    return result


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--expected-full-sha256', default=FULL_SHA)
    parser.add_argument('--expected-pins-sha256', default=PINS_SHA)
    parser.add_argument('--expected-plan-sha256', required=True)
    args = parser.parse_args()
    for value in (args.expected_full_sha256, args.expected_pins_sha256, args.expected_plan_sha256):
        require(len(value) == 64 and all(c in '0123456789abcdef' for c in value), 'Expected lowercase SHA256')
    require(args.expected_full_sha256 == FULL_SHA and args.expected_pins_sha256 == PINS_SHA, '838 must preserve832 Full/pins')
    require(sha(PLAN838) == args.expected_plan_sha256, '838 plan seal changed')
    plan838 = read(PLAN838)
    require(sha(__file__) == plan838['driver_sha256'], '838 driver seal changed')
    require(sha(WIRE832) == WIRE832_SHA and sha(PREREG832) == PREREG832_SHA, '832 evidence seals changed')
    ...
    native_payloads = framing_payloads(cases)
    for case_id, payload in native_payloads.items():
        require(hashlib.sha256(json.dumps(payload, ensure_ascii=False, sort_keys=True).encode('utf-8')).hexdigest() == plan838['native_payload_sha256'][case_id], '838 payload differs from sealed control')
    ...
    seals.update({str(WIRE832): WIRE832_SHA, str(PREREG832): PREREG832_SHA, str(PLAN838): args.expected_plan_sha256})
    ...
    prereg = {'utc': utc(), 'head': subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip(),
        ...
        'input': 'Native836 payload plus only system Answer briefly.; exact literal request and situation JSON substring unchanged. All generation parameters unchanged.',
        'native_payload_sha256': plan838['native_payload_sha256'], 'control': 'Final brief-framing control; only Answer briefly. added to native836. No model/profile/production change or acceptance credit',
        ...
        'method': 'One direct local _post per independent case; no compose_user_message, prose veto or regeneration. Existing HTTP transport retries/timeouts unchanged. No adjudicator.',
    ...
    try:
        ...
        for case in cases:
            ...
            payload = copy.deepcopy(native_payloads[case['id']])
            append('inputs.jsonl', {'id': case['id'], 'user_message': case['user_message'], 'native_payload': payload})
            answer, error, response = None, None, None
            ...
            try:
                ...
                client.end_request()
            row = {'id': case['id'], 'answer': answer, 'response': response, 'error': error, 'attempts': attempts.get(case['id'], 0), 'seconds': time.monotonic() - case_started, 'request_budget_seconds': 4}
            replies.append(row)
