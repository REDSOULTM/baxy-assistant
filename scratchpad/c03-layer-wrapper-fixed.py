...
OUT=ROOT/f'artifacts/comprobaciones/C03/astra-layer-wrapper-fixed-{model_key}'
...
cases=[c for c in cases if c['id'] in {'t8','t9','t13'}]
for c in cases:c['history']=[]
...
prereg={'kind':'postfix-language-wrapper-three-case-replay-not-acceptance','modelRole':model_key,'model':str(MODEL),
 ...
 'scope':'After envelope-classification fix: three existing questions, history empty; real chat, same seed0 and128 tokens. Not a rerun of all layers. No PC effects or current-state questions without facts. Compositor, kernel/app and UI must be measured separately afterward.',
