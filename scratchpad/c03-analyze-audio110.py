...
out = root / 'artifacts/comprobaciones/C03/astra-audio110'
private = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-audio110-private'
...
report={'sttDirectory':str(stt),'provider':'CPU','method':'Same registered offline transducer; no hotwords/expected-text prompt; raw and peak-normalized reads both reported.',
        'rows':rows}
(private/'transcription110.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
(out/'TRANSCRIPTION_INDEX.json').write_text(json.dumps({'privatePath':str(private/'transcription110.json'),
    'sha256':hashlib.sha256((private/'transcription110.json').read_bytes()).hexdigest()},indent=2),encoding='utf-8')
