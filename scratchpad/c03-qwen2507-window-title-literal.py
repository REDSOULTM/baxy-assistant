...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen2507-window-title-literal'
...
files = ['src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen2507-window-title-literal.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen2507-window-title-literal/profile/sitecustomize.py',
         'scratchpad/c03-qwen2507-window-title-literal.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Inherit the historical close-target literal path in the existing typed argument extractor. Copy the explicit window title without asking the model to infer a process. Preserve spelling, quotes and accents; require the title-capable catalog schema and verified provider identity before exact confirmation. Same six development turns. No model/sampler change.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen2507-window-title-literal'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen2507-window-title-literal.turns.jsonl')]
