...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen8-dialogue-state'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen8-dialogue-state.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen8-dialogue-state/profile/sitecustomize.py',
         'scratchpad/c03-qwen8-dialogue-state.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'The shell now sends actual pendingClarification state. A social question mark cannot suppress deterministic knowledge and independent no-effect agreement. Uncertain effects also retain structured unknown/pending facts instead of prose or completed statuses. Same GGUF and sampler.',
          'expected': 'Explain encryption after a greeting remains knowledge. Twenty-one development turns including personal list read and confirmation/cancel/confirm with the Calculator window already opened by prior app.open at08:44:02 (CalculatorApp PID8328). Do not replay the earlier uncertain opening. This profile is separate from the retained uncertain mission.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen8-dialogue-state'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen8-dialogue-state.turns.jsonl')]
