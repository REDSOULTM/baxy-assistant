...
out = ROOT / 'artifacts/comprobaciones/C03/astra-files72-qwen35'
...
files = ['tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-files72-qwen35.turns.jsonl',
         'scratchpad/c03-files72-qwen35.py']
files.append('src/Baxy.App/UserMessagePolicy.cs')
files.append('src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.dll')
files.append('artifacts/comprobaciones/C03/astra-files72-qwen35/CASES.json')
...
prereg['method'] = 'Source63 unchanged. Inherited Qwen3.5-4B Q4_K_M model override only; registry untouched. Same first five readonly file controls as63/64, then five technical transfer cases: actual error explanation, checksum knowledge, capabilities, clock/audio/CPU and missing volume level. No volume value is given or change requested. Same backend/ngl99/q8 KV/3x4096 and existing per-role samplers; non-thinking. Model71 selected9/10 versus registered6/10 full-context native cases, but requested a read for a why question. This panel checks real routing and composition, including that risk. Hook47 captures exact HTTP without changing packets; I/O overhead means not a timing benchmark. No UI/audio or human reserve. Model not promoted; judge entire turns.'
...
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-files72-qwen35'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
...
prereg['wireMethod']='Ten technical requests; hook47 records actual unchanged packets for inherited model override. See method for the five-case prefix and transfer cases.'
        ...
        while process.poll() is None:
            if not released:
                ...
                if time.monotonic()-started > 50:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-files72-qwen35.turns.jsonl').read_bytes()
