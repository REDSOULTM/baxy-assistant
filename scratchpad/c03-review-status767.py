"""Correlate frozen767 outputs; no verdict inferred from terminal status."""
...
public = root / 'artifacts/comprobaciones/C03/STATUS_BATCH767'
truncated = []
...
if not args.partial:
    ...
    assert all(outcome[k] for k in ['manifest_unchanged', 'sources_unchanged',
                                  'source764_unchanged', 'source766_unchanged', 'runner_unchanged', 'app_dll_unchanged'])
...
if not args.partial:
    ...
    markdown = ['# Respuestas privadas767 — pendientes de adjudicación',
