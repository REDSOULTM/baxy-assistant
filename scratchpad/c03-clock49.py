...
out = ROOT / 'artifacts/comprobaciones/C03/astra-clock49'
...
files = ['tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-clock49.turns.jsonl',
         'scratchpad/c03-clock49.py']
files.append('src/Baxy.App/UserMessagePolicy.cs')
files.append('src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.dll')
files.append('artifacts/comprobaciones/C03/astra-clock49/CASES.json')
...
prereg['method'] = 'Same seven inputs and registered runtime as compound-shell48. Only source difference is effect_intent clause normalization: retain shared clock/audio observations and coordinated state questions, preserving ordering and catalog completeness. No new clock/audio classifier or fixed plan; no model/prompt/sampler change. Known progress hallucination and false failure-polarity rejection remain unfixed in this comparison. Do not count a faithful final as a pass when progress is false. Technical read-only panel, not human reserve or UI/voice acceptance.'
...
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-clock49'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
        ...
        while process.poll() is None:
            if not released:
                ...
                if time.monotonic()-started > 50:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-clock49.turns.jsonl').read_bytes()
