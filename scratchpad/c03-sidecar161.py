...
out=base/'astra-sidecar161'
out.mkdir(exist_ok=False)
private=Path(os.environ['LOCALAPPDATA'])/'BAXY/C03-sidecar161-private'
...
client=ObservedProcess([observed['cmdline'][0],'-u','-X','utf8',str(root/'scratchpad/c03-sidecar161-entry.py')],environment=env,cwd=root)
rows=[]
...
try:
    ...
    names=['messages.jsonl','stderr.log','stacks161.log','catalog.json']
    indexes=[]
    for name in names:
        p=private/name
        with p.open('rb') as f:
