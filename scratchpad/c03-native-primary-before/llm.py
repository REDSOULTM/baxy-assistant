...
def compose_visible_defect(
    ...
    if visible_reply_invents_a_spanish_infinitive(stripped):
        return "invented"
    if (
        _SNAKE_CODE.search(stripped) is not None
        or _DOTTED_OP.search(stripped) is not None
    ):
...
class LlmRuntime:
    ...
    def compose_user_message(
        ...
        acting_facts = _compose_user_content(user_text, prompt_facts, language_contract)
        if cause == "acting":
            message_prompt = cpu_prompt
        ...
        retry_hint = {
            ...
            "acting_asserted": (
                "Still working." if response_language == "en" else "Sigo."
            ),
        ...
        retry_user = _compose_user_content(user_text, prompt_facts, "")
        retry_payload["messages"] = [
        ...
        third_user = _compose_user_content(user_text, prompt_facts, "")
