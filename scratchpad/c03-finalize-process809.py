...
private = base / "C03-process-batch809-private"
out = root / "artifacts/comprobaciones/C03/PROCESS_BATCH809"
...
seal_flags = ("manifest_unchanged", "sources_unchanged", "source808_unchanged",
...
old807 = {row["case_id"]: row["passed"] for row in read(
    root / "artifacts/comprobaciones/C03/PROCESS_BATCH807/ROOT_ADJUDICATION.json")["cases"]}
assert set(old801) == set(old807) == set(judgments)
assert len(old801) == len(old807) == 50 and sum(old801.values()) == 35 and sum(old807.values()) == 37
...
comparison = {str(batch): {"previous_valid": sum(old.values()),
              ...
              for batch, old in ((801, old801), (807, old807))}
previous_rows = read(base / "C03-process-batch807-private/review.json")
assert len(previous_rows) == 50
assert [{"case_id": row["case_id"], "group": row["group"]} for row in previous_rows] == prereg["cases"]
latency = {"metric": "Difference between first and last recorded turn-scoped shell event; completed finals only. Not acoustic latency.809 and807 each use their complete continuous50 review.json directly; no interrupted or resumed profiles are combined.",
           "809": {"all": times(review), "by_group": {group: times([row for row in review if row["group"] == group]) for group in groups}},
           "807": {"all": times(previous_rows), "by_group": {group: times([row for row in previous_rows if row["group"] == group]) for group in groups}}}
...
result = {"method": "Root read every final against its newly observed facts and the sealed criterion; this script assembles only those saved judgments.",
          "candidate": 808, "adopted": False, "continuous_50": True, "valid": valid, "failed": 50 - valid,
          ...
          "cases": cases, "evidence_sha256": {str(path): sha(path) for path in (
              ...
              root / "artifacts/comprobaciones/C03/PROCESS_BATCH807/ROOT_ADJUDICATION.json",
              base / "C03-process-batch807-private/review.json")},
          "survey_counts": {"covered": 28, "open": 714, "not_applicable": 0}}
write(out / "ROOT_ADJUDICATION.json", result)
write(out / "VERIFICATION_STATUS.json", {"source_candidate": 808, "source_adopted": False,
...
markdown = ["# Respuestas809 — adjudicación raíz", f"{valid}/50 válidas;{50-valid}/50 fallidas. La decisión de adopción se registra aparte."]
for row in review:
