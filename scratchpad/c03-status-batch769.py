...
OUT = ROOT / 'artifacts/comprobaciones/C03/STATUS_BATCH769'
PRIVATE = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-status-batch769-private'
PROFILE = PRIVATE.parent / 'C03-status-profile769'
...
pins768 = read(ROOT / 'artifacts/comprobaciones/C03/INVENTORY_CORRECTION768/SOURCE_PINS.json')
pins764 = read(ROOT / 'artifacts/comprobaciones/C03/DENSE_INVENTORY764/SOURCE_PINS.json')
assert all(sha(ROOT / path) == digest for path, digest in {**pins768, **pins764}.items())
...
prereg = {
    'utc': utc(), 'method': 'Registered73 product regression after inventory correction768 with dense764; original panel/criteria. Build prepared before sealing DLL; no model comparison.',
    ...
    'validation': 'Published768 owners/current pin integrity and Fast, dense764 owners/Fast retained. Full7 historical, no final acceptance.',
    'source768_pins': pins768, 'source764_pins': pins764, 'sources': sources, 'runner_sha256': sha(__file__),
...
outcome = {'utc': utc(), 'exit_code': code, 'manifest_unchanged': sha(manifest) == prereg['manifest_sha256'],
           ...
           'source768_unchanged': all(sha(ROOT / path) == digest for path, digest in pins768.items()),
