"""One-variable native replay of actual366 confirmation payloads."""
...
out = root / 'artifacts/comprobaciones/C03/astra-pending-action367'
out.mkdir(exist_ok=False)
base = Path(os.environ['LOCALAPPDATA']) / 'BAXY'
private = base / 'C03-pending-action367-private'
...
prereg = {
    ...
    'method': 'Replay four actual366 confirmation first payloads on the same diagnostic Qwen3.5. One difference: pendingAction identifying memory.enable and private local memory, identical to the existing safe initial-prompt projection. Initial t1/t6 already contain this fact and are unchanged controls; invalid-reply t2/t7 acquire it. No cause, language, prose rule, sampler, user message or source change. Only native completions; no effects or acceptance promotion.',
