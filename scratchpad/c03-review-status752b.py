"""Correlate frozen752B outputs; no verdict inferred from terminal status."""
...
public = root / 'artifacts/comprobaciones/C03/STATUS_BATCH752B'
truncated = []
...
if not args.partial:
    ...
    assert all(outcome[k] for k in ['manifest_unchanged', 'sources_unchanged',
                                  'source751_unchanged', 'runner_unchanged', 'app_dll_unchanged'])
...
if not args.partial:
    ...
    markdown = ['# Respuestas privadas752B — pendientes de adjudicación',
