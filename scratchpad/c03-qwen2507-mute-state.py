...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen2507-mute-state'
...
files = ['src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen2507-mute-state.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen2507-mute-state/profile/sitecustomize.py',
         'scratchpad/c03-qwen2507-mute-state.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'The narrator inverted observed muted=false in completed-request t6. Present the mute state as a categorical muted/unmuted fact, analogous to the existing derived local clock, retaining raw boolean for verification. No phrase blacklist, prompt or sampler change. Repeat the same 21 development inputs.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen2507-mute-state'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen2507-mute-state.turns.jsonl')]
