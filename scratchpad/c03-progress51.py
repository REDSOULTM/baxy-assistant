...
out = ROOT / 'artifacts/comprobaciones/C03/astra-progress51'
...
files = ['tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-progress51.turns.jsonl',
         'scratchpad/c03-progress51.py']
files.append('src/Baxy.App/UserMessagePolicy.cs')
files.append('src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.dll')
files.append('artifacts/comprobaciones/C03/astra-progress51/CASES.json')
...
prereg['method'] = 'Same seven inputs, registered runtime and source49 except llm progress narration. Native role51 restored explicit narration responsibility and gave five useful raw replies; two earlier representation-only variants were rejected. The composer now retains that instruction in first/retry attempts, without prescribing visible words. Existing instrument reading/predicate grammar rejects fabricated measurements even alongside a progress marker, while literal pending numeric targets remain allowed. No changes in C# or provider/model/sampler. Known C# reversed_result on no-failures prose remains unfixed for isolation. Technical read-only regression, not human reserve or UI/voice acceptance. Judge the complete visible turn, including labels.'
...
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-progress51'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
        ...
        while process.poll() is None:
            if not released:
                ...
                if time.monotonic()-started > 50:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-progress51.turns.jsonl').read_bytes()
