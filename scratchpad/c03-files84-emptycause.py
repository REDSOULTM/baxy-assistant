...
out = ROOT / 'artifacts/comprobaciones/C03/astra-files84-emptycause'
...
files = ['tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-files84-emptycause.turns.jsonl',
         'scratchpad/c03-files84-emptycause.py']
files.append('src/Baxy.App/UserMessagePolicy.cs')
files.append('src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.dll')
files.append('artifacts/comprobaciones/C03/astra-files84-emptycause/CASES.json')
...
prereg['method'] = 'Source81 plus84: reuse the verified identity-producer projection to distinguish an explicit zero-result sandbox file search/list from missing or unverified data. MindPlanSession stops the dependent read with file_search_no_matches before requesting an impossible identity from the model. No provider/catalog/model or prompt change, no basename substitution or global absence claim. Same four readonly inputs/fixtures and Qwen3.5 isolated override as83 (2/4 useful, cause lost to step_data_missing). No UI/audio/human reserve or promotion.'
...
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-files84-emptycause'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
...
prereg['wireMethod']='Four readonly technical requests from83; hook77 records actual HTTP payloads after system-prefix serialization, without changing them. Qwen3.5 isolated override; not acceptance.'
        ...
        while process.poll() is None:
            if not released:
                ...
                if time.monotonic()-started > 50:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-files84-emptycause.turns.jsonl').read_bytes()
