"""Complete battery category plus ten development variants through the registered, unmodified product.
...
OUT = ROOT / 'artifacts/comprobaciones/C03/STATUS_BATCH777'
PRIVATE = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-status-batch777-private'
PROFILE = PRIVATE.parent / 'C03-status-profile777'
...
assert len(panel) == 7
for index, text in enumerate(['Dime cuánta batería le queda a mi laptop.', 'Baxy, comprueba cuánta batería le queda a esta computadora.', '¿Cuánta batería le queda a este PC?', 'Dime cuánta batería le queda a mi notebook.', '¿Cuánta batería le queda a la laptop?', 'Comprueba cuánta batería le queda a mi computadora.', 'Baxy, dime cuánta battery le queda a mi laptop.', 'How much battery is left?', 'Check the battery level, please.', 'Baxy, battery level please.'], 1):
...
pins775 = read(ROOT / 'artifacts/comprobaciones/C03/BATTERY_ACTOR775/SOURCE_PINS.json')
pins764 = read(ROOT / 'artifacts/comprobaciones/C03/DENSE_INVENTORY764/SOURCE_PINS.json')
assert all(sha(ROOT / path) == digest for path, digest in {**pins775, **pins764}.items())
...
prereg = {
    'utc': utc(), 'method': 'Complete seven-case battery category plus ten declared development variants after battery actor775 candidate; original battery criterion unchanged. Build prepared before sealing DLL; no model comparison.',
    ...
    'validation': 'Candidate775 owners/current pin integrity and Fast; not adopted yet, dense764 owners/Fast retained. Full7 historical, no final acceptance.',
    'source775_pins': pins775, 'source764_pins': pins764, 'sources': sources, 'runner_sha256': sha(__file__),
...
with (PRIVATE / 'launch.log').open('w', encoding='utf-8') as log:
    ...
    print(json.dumps({'started': True, 'launcher': process.pid, 'registered_turns': 17}), flush=True)
...
outcome = {'utc': utc(), 'exit_code': code, 'manifest_unchanged': sha(manifest) == prereg['manifest_sha256'],
           ...
           'source775_unchanged': all(sha(ROOT / path) == digest for path, digest in pins775.items()),
