...
validation = read(BASE / 'INVENTORY_IDENTITY789/VALIDATION.json')
...
pins = read(BASE / 'INVENTORY_IDENTITY789/SOURCE_PINS.json')
...
prereg = {'utc': utc(), 'head': subprocess.check_output(['git', 'rev-parse', 'HEAD'], text=True).strip(),
    'candidate': 'INVENTORY_IDENTITY789 including preserved787; validated but not adopted yet', 'cases': len(cases),
