...
OUT = ROOT / "artifacts/comprobaciones/C03/astra-install218-retry"
...
def versions():
    return {dist.metadata["Name"].lower().replace("_", "-").replace(".", "-"): dist.version for dist in importlib.metadata.distributions()}
before = versions()
