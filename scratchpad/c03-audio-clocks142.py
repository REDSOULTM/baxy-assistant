...
out = root/'artifacts/comprobaciones/C03/astra-clocks142'
...
with pa.PyAudio() as backend:
    ...
    try:
        with sd.InputStream(device=wasapi['default_input_device'], extra_settings=sd.WasapiSettings(auto_convert=True),
...
metadata.update(columns=['perfCounter','inputBufferAdcTime','currentTime','frames','anyStatus'],
                method='WASAPI input on same default physical Realtek; shared auto_convert enabled; clocks only, no PCM retained/playback/volume change.',rows=rows)
