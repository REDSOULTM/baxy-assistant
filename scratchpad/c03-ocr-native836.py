"""Eight native framing controls over the sealed physical OCR832 payloads.

Root must supply the collected Full and final source-pin byte hashes. No grading.
Prepared without inference; only BAXY system/language framing is removed.
"""
...
OUT = BASE / 'OCR_NATIVE836'
PRIVATE = LOCAL / 'C03-ocr-native836-private'
PLAN836 = LOCAL / 'C03-ocr836-native-proposal/PLAN.json'
...
def framing_payloads(cases):
    ...
    for case, event, sent in zip(cases, merged, sends):
        ...
        native['messages'] = [{'role': 'user', 'content': content[:len(prefix) + json_end]}]
...
def main():
    ...
    require(args.expected_full_sha256 == FULL_SHA and args.expected_pins_sha256 == PINS_SHA, '836 must preserve832 Full/pins')
    require(sha(PLAN836) == args.expected_plan_sha256, '836 plan seal changed')
    plan836 = read(PLAN836)
    require(sha(__file__) == plan836['driver_sha256'], '836 driver seal changed')
    ...
    for case_id, payload in native_payloads.items():
        require(hashlib.sha256(json.dumps(payload, ensure_ascii=False, sort_keys=True).encode('utf-8')).hexdigest() == plan836['native_payload_sha256'][case_id], '836 payload differs from sealed control')
    ...
    seals.update({str(WIRE832): WIRE832_SHA, str(PREREG832): PREREG832_SHA, str(PLAN836): args.expected_plan_sha256})
    ...
    prereg = {'utc': utc(), 'head': subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip(),
        ...
        'input': '832 physical merged payload: remove BAXY system message and final generated language suffix only; exact literal request and situation JSON substring unchanged. All generation parameters unchanged.',
        'native_payload_sha256': plan836['native_payload_sha256'], 'control': 'Framing only; no model/profile/production change or acceptance credit',
