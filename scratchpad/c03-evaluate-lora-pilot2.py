...
OUT=ROOT/'artifacts/comprobaciones/C03/astra-lora-pilot2-evaluation'
...
OUT.mkdir()
prereg={'kind':'direct-model-pilot-evaluation-not-product-acceptance','cases':cases,'variants':['base','pilot_lora'],
...
class Candidate(LlmRuntime):
    ...
    def _server_command(self):
        return super()._server_command()+(['--lora',str(ADAPTER)] if self.adapter else [])
results=[]
for variant in ['base','pilot_lora']:
    client=Candidate(variant=='pilot_lora');gpu=ProcessTreeGpuSampler(os.getpid());ram=RamSampler(os.getpid())
