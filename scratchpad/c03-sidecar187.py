...
client=ObservedProcess([observed['cmdline'][0],'-u','-X','utf8',str(root/'scratchpad/c03-sidecar187-entry.py')],environment=env,cwd=root)
...
try:
    ...
    names=['messages.jsonl','stderr.log','stacks187.log','catalog.json','tts-state.jsonl']
    names.extend(name for name in ['tap187.npz','tap187.json','generated187.npz','generated187.json'] if (private/name).is_file())
    indexes=[]
    for name in names:
        p=private/name
        with p.open('rb') as f:
