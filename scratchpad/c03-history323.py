...
out = root / 'artifacts/comprobaciones/C03/astra-history-retention323'
out.mkdir(exist_ok=False)
private = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-memory-product322-private'
...
current = 'nono, te pregunte quien soy yo, no tu, dime quien eres tu y quien soy yo'
original = next(row['payload'] for row in posts if row['stage'] == 'request'
    and not row['payload'].get('tools')
    and row['payload']['messages'][-1].get('content') == current
    and row['payload']['messages'][0]['content'].startswith('Eres BAXY, un compañero que vive'))
assert all(message['role'] == 'system' for message in original['messages'][:-1])
...
for row in events:
    ...
    if entry['src'] == 'YOU' and entry['msg'] == current:
        break
    history.append({'role':'user' if entry['src'] == 'YOU' else 'assistant','content':entry['msg']})
restored = copy.deepcopy(original)
restored['messages'] = [*original['messages'][:-1], *history, original['messages'][-1]]
cases = [{'request':current,'variant':'before','payload':original},
         {'request':current,'variant':'retained_dialogue','payload':restored}]
...
(out / 'PREREG.json').write_text(json.dumps({
    ...
    'method': 'Replay exact chat payload322 for the final identity correction, merging leading systems as production does. Before is actual empty-history chat; after restores only the actual published dialogue available before that request. Same prompt/sampling/output limit; no contextual resolver or new grammar. This tests the history lost by App while a clarification was pending, without changing selection or authorization. Same registered model/backend/KV profile, standalone hidden server, no desktop UI or effects. Native replies319 remain baseline.',
...
try:
    ...
    for case in cases:
        ...
        result = {'request': case['request'], 'variant': case['variant']}
        try:
            result['answer'] = runtime._post(case['payload'])['choices'][0]['message']['content']
