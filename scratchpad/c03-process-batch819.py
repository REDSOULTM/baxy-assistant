"""Frozen 50-case process-read category through the registered product with cumulative repairs818, pending owners, Fast and Full guards.
...
PRIVATE = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-process-batch819-private'
PROFILE = PRIVATE.parent / 'C03-process-profile819'
...
plan = read(plan_path)
# Cumulative repairs818 requires its sealed owners, Fast and Full before product measurement.
...
assert read(validation_root / 'OWNERS_DOTNET_EXIT.json')['exit_code'] == 0
...
pins818 = read(validation_root / 'SOURCE_PINS.json')
assert len(pins818) == 33, 'repairs818 source pin set'
assert all(sha(ROOT / path) == digest for path, digest in pins818.items())
...
prereg = {
    'utc': utc(), 'method': 'Registered50 process-read category with sealed candidate818; nine historical requests and41 variants. Build prepared before sealing DLL; model selection is closed.',
    ...
    'validation': 'Cumulative C#816/818 and Python817 require PROCESS_REPAIRS818/FULL_RETRY1 runtime and reference owners, Fast and Full exit0; original Full818 red preserved against the common33 SOURCE_PINS before launch. New real product confirmation; adoption remains governed by the goal. No native model comparison or automated coverage credit.',
    ...
    'owners_dotnet_path': str(validation_root / 'OWNERS_DOTNET_EXIT.json'),
    'owners_dotnet_sha256': sha(validation_root / 'OWNERS_DOTNET_EXIT.json'),
    ...
    'source818_pins_sha256': sha(validation_root / 'SOURCE_PINS.json'),
    ...
    'source818_pins': pins818, 'sources': sources, 'runner_sha256': sha(__file__),
    'reviewer_sha256': sha(ROOT / 'scratchpad/c03-review-process819.py'),
...
outcome = {'utc': utc(), 'exit_code': code, 'manifest_unchanged': sha(manifest) == prereg['manifest_sha256'],
           ...
           'source818_unchanged': all(sha(ROOT / path) == digest for path, digest in pins818.items()),
