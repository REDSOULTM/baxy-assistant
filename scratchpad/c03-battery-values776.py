"""Fifty declared synthetic battery scenarios through the real local composer.
...
PREVIOUS = PRIVATE.parent / 'C03-status-batch774-private'
...
assert b'483 passed, 1 skipped in 5.63s' in (Path(os.environ['TEMP']) / 'c03-battery-actor775-final.log').read_bytes()
assert b'source_quality_gate_passed: mode=Fast' in (Path(os.environ['TEMP']) / 'c03-battery-actor775-fast.log').read_bytes()
...
pins = read(BASE / 'BATTERY_ACTOR775/SOURCE_PINS.json')
...
processes774 = read(PREVIOUS / 'processes.json')
prior_commands = [p['command'] for p in processes774.values() if p['name'].lower() == 'llama-server.exe']
assert len(prior_commands) == 1
previous_command = prior_commands[0]
captured = read(PREVIOUS / 'review.json')[1]['compose'][0]
template = json.loads(captured['situation'])
assert template['observed']['battery']['chargePercent'] == 100
OUT.mkdir()
PRIVATE.mkdir()
cases = []
questions = {
    'percent': ['Dime cuánta batería le queda a mi notebook.', 'How much battery is left?',
                'Baxy, dime cuánta battery le queda a mi laptop.'],
    'charging': ['¿Está cargando la batería?', 'Is the battery charging right now?',
                 'Baxy, ¿está cargando la battery ahora?'],
    'absence': ['¿Cuánta batería tiene este PC?', 'How much battery does this computer have?',
                'Dime cuánta battery tiene este PC.'],
    'unknown': ['¿Qué porcentaje de batería queda?', 'What percentage of battery is remaining?'],
}


def add(group, values, requests):
    for index, request in enumerate(requests):
        situation = copy.deepcopy(template)
        situation['observed']['battery'] = copy.deepcopy(values)
        cases.append({'id': f'battery776-{len(cases) + 1:02d}', 'group': group,
                      'language': ['es', 'en', 'mixed'][index], 'request': request,
                      'situation': situation, 'synthetic_observation': True,
                      'criterion': 'Answer requested percentage/charging state from the supplied fixture only. '
                                   'Absent battery is not zero charge; null measurements remain unknown. '
                                   'Preserve subject, language and every additional claim; do not infer charging from AC.'})


for index, percent in enumerate([0, 1, 9, 17, 31, 49, 67, 82, 99, 100]):
    charging = index % 2 == 1 and percent < 100
    add('percent', {'isPresent': True, 'chargePercent': percent, 'isCharging': charging,
                    'isAcOnline': charging or percent == 100}, questions['percent'])
for percent, charging, ac in [(17, True, True), (82, False, True), (49, False, False), (100, False, True)]:
    add('charging', {'isPresent': True, 'chargePercent': percent, 'isCharging': charging,
                     'isAcOnline': ac}, questions['charging'])
for ac in [True, False]:
    add('absence', {'isPresent': False, 'chargePercent': None, 'isCharging': None,
                    'isAcOnline': ac}, questions['absence'])
add('unknown', {'isPresent': True, 'chargePercent': None, 'isCharging': None,
                'isAcOnline': None}, questions['unknown'])
assert len(cases) == 50
...
prereg = {'utc': utc(), 'head': subprocess.check_output(['git', 'rev-parse', 'HEAD'], text=True).strip(),
    'candidate': 'BATTERY_ACTOR775, validated but not adopted yet', 'cases': len(cases),
    'groups': {'percent': 30, 'charging': 12, 'absence': 6, 'unknown': 2},
    ...
    'template_source': '774 complete observed situation; only battery values and user request vary in declared fixtures.',
    ...
    'environment': environment, 'previous774_server_command': previous_command,
    'environment_limit': 'Explicit OMP/MKL4 and TOKENIZERS=false match __main__ defaults; actual774 environment was not byte-captured.',
...
try:
    ...
    write(OUT / 'READY.json', {'utc': utc(), 'runner': os.getpid(), 'server_pid': client._process.pid,
                             'server_command': command, 'same774_command_except_port': True})
