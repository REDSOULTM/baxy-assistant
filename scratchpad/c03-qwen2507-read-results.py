...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen2507-read-results'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen2507-read-results.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen2507-read-results/profile/sitecustomize.py',
         'scratchpad/c03-qwen2507-read-results.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Read result descriptions must not claim state changes. Remove the instruction that prescribed a state change and both clock cropping functions; invalid drafts must recompose, not be converted to synthetic digits. Same candidate and profile. Four inherited controls plus mixed and unaffected greeting/non-action controls.',
          'expected': 'Observed time/audio conserved, no claim of changing the device, no synthetic clock-only crop; raw valid prose published or invalid prose recomposed.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen2507-read-results'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen2507-read-results.turns.jsonl')]
