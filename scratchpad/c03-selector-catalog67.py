...
out = base / 'astra-selector-catalog67'
...
prereg = {'method': 'Four exact source63 selector packets captured in files64-wire. '
...
try:
    ...
    for index, packet in enumerate(selected, 1):
        for variant in ('captured', 'file_description'):
            payload = copy.deepcopy(packet['payload'])
            context = json.loads(payload['messages'][-1]['content'])
            if variant == 'file_description':
