"""Correlate frozen819 outputs; no verdict inferred from terminal status."""
...
public = root / 'artifacts/comprobaciones/C03/PROCESS_BATCH819'
truncated = []
...
assert hashlib.sha256(scope_pins_path.read_bytes()).hexdigest() == prereg['source818_pins_sha256']
assert read(scope_pins_path) == prereg['source818_pins']
assert len(prereg['source818_pins']) == 33
assert read(validation_root / 'VALIDATION_EXIT.json')['source_pins_unchanged'] is True
assert read(validation_root / 'VALIDATION_EXIT.json')['source_pins_sha256'] == prereg['source818_pins_sha256']
...
if not args.partial:
    ...
    assert all(outcome[k] for k in ['manifest_unchanged', 'sources_unchanged',
                                  'source818_unchanged', 'runner_unchanged', 'app_dll_unchanged'])
...
if not args.partial:
    ...
    markdown = ['# Respuestas privadas819 — pendientes de adjudicación',
