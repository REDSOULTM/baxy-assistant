...
OUT = BASE / "astra-human231"
PRIVATE_ROOT = Path(os.environ["LOCALAPPDATA"]) / "BAXY"
PRIVATE = PRIVATE_ROOT / "C03-human231-private"
...
baseline_path = BASE / "astra-source225-snapshot/src/baxy_mind/voice.py"
assert sha(baseline_path) == read(BASE / "astra-source225-snapshot/FILES.json")["src/baxy_mind/voice.py"]
spec = importlib.util.spec_from_file_location("baxy_mind.voice_baseline231", baseline_path)
...
for row in inputs:
    ...
    variants = []
    for name, module in [("before225", baseline), ("after230", current)]:
