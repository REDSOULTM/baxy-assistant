"""Complete clock category plus35 development variants through the registered, unmodified product.
...
OUT = ROOT / 'artifacts/comprobaciones/C03/STATUS_BATCH779'
PRIVATE = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-status-batch779-private'
PROFILE = PRIVATE.parent / 'C03-status-profile779'
...
pins778 = read(ROOT / 'artifacts/comprobaciones/C03/CLOCK_SCOPE778/SOURCE_PINS.json')
pins764 = read(ROOT / 'artifacts/comprobaciones/C03/DENSE_INVENTORY764/SOURCE_PINS.json')
assert all(sha(ROOT / path) == digest for path, digest in {**pins778, **pins764}.items())
...
prereg = {
    'utc': utc(), 'method': 'Complete fifteen-case clock category plus35 declared development variants after shared clock scope778; original clock criterion unchanged. Build prepared before sealing DLL; no model comparison.',
    ...
    'validation': 'Candidate778 owners/current pin integrity and Fast; not adopted yet, dense764 owners/Fast retained. Full7 historical, no final acceptance.',
    'source778_pins': pins778, 'source764_pins': pins764, 'sources': sources, 'runner_sha256': sha(__file__),
...
outcome = {'utc': utc(), 'exit_code': code, 'manifest_unchanged': sha(manifest) == prereg['manifest_sha256'],
           ...
           'source778_unchanged': all(sha(ROOT / path) == digest for path, digest in pins778.items()),
