...
from baxy_mind.llm import LlmRuntime, _clarification_style_messages
from scripts.measure_mind_budget import ProcessTreeGpuSampler,RamSampler
OUT=ROOT/'artifacts/comprobaciones/C03/astra-direct-missing-language';OUT.mkdir(exist_ok=False)
...
prompt={'inherited':'extract_direct_arguments','change':{'old':old_instruction,'new':new_instruction,'language_style':'_clarification_style_messages(user_text)'},'tool':tool}
...
prereg={'cases':cases,'stages':['missing_only'],'prompt':prompt,
    'method':'Five direct extraction calls, three consumed literal corpus inputs plus two explicitly synthetic complete-argument controls. Add the existing clarification language/style policy to the prior missing-only variant; baseline and prior variant retained in astra-direct-missing-fields. Same schema and sampling, registered assets; no effects or model/runtime promotion.',
...
class Client(LlmRuntime):
    ...
    def _post(self,payload,*args,**kwargs):
        if self.stage=='missing_only':
            ...
            payload['messages'][1:1]=_clarification_style_messages(payload['messages'][-1]['content'])
