...
out = base / 'astra-gemma-native493'
private = local / 'C03-gemma-native493-private'
...
assert len(cases) == 14
for case in cases:
    case['payload'].update(temperature=1.0,top_p=.95,top_k=64,min_p=0.0,presence_penalty=0.0,repeat_penalty=1.0,max_tokens=3072,reasoning_budget_tokens=-1)
...
config.update(gguf='D:/BAXYRuntime/experiments/models/baxy-gemma4-e2b-published-f9b84ecd/gemma-4-E2B-it-Q4_K_M.gguf',llama_server='D:/BAXYRuntime/assets/llama-v0.4.0-b10809-cuda12.4/llama-server.exe',ngl=99)
assert sha(config['gguf'])=='9d4a5a653f2733a5faeb5f58a0e30bc064dfd569b0059cabb2547dbc4ba0f4b7'
...
try:
    ...
    for case in cases:
        ...
        print(json.dumps({'id': case['id'], 'variant': case['variant'],
                          'seconds': row['seconds'], 'choices': [{k:v for k,v in choice.items() if k!='message'} | {'message':{k:v for k,v in choice.get('message',{}).items() if k not in {'reasoning_content','reasoning'}}} for choice in value.get('response', {}).get('choices',[])],
