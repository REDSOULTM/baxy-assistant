...
out = root / 'artifacts/comprobaciones/C03/astra-recall-policy373'
private = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-recall-policy373-private'
...
hook = private / 'hook'
hook.mkdir()
hook_source = (root / 'scratchpad/c03-owner370-hook/sitecustomize.py').read_text(encoding='utf-8').replace('C03-memory-product370-private', 'C03-recall-policy373-private')
(hook / 'sitecustomize.py').write_text(hook_source, encoding='utf-8')
prereg = {'utc': datetime.now(timezone.utc).isoformat(), 'cases': cases,
    'method': 'Same five development inputs/history371 through actual sidecar turn.decide with authenticated current Core catalog. Core starts only for its read-only catalog hello, no operation request. Mind can propose actions but this diagnostic never executes them. Compare the forced-recall/error path evidenced366/370 against the existing contextual decision path, not a new prompt or context annotation. No source change, runtime promotion, fresh human acceptance, UI or voice.',
    'reason': '371 full history and372 human-only history do not solve the error-composition task; stop context variants. Generic personal questions are forced by the private parser to memory.recall before the mind sees them. Test the earlier decision owner; preserve private sealing and exact confirmation before any future source adoption.',
...
try:
    ...
    configure = {'id': 'catalog373', 'type': 'catalog.configure', 'capabilities': capabilities}
    ...
    client.close(graceful_message={'id': 'close373', 'type': 'shutdown'}, timeout=15)
