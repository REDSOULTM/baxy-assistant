"""Correlate frozen822 outputs; no verdict inferred from terminal status."""
...
public = root / 'artifacts/comprobaciones/C03/PROCESS_BATCH822'
truncated = []
...
assert hashlib.sha256(Path(__file__).read_bytes()).hexdigest() == prereg['reviewer_sha256']
...
assert baseline_pins_path == root / 'artifacts/comprobaciones/C03/PROCESS_REPAIRS818/FULL_RETRY1/SOURCE_PINS.json'
assert full_path == baseline_pins_path.parent / 'FULL_EXIT.json'
...
for path_key, sha_key, filename in [
    ...
    assert validation_path == (baseline_pins_path.parent if filename == 'OWNERS_DOTNET_EXIT.json' else validation_root) / filename
    ...
    if filename != 'OWNERS_DOTNET_EXIT.json':
        assert read(validation_path)['source_pins_unchanged'] is True
        assert read(validation_path)['source_pins_sha256'] == prereg['source821_pins_sha256']
scope_pins_path = validation_root / 'SOURCE_PINS.json'
assert hashlib.sha256(scope_pins_path.read_bytes()).hexdigest() == prereg['source821_pins_sha256']
assert read(scope_pins_path) == prereg['source821_pins']
assert len(prereg['source821_pins']) == 33
assert read(validation_root / 'VALIDATION_EXIT.json')['source_pins_unchanged'] is True
assert read(validation_root / 'VALIDATION_EXIT.json')['source_pins_sha256'] == prereg['source821_pins_sha256']
...
if not args.partial:
    ...
    assert all(outcome[k] for k in ['manifest_unchanged', 'sources_unchanged',
                                  'source821_unchanged', 'runner_unchanged', 'app_dll_unchanged'])
...
if not args.partial:
    ...
    markdown = ['# Respuestas privadas822 — pendientes de adjudicación',
