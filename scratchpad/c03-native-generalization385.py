...
out = root / 'artifacts/comprobaciones/C03/astra-native-generalization385'
private = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-native-generalization385-private'
...
cases_path = root / 'scratchpad/c03-native-generalization385-cases.json'
...
hook = private / 'hook'
hook.mkdir()
hook_source = (root / 'scratchpad/c03-owner370-hook/sitecustomize.py').read_text(encoding='utf-8').replace('C03-memory-product370-private', 'C03-native-generalization385-private')
hook_source += '\n_chat_without_draft = LlmRuntime.chat\ndef chat_without_draft(self, *args, **kwargs):\n    kwargs.pop(\"initial_reply\", None)\n    return _chat_without_draft(self, *args, **kwargs)\nLlmRuntime.chat = chat_without_draft\n'
(hook / 'sitecustomize.py').write_text(hook_source, encoding='utf-8')
prereg = {'utc': datetime.now(timezone.utc).isoformat(), 'cases': cases,
    'method': 'Seventeen frozen development/generalization controls. Baseline385 drops initial_reply at chat entry to reproduce the prior redundant decoder;386 keeps source383 unchanged. Both use the same frozen cases/history, authenticated catalog, model, prompt, sampling, guard/retry and no-effect sidecar harness. Every output adjudicated; no fresh human, UI, voice, operation execution or runtime promotion.',
    'case_file_sha256': sha(cases_path),
    'reason': '373 HTTP1/9 already answered Jordan correctly; generic possessive guard rejected them and retry2/10 lost history.374 repairs that owner.371/372 error history variants were not adopted. Generic personal questions are forced by the private parser to memory.recall before the mind sees them. Test the earlier decision owner; preserve private sealing and exact confirmation before any future source adoption.',
...
try:
    ...
    configure = {'id': 'catalog385', 'type': 'catalog.configure', 'capabilities': capabilities}
