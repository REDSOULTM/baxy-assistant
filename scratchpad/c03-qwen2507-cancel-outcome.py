...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen2507-cancel-outcome'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen2507-cancel-outcome.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen2507-cancel-outcome/profile/sitecustomize.py',
         'scratchpad/c03-qwen2507-cancel-outcome.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Cancellation was projected as outcome completed alongside remaining steps cancelled; first draft leaked an opaque windowId, retry then claimed the cancelled close happened. Project actual cancelled terminal, retain previous effects, remove window capability IDs only from narration projection, keep prepared action unchanged in shell.',
          'expected': 'Same21 requests/model/profile as spoken-clock. t18 must describe cancelled close with no false effect or leaked opaque ID; confirmation, closure, personal read and clocks preserved. No extra prose instructions, new veto or model sweep. Full mixed and knowledge failures remain adjudicated; no reserved acceptance or promotion.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen2507-cancel-outcome'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen2507-cancel-outcome.turns.jsonl')]
