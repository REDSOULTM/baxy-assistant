"""Frozen 50-case process-read category through the registered product with sealed process repair796.
...
OUT = ROOT / 'artifacts/comprobaciones/C03/PROCESS_BATCH797_FINAL'
PRIVATE = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-process-batch797-final-private'
PROFILE = PRIVATE.parent / 'C03-process-profile797-final'
...
plan = read(plan_path)
# This is an unadopted candidate experiment. Full39408 completed with only
# the historical metadata line-ending failure; its owning suite must pass
# after restoring exact published bytes. A green Full is still required
# before adopting source796. Do not classify the previous Full as passed.
repair_root = ROOT / 'artifacts/comprobaciones/C03/PROCESS_REPAIR796'
full = read(repair_root / 'FULL_RESULT.json')
assert full['exit_code'] == 1 and full['python']['failed'] == 1
assert full['failed_test'] == 'test_r278_reports_a_repaired_tree_with_no_restorable_files_left'
assert full['source796_unchanged'] and not full['eligible_for_adoption']
assert read(repair_root / 'INTEGRITY_EXIT.json')['exit_code'] == 0
assert len(full['repairs']) == 2 and all(
    r['restored_exact_head_blob'] and r['json_values_unchanged']
    and sha(ROOT / r['path']) == r['after_sha256'] for r in full['repairs'])
assert sha(panel_path) == plan['panel_sha256']
original_panel = read(panel_path)
assert len(original_panel) == 50
assert read(ROOT / 'artifacts/comprobaciones/C03/PROCESS_BATCH797/EXIT.json')['exit_code'] == 1
assert read(ROOT / 'artifacts/comprobaciones/C03/PROCESS_BATCH797/RESOURCES.json')['violations'] == ['system_free_ram_bound']
original_review = read(PRIVATE.parent / 'C03-process-batch797-private/live-review.json')
assert len(original_review) == 42
continuation_events = PRIVATE.parent / 'C03-process-batch797-resume-private/capture/events.jsonl'
with continuation_events.open(encoding='utf-8-sig') as stream:
    assert sum(json.loads(line).get('type') == 'terminal' for line in stream if line.strip()) == 5
assert read(ROOT / 'artifacts/comprobaciones/C03/PROCESS_BATCH797_RESUME/RESOURCES.json')['violations'] == ['system_free_ram_bound']
panel = original_panel[47:]
assert len(panel) == 3 and len({c['case_id'] for c in panel}) == 3
...
pins796 = read(ROOT / 'artifacts/comprobaciones/C03/PROCESS_REPAIR796/SOURCE_PINS.json')
pins764 = read(ROOT / 'artifacts/comprobaciones/C03/DENSE_INVENTORY764/SOURCE_PINS.json')
assert all(sha(ROOT / path) == digest for path, digest in {**pins796, **pins764}.items())
# Reuse the exact already validated DLL; do not restart idle build servers.
preparation_stdout = (PRIVATE.parent / 'C03-process-batch797-private/build-preparation.log').read_bytes()
assert all(sha(ROOT / path) == digest for path, digest in {**pins796, **pins764}.items())
...
app_sha = sha(app)
assert app_sha == read(ROOT / 'artifacts/comprobaciones/C03/PROCESS_BATCH797/PREREG.json')['app_dll_sha256']
OUT.mkdir()
PRIVATE.mkdir()
...
prereg = {
    'utc': utc(), 'method': 'Complete final3 independent frozen process requests after797 yielded42 and its first continuation5 before external RAM pressure. Steam and Playnite closed gracefully; editor preserved. Same17source pins/model/profile settings/DLL; fresh private profile means this is a second segment, the third segment, not an uninterrupted50-case run. No aggregate latency comparison with795.',
    ...
    'effect_scope': plan['effect_scope'], 'profile': 'Fresh isolated continuation profile after resource guard; conversation reset disclosed, wake disabled.', 'continuation_of':797, 'original_case_offset':47, 'case_count':3,
    'validation': 'Unadopted candidate796:3248 Python owner passes, Fast0; Full39408 had12653 Python passes and only a historical metadata integrity failure, repaired to exact HEAD bytes with the owning suite green. Full must be green before source adoption. This experiment does not approve the source or close C03.',
    'source796_pins': pins796, 'source764_pins': pins764, 'sources': sources, 'runner_sha256': sha(__file__),
...
with (PRIVATE / 'launch.log').open('w', encoding='utf-8') as log:
    ...
    print(json.dumps({'started': True, 'launcher': process.pid, 'registered_turns': 3}), flush=True)
...
outcome = {'utc': utc(), 'exit_code': code, 'manifest_unchanged': sha(manifest) == prereg['manifest_sha256'],
           ...
           'source796_unchanged': all(sha(ROOT / path) == digest for path, digest in pins796.items()),
