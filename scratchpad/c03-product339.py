...
out = root/'artifacts/comprobaciones/C03/astra-memory-product339'
...
prereg = {
    ...
    'method':'py main.py --conductor, default registered2507, actual source321/324/326/330/333/336/338 and current Release; full shared product admission/classification/chat/publication. Six exact human requests from owner264 transcript282 indices99,101,103,105,107,109 in their original order, one continuous session. Old assistant failures are not injected; current product generates replies. Dedicated private profile. No classifier/model injection or sampling override. The inherited human request explicitly asks to remember the name; any persistence stays in a dedicated private test profile, never owner data. This is development replay, not fresh acceptance. Conductor is not graphical UI or physical voice evidence.',
    'profile_inheritance':'Same isolation as319/322: dedicated direct child of LOCALAPPDATA/BAXY. Compared with337, only source338 scopes conversational capability facts to conversation situations; the new profile remains memory-disabled by default and no activation is injected; registered model/configuration unchanged.',
...
for name in list(env):
    if name.startswith(('BAXY_MIND_','BAXY_VOICE_','BAXY_C03_','BAXY_FIELD_')) or name in {'PYTHONPATH','BAXY_DATA_DIR','BAXY_ASSET_DESCRIPTOR','BAXY_APP_TRACE'}:
        env.pop(name,None)
env.update(BAXY_MIND_PYTHONPATH=str(root/'scratchpad/c03-owner339-hook')+os.pathsep+str(root/'src'), BAXY_VOICE_WAKE_ON_START='0',
...
command=['py','main.py','--conductor','--profile',str(private.parent/'C03-memory-profile339'),
