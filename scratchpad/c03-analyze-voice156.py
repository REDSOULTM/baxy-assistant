...
out = root / "artifacts/comprobaciones/C03/astra-analysis156"
out.mkdir(exist_ok=False)
(out / "PREREG.json").write_text(json.dumps({
    "purpose": "Offline content and alignment comparison of physical candidate154 capture",
    "sources": [154], "newPlayback": False, "sourceMutation": False,
...
for number in [154]:
    ...
    rows = []
    for phase in json.loads((source / "PREREG.json").read_text(encoding="utf-8"))["phases"]:
        ...
        generated = next(r for r in json.loads((source / "GENERATED_INDEX.json").read_text(encoding="utf-8")) if r["phase"] == phase)
    ...
    target = private / "analysis156.json"
