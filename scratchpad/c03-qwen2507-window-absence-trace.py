...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen2507-window-absence-trace'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen2507-window-absence-trace.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen2507-window-absence-trace/profile/sitecustomize.py',
         'scratchpad/c03-qwen2507-window-absence-trace.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'The original window.resolve already returns window_not_found in the journal. After preserving the response reason, a generic shell exception masked it as unsafe_completion. Trace only exception type and method (no message/private data), keep a truthful result_unverified terminal, and verify numeric clock-unit equivalence also in C#.',
          'expected': 'Identify the bounded post-observation failure; no new fixture or assumption of an open calculator. Cancellation does not certify confirmation. Three-turn development diagnostic.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen2507-window-absence-trace'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen2507-window-absence-trace.turns.jsonl')]
