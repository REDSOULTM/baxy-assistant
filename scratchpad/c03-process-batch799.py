"""Frozen 50-case process-read category through the registered product with sealed process repair796 plus vocabulary798.
...
OUT = ROOT / 'artifacts/comprobaciones/C03/PROCESS_BATCH799'
PRIVATE = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-process-batch799-private'
PROFILE = PRIVATE.parent / 'C03-process-profile799'
...
plan = read(plan_path)
# Require the current combined C#/Python source to have passed the complete gate.
validation_root = ROOT / 'artifacts/comprobaciones/C03/PROCESS_VOCABULARY798'
assert read(validation_root / 'FULL_EXIT.json')['exit_code'] == 0
assert read(validation_root / 'OWNERS_EXIT.json')['exit_code'] == 0
assert read(validation_root / 'REPLAY.json')['unchanged_first_call'] == 55
...
pins798 = read(ROOT / 'artifacts/comprobaciones/C03/PROCESS_VOCABULARY798/SOURCE_PINS_FINAL.json')
pins764 = read(ROOT / 'artifacts/comprobaciones/C03/DENSE_INVENTORY764/SOURCE_PINS.json')
assert all(sha(ROOT / path) == digest for path, digest in {**pins798, **pins764}.items())
...
prereg = {
    'utc': utc(), 'method': 'Registered50 process-read category with sealed candidate798; nine historical requests and41 variants. Build prepared before sealing DLL; model selection is closed.',
    ...
    'validation': 'Complete Full798 exit0; owning Python3247 passes/1 environmental skip/121subtests;71 C# passes;55 captured draft occurrences from10 cases survive vocabulary validation unchanged. New real product confirmation; no native model comparison or automated coverage credit.',
    'source798_pins': pins798, 'source764_pins': pins764, 'sources': sources, 'runner_sha256': sha(__file__),
...
outcome = {'utc': utc(), 'exit_code': code, 'manifest_unchanged': sha(manifest) == prereg['manifest_sha256'],
           ...
           'source798_unchanged': all(sha(ROOT / path) == digest for path, digest in pins798.items()),
