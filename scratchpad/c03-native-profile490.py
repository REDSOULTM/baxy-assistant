...
out=base/'astra-native-profile490';private=local/'C03-native-profile490-private'
...
cases=[dict(case, id=case['id']+'-seed'+str(seed), seed=seed) for case in cases for seed in ([0,17] if case['id'] in {'owner46','owner51'} else [0])]
...
write(out/'PREREG.json',{'utc':datetime.now(timezone.utc).isoformat(),'method':'Full production mind sidecar with current catalogue/real app snapshots, source487 unchanged; comparison against489 with only native sampler/profile changed and current registered model/runtime. Actual recovered owner264 indexes44/46/51 with exact immediately relevant pairs, plus five synthetic development controls. catalog.configure then turn.decide only: no provider invocations, volume/mute changes, UI or speech. Single diagnostic intervention: for cases with preceding user context, rank tools using that exact preceding user turn plus current text. Record both baseline and contextual ranked tools. No tool IDs from expected outcomes, no fabricated query, no forced decision, no prompt/source changes. Native calls alone now use the official Qwen2507 non-thinking profile: temperature .7, top_p .8, top_k20, min_p0, presence0, repeat1; seeds0/17 for both contextual requests, max1024 to expose finish reasons. All other calls remain production parameters. https://huggingface.co/Qwen/Qwen3-4B-Instruct-2507, researched448-484; no thinking available. This does not claim an optimum or reject the model globally. Production guards retained. This is not an adopted general history policy and does not authorize stale actions. Wait for actual E5 resources readiness as409; no fake readiness.','profile_reason':'Current registered runtime baseline for source regression, not a model-capability ranking or claim that defaults are optimal. Model-specific profile research448-484 remains; a bad result here is inspected at its first boundary, not a new default-based model rejection.','cases':cases,'criteria':'Useful operation-level proposal preserving both compound effects and order; correct grounded level and unmute=false where returned; negative/knowledge controls must not unmute. A plan with arguments to be bound later only proves selection, not provider success. Unresolved owner46/51 remain open until whole product is demonstrated. Compare whether audio.volume enters the shortlist and whether owner51 becomes an honest compound proposal. Owner46 diagnoses whether retrieval is sufficient there. Remaining cases control the previous correction. No reserve, acceptance or physical-resource credit.','model':config['gguf'],'model_sha256':sha(config['gguf']),'server':config['llama_server'],'server_sha256':sha(config['llama_server']),'manifest_sha256':sha(manifest),'core':str(core),'core_sha256':sha(core),'source_sha256':{str(p):sha(p) for p in [root/'src/baxy_mind/effect_intent.py',root/'src/baxy_mind/llm.py',root/'src/baxy_mind/__main__.py']},'private':str(private),'limits':{'gpu_stop_mib':3800,'minimum_free_ram_mib':768,'request_seconds':90}})
write(private/'cases.json',cases)
hook=private/'hook';hook.mkdir()
hook_source=(root/'scratchpad/c03-owner370-hook/sitecustomize.py').read_text(encoding='utf-8-sig').replace('C03-memory-product370-private','C03-native-profile490-private')
hook_source+=(root/'scratchpad/c03-observe407-hook.py').read_text(encoding='utf-8-sig').replace("Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-retrieval-startup407-private/startup-observer.jsonl'","Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-native-profile490-private/startup.jsonl'")
hook_source+=(root/'scratchpad/c03-context-retrieval489-hook.py').read_text(encoding='utf-8-sig').replace('C03-context-retrieval489-private','C03-native-profile490-private')
hook_source+=(root/'scratchpad/c03-native-profile490-hook.py').read_text(encoding='utf-8-sig')
...
try:
 ...
 config_req={'id':'catalog490','type':'catalog.configure','capabilities':capabilities}
 ...
 for case in cases:
  write(private/'current-profile.json',{'seed':case['seed']})
 ...
 stop.set();client.close(graceful_message={'id':'close490','type':'shutdown'},timeout=15);guard.join(timeout=5);gpu.stop();ram.stop()
