...
out = root / 'artifacts/comprobaciones/C03/astra-retry-history394'
private = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-retry-history394-private'
...
class Capture(llm.LlmRuntime):
    ...
    def _post(self, payload, *args, **kwargs):
        ...
        if is_retry:
            self.retries += 1
            if self.variant == 'retain-history':
                actual['messages'][-1:-1] = llm._bounded_history(self.case['history'])
...
try:
    ...
    for case in cases:
        ...
        for variant in ['baseline', 'retain-history']:
