...
out = base / 'astra-files-sandbox61'
...
prereg = {'method': 'Reuse the two scope59 native HTTP payloads byte-for-byte except one cause value: '
          'step data missing becomes no matches in the filesystem sandbox. The captured verified lookup has count=0; '
          ...
          'No product source promotion, PC effects, UI/audio or human reserve. Scope59 alone gave 0/2 useful replies. Empty60 removed the false system-data explanation but left the searched scope unnamed; this variation identifies the actual known scope in the cause fact.',
...
try:
    ...
    for row in rows:
        ...
        user['content'] = user['content'].replace('step data missing', 'no matches in the filesystem sandbox', 1)
