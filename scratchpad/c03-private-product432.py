...
out = root/'artifacts/comprobaciones/C03/astra-private-product432'
...
prereg = {
    ...
    'production_verification': 'Source431 explicit generic-app clarification; source425 cache-ram0 and GPU no-mmap. Hook observes HTTP and adds server verbosity/log only. Same eight product426 development turns; no source changes during run.',
...
for name in list(env):
    if name.startswith(('BAXY_MIND_','BAXY_VOICE_','BAXY_C03_','BAXY_FIELD_')) or name in {'PYTHONPATH','BAXY_DATA_DIR','BAXY_ASSET_DESCRIPTOR','BAXY_APP_TRACE'}:
        env.pop(name,None)
env.update(BAXY_MIND_LLM_GGUF=str(model), BAXY_MIND_PYTHONPATH=str(root/'scratchpad/c03-owner432-hook')+os.pathsep+str(root/'src'), BAXY_VOICE_WAKE_ON_START='0',
...
command=['py','main.py','--conductor','--profile',str(private.parent/'C03-private-profile432'),
