...
out = root/'artifacts/comprobaciones/C03/astra-private-product424'
private = Path(os.environ['LOCALAPPDATA'])/'BAXY/C03-private-product424-private'
...
prereg = {
    ...
    'loading_ablation': 'Only remove --no-mmap compared423. Cache-ram0 remains. Return to original mmap default if no resource or behavior regression; no other change.',
    'cache_change': 'Only --cache-ram0 relative422; exact b9980 default8192MiB host prompt cache. Same context slots KV precision weights and no-mmap. Native verbosity4 and process commandline are observation only. Quality/latency measured, not assumed.',
    'attribution_only': 'Same421 with per-process RSS/private commit and system available memory every guard tick. Measures cache limit change and allocation ownership; no fresh acceptance. No thresholds/source/weights change.',
    'load_change': '--no-mmap only; inherited390 mechanism on the same 4B model, not a promotion. Historical no-mmap flag is removed by this ablation. Exact b9980 help supports this flag. Record effective command. All remaining parameters and resource limits unchanged.',
    'method': 'Eight synthetic development turns in a new isolated real conductor profile. Same404b name/enable/store/read/conversation sequence, with an underspecified application request inserted before each explicit stored-name read. Source418 gives the already typed private request precedence over the pending public clarification, preserving exact private confirmation. Source416 OS fix also present but no OS requests. No prompt/weights/sampler/decision injection; native loading only disables mmap. Record whether each inserted request really leaves clarification pending; only then claim the targeted scenario was exercised. Not fresh acceptance, UI or physical voice.',
...
for name in list(env):
    if name.startswith(('BAXY_MIND_','BAXY_VOICE_','BAXY_C03_','BAXY_FIELD_')) or name in {'PYTHONPATH','BAXY_DATA_DIR','BAXY_ASSET_DESCRIPTOR','BAXY_APP_TRACE'}:
        env.pop(name,None)
env.update(BAXY_MIND_LLM_GGUF=str(model), BAXY_MIND_PYTHONPATH=str(root/'scratchpad/c03-owner424-hook')+os.pathsep+str(root/'src'), BAXY_VOICE_WAKE_ON_START='0',
...
command=['py','main.py','--conductor','--profile',str(private.parent/'C03-private-profile424'),
