...
out = ROOT / 'artifacts/comprobaciones/C03/astra-routes-regression46'
...
files = ['src/baxy_mind/voice_output.py','artifacts/comprobaciones/C03/TRAMO45_PINS.json','tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-routes-regression46.turns.jsonl',
         'scratchpad/c03-routes-regression46.py']
...
prereg={'kind':'qwen-base-integrated-diagnostic-not-acceptance','utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
 'method':'Registered Qwen runtime with no model/KV/PythonPath/sampling overrides or sitecustomize instrumentation. Existing 33 consumed technical regression requests (including historical synthetic mixed prompts, NOT human acceptance), then separate reject/timeout/exhaust injections each followed by restore and a normal request in the SAME process/profile/session. Recovery requests do not count as normal acceptance. Existing own empty window fixture; no user windows are closed. Conductor inherently disables wake; this run cannot prove simultaneous voice budget or graphical UI.',
...
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-routes-regression46'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
        ...
        while process.poll() is None:
            if not released:
                ...
                if time.monotonic()-started > 50:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-routes-regression46.turns.jsonl').read_bytes()
