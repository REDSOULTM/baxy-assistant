...
PUBLIC = ROOT / 'artifacts/comprobaciones/C03/astra-fixture-startup-probe713'
...
HELPER = ROOT / 'tests/Baxy.Integration.Tests/TemporaryStartupProbe713.cs'
...
replacement = newline.join([
    b'            await using var viewModel = new MainWindowViewModel();',
    b'            using (TemporaryStartupProbe713.Observe(viewModel))',
    b'            {',
    b'                await viewModel.InitializeAsync(CancellationToken.None);',
    b'            }',
    b'            Assert.Multiple(() =>',
...
command = ['dotnet', 'test', 'tests/Baxy.Integration.Tests', '-c', 'Release', '--nologo', '-v:minimal',
           '--filter', 'FullyQualifiedName~TemporaryStartupProbe713', '--logger', 'console;verbosity=normal']
write(PUBLIC / 'PREREG.json', {
    ...
    'fixture_before_sha256': sha(FIXTURE), 'candidate': '../astra-catalog-source712/CANDIDATE.json',
...
(PUBLIC / 'TemporaryStartupProbe713.cs.txt').write_bytes(HELPER.read_bytes())
