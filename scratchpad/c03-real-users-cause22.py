...
out = ROOT / 'artifacts/comprobaciones/C03/astra-real-users-cause22'
...
files = ['tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-real-users-cause22.turns.jsonl',
         'scratchpad/c03-real-users-cause22.py']
files.append('artifacts/comprobaciones/C03/astra-real-users-cause22/CASES.json')
...
prereg={'kind':'qwen-base-integrated-diagnostic-not-acceptance','utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
 'method':'Twenty known literal historical inputs plus two cleanup/verification requests with CASES provenance and expectations. Inherited conductor, registered runtime without model/KV/PythonPath/sampling overrides. No rewritten text or forced spanglish instructions. Development only: source freshness/training independence not established. Actual audio effects are followed by an explicit volume100 request and independent status reading; preserve all failed context turns. No synthetic fixture or injected errors. Conductor disables wake by design; not UI or hardware acceptance.',
...
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-real-users-cause22'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
        ...
        while process.poll() is None:
            if not released:
                ...
                if time.monotonic()-started > 50:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-real-users-cause22.turns.jsonl').read_bytes()
