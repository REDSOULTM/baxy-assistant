...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen2507-confirmation-route'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen2507-confirmation-route.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen2507-confirmation-route/profile/sitecustomize.py',
         'scratchpad/c03-qwen2507-confirmation-route.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Exercise real app.close confirmation (WorkLoss/Sensitive in the catalog) after opening Calculator. Cancel instead of confirming in ES and EN; read-only queries verify continuation. No request to send external messages or power off. Original product guards/prompts.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen2507-confirmation-route'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen2507-confirmation-route.turns.jsonl')]
