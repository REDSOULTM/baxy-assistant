...
private = base / "C03-process-batch824-private"
out = root / "artifacts/comprobaciones/C03/PROCESS_BATCH824"
...
seal_flags = ("manifest_unchanged", "sources_unchanged", "source823_unchanged",
...
# Only root creates this private file after reviewing all 50 terminals against sealed criteria.
assert (private / "root-adjudication.json").is_file(), "Root must save 50 judgments after panel824; no automatic adjudication"
...
# Validate the scripts, source823 and DLL against this run's own preregistration.
runner = root / "scratchpad/c03-process-batch824.py"
reviewer = root / "scratchpad/c03-review-process824.py"
assert sha(runner) == prereg["runner_sha256"]
assert sha(reviewer) == prereg["reviewer_sha256"]
validation_root = root / "artifacts/comprobaciones/C03/PROCESS_NARRATOR823"
baseline_root = root / "artifacts/comprobaciones/C03/PROCESS_REPAIRS818/FULL_RETRY1"
pins_path = validation_root / "SOURCE_PINS.json"
assert sha(pins_path) == prereg["source823_pins_sha256"]
assert read(pins_path) == prereg["source823_pins"]
assert len(prereg["source823_pins"]) == 33
assert all(sha(root / path) == digest for path, digest in prereg["source823_pins"].items())
assert all(sha(root / path) == digest for path, digest in prereg["sources"].items())
app = root / "src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.dll"
assert sha(app) == prereg["app_dll_sha256"]
for path_key, hash_key, filename in (
    ...
    ("owners_dotnet_path", "owners_dotnet_sha256", "OWNERS_DOTNET_EXIT.json"),
    ("owners_reference_path", "owners_reference_sha256", "OWNERS_REFERENCE_EXIT.json"),
    ("full_validation_path", "full_validation_sha256", "FULL_EXIT.json"),
):
    receipt_path = Path(prereg[path_key])
    assert receipt_path == (baseline_root if filename in ("FULL_EXIT.json", "OWNERS_DOTNET_EXIT.json") else validation_root) / filename
    assert sha(receipt_path) == prereg[hash_key]
    receipt = read(receipt_path)
    assert type(receipt["exit_code"]) is int and receipt["exit_code"] == 0
    if filename != "OWNERS_DOTNET_EXIT.json":
        assert receipt["source_pins_unchanged"] is True
        assert receipt["source_pins_sha256"] == prereg["full_baseline_source_pins_sha256" if filename == "FULL_EXIT.json" else "source823_pins_sha256"]
assert Path(prereg["full_baseline_source_pins_path"]) == baseline_root / "SOURCE_PINS.json"
assert prereg["full_baseline_source_pins_sha256"] == sha(baseline_root / "SOURCE_PINS.json")
...
old822 = {row["case_id"]: row["passed"] for row in read(
    root / "artifacts/comprobaciones/C03/PROCESS_BATCH822/ROOT_ADJUDICATION.json")["cases"]}
assert set(old801) == set(old822) == set(judgments)
assert len(old801) == len(old822) == 50 and sum(old801.values()) == 35 and sum(old822.values()) == 43
...
comparison = {str(batch): {"previous_valid": sum(old.values()),
              ...
              for batch, old in ((801, old801), (822, old822))}
previous_rows = read(base / "C03-process-batch822-private/review.json")
assert len(previous_rows) == 50
assert [{"case_id": row["case_id"], "group": row["group"]} for row in previous_rows] == prereg["cases"]
latency = {"metric": "Difference between first and last recorded turn-scoped shell event; all 50 completed terminals regardless of terminal kind, explicitly including composition_failed. Not acoustic latency.824 and822 each use their complete continuous50 review.json directly; no interrupted or resumed profiles are combined.",
           "824": {"all": times(review), "by_group": {group: times([row for row in review if row["group"] == group]) for group in groups}},
           "822": {"all": times(previous_rows), "by_group": {group: times([row for row in previous_rows if row["group"] == group]) for group in groups}}}
valid = sum(row["passed"] for row in cases)
result = {"method": "Root read every terminal, including composition_failed, against its newly observed facts and the sealed criterion; this script assembles only those saved judgments.",
          "candidate": 823, "adopted": False, "continuous_50": True, "valid": valid, "failed": 50 - valid,
          ...
          "cases": cases, "evidence_sha256": {str(path): sha(path) for path in (
              runner, reviewer, pins_path, app,
              validation_root / "OWNERS_PYTHON_EXIT.json", baseline_root / "OWNERS_DOTNET_EXIT.json",
              validation_root / "VALIDATION_EXIT.json", baseline_root / "FULL_EXIT.json", baseline_root / "SOURCE_PINS.json",
              validation_root / "OWNERS_REFERENCE_EXIT.json",
              ...
              root / "artifacts/comprobaciones/C03/PROCESS_BATCH822/ROOT_ADJUDICATION.json",
              base / "C03-process-batch822-private/review.json")},
          "survey_counts": {"covered": 28, "open": 714, "not_applicable": 0}}
write(out / "ROOT_ADJUDICATION.json", result)
write(out / "VERIFICATION_STATUS.json", {"source_candidate": 823, "source_adopted": False,
...
markdown = ["# Respuestas824 — adjudicación raíz", f"{valid}/50 válidas;{50-valid}/50 fallidas. La decisión de adopción se registra aparte."]
for row in review:
