...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen8-uncertain-resume'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen8-uncertain-resume.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen8-uncertain-resume/profile/sitecustomize.py',
         'scratchpad/c03-qwen8-uncertain-resume.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Resume the retained uncertain app.open from the noun-prose profile without replaying its invocation. Structured pending/unknown/evidence-retained facts must prevent false closed or cancelled claims. The observed Calculator window remains open; no action will be repeated by this diagnostic.',
          'expected': 'Three development turns on the original durable profile: close request, cancel, clock read. Truthful uncertainty, retained evidence, no app.close/open execution and verified clock recovery. No acceptance or promotion.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen8-uncertain-resume.turns.jsonl')]
