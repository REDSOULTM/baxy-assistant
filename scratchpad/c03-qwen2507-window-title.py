...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen2507-window-title'
...
files = ['src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen2507-window-title.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen2507-window-title/profile/sitecustomize.py',
         'scratchpad/c03-qwen2507-window-title.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Window provider enumerated only process.MainWindowHandle and process names. Inherit EnumWindows and existing application title matching to return verified windows by process or title, preserving distinct windows of a shared host; include observed title for disambiguation. No app-specific aliases.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen2507-window-title'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen2507-window-title.turns.jsonl')]
