...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen8-request-preserved'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen8-request-preserved.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen8-request-preserved/profile/sitecustomize.py',
         'scratchpad/c03-qwen8-request-preserved.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Preserve the actual request in progress composition instead of sending only a generic status; stop deleting the first word of imperative drafts to make them pass. Same model, sampling, corpus and budgets as astra-qwen8-sentences. Timing instrumentation distinguishes preparation calls without changing their inputs or outputs.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen8-request-preserved'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen8-request-preserved.turns.jsonl')]
