...
client=ObservedProcess([observed['cmdline'][0],'-u','-X','utf8',str(root/'scratchpad/c03-sidecar165-entry.py')],environment=env,cwd=root)
rows=[]
...
try:
    ...
    names=['messages.jsonl','stderr.log','stacks165.log','catalog.json']
    indexes=[]
    for name in names:
        p=private/name
        with p.open('rb') as f:
