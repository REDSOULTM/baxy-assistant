...
prereg = {'method': 'Same five89 cases and packets, replacing previousExchange data with previousResponse only. '
...
try:
    ...
    for key, text, language, previous_request, previous_answer in cases:
        for variant in ('previous_response_data',):
            payload = copy.deepcopy(template)
            situation = {'kind': 'conversation'}
            if variant == 'previous_response_data':
