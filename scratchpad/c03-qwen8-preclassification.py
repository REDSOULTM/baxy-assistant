...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen8-preclassification'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen8-preclassification.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen8-preclassification/profile/sitecustomize.py',
         'scratchpad/c03-qwen8-preclassification.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Remove synchronous progress composition before any turn classification or catalog probe. It was introduced by c1ebb79b inside an unchanged 17-second normal deadline and consumed that deadline in unrelated retries. Keep known compound and execution progress, classification, catalog validation, model, sampling, corpus and budgets unchanged. This tests scheduling, not another prose prompt.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen8-preclassification'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen8-preclassification.turns.jsonl')]
