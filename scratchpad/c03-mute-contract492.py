...
out = base / 'astra-mute-contract492'
private = local / 'C03-mute-contract492-private'
...
write(out / 'PREREG.json', {
    ...
    'method': 'Native requests only. One new contract fact added to audio.mute description in both interfaces: state=true mutes output;state=false restores sound/unmute. This is the actual provider semantic (AudioMuteHandler forwards state to SetMuteAsync; WindowsAudioControlProvider calls SetMuted) absent from current schema/descriptor. Four exact contextual payloads490 with official Qwen2507 profile/seeds0,17; six synthetic consumed controls compare zero-argument versus actual typed schemas, each with the same new descriptor fact with same28 candidates and no history. Typed interface replaces only zero schemas with authenticated current schemas and removes the now-false zero-argument sentence. No forced tools, examples or expected operation lists. The descriptor fact is the sole new factor versus respective490/491 zero/typed references; existing typed interface removes the zero-arguments sentence as before. No BAXY semantic guards, ranking, argument extraction, providers, UI or voice in this isolated test.',
    'hypothesis': '491 real schemas recover both operations in owner51 but both seeds produce state=true, which mutes instead of unmuting. The actual state property has only type:boolean and no polarity description. Test explicit verified polarity in the sole catalogue description before changing production; also compare zero interface to choose the lightest improvement.',
