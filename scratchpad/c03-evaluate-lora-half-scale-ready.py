"""Base vs pilot LoRA on the frozen 18 holdout and 9 historical development cases."""
...
OUT=ROOT/'artifacts/comprobaciones/C03/astra-lora-pilot2-half-scale-ready'
assert not OUT.exists()
BASE=Path('D:/BAXYRuntime/experiments/models/qwen3-4b-instruct-2507-training-cdbee75f')
ADAPTER=BASE/'c03-pilot-lora-v2-f32.gguf'
MODEL=Path('D:/BAXYRuntime/experiments/models/qwen3-4b-instruct-2507-a06e946b/Qwen3-4B-Instruct-2507-Q4_K_M.gguf')
assert ADAPTER.is_file()
training=json.loads((ROOT/'artifacts/comprobaciones/C03/astra-lora-pilot2-training/RESULT.json').read_text())
assert training['status']=='trained-not-promoted' and training['gpuPeakMiB']<=4096
def digest(path):
    with path.open('rb') as stream:return hashlib.file_digest(stream,'sha256').hexdigest()
holdout=ROOT/'artifacts/comprobaciones/C03/astra-lora-pilot2-data/HOLDOUT.jsonl'
...
OUT.mkdir()
prereg={'kind':'direct-model-pilot-evaluation-not-product-acceptance','hypothesis':'Reducing adapter strength may preserve language gains without losing base explanation quality. One fixed scale 0.5 versus already recorded 1.0; no training or acceptance changes.','cases':cases,'variants':['half_scale'],
         ...
         'hashes':{str(path):digest(path) for path in [MODEL,ADAPTER,holdout,prior,ROOT/'src/baxy_mind/llm.py']},
...
class Candidate(LlmRuntime):
    ...
    def _server_command(self):
        return super()._server_command()+(['--lora-scaled',str(ADAPTER)[2:]+':0.5'] if self.adapter else [])
results=[]
for variant in ['half_scale']:
    client=Candidate(True);gpu=ProcessTreeGpuSampler(os.getpid());ram=RamSampler(os.getpid())
