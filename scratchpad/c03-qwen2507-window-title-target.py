...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen2507-window-title-target'
...
files = ['src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen2507-window-title-target.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen2507-window-title-target/profile/sitecustomize.py',
         'scratchpad/c03-qwen2507-window-title-target.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Previous capture: domain_grounding vetoed a named window and no close effect survived. Recognize the window-title grammatical relation without app aliases; retain app.close as the effect and leave title-to-window resolution and confirmation to existing planner/kernel. Same six development cases; voice timeout and task selection remain measured regressions.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen2507-window-title-target'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen2507-window-title-target.turns.jsonl')]
