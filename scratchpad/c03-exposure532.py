"""Refresh532 cumulative483 exposure; no case is executed, displayed or frozen."""
...
prior = base / 'C03-private-exposure483-private/review.jsonl'
...
out = base / 'C03-exposure532-private'
...
with (out/'review.jsonl').open('x', encoding='utf-8') as stream:
    for row in rows:
        ...
        row['current_exposure532'] = sorted(matches.get(row['id'], []))
        if row['current_exposure532']:
...
summary = dict(prior_review=str(prior), cumulative_prior_exposure_preserved=True, joined=len(rows), source_count=len(sources), matched_current=len(matches),
    newly_exposed_since483=len(newly_exposed), both_positive_still_requires_audit=len(remaining),
...
public = root/'artifacts/comprobaciones/C03/astra-exposure532'
