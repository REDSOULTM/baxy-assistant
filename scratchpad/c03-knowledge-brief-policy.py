...
OUT=ROOT/'artifacts/comprobaciones/C03/astra-knowledge-brief-policy';OUT.mkdir(exist_ok=False)
...
new_policy='la extensión que necesite el pedido. Para una pregunta factual sencilla suele bastar una o dos frases; amplía cuando la persona pida detalle y respeta su formato.'
...
prereg={'cases':cases,'stages':['current','brief_by_request'],'prompt':prompt,
    'method':'Ten BAXY chat calls: three consumed literal corpus inputs plus two labeled synthetic regression controls. No-history knowledge, temperature0 matching the product caller. Compare current maximum-four-sentences policy with brief-by-request guidance; no effect tools, no source or registered runtime change.',
