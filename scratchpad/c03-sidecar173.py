...
private=Path(os.environ['LOCALAPPDATA'])/'BAXY/C03-sidecar173-private'
...
client=ObservedProcess([observed['cmdline'][0],'-u','-X','utf8',str(root/'scratchpad/c03-sidecar173-entry.py')],environment=env,cwd=root)
...
try:
    ...
    names=['messages.jsonl','stderr.log','stacks173.log','catalog.json','tts-state.jsonl']
    indexes=[]
    for name in names:
        p=private/name
        with p.open('rb') as f:
