...
out = base / 'astra-qwen-precision479'
private = local / 'C03-qwen-precision479-private'
...
for key in list(os.environ):
    if key.startswith('BAXY_MIND_'):
        os.environ.pop(key)
os.environ.update(BAXY_MIND_LLM_GGUF=str(model), BAXY_MIND_LLAMA_SERVER=str(server), BAXY_MIND_NGL='26', BAXY_MIND_KV_CACHE_TYPE='q8_0')
...
write(out / 'PREREG.json', {
    ...
    'models': [{'precision':tag,'path':str(path),'sha256':digest} for tag,path,digest in models], 'ngl':26, 'server': str(server), 'server_sha256': sha(server),
...
class Measured(LlmRuntime):
    def _server_command(self):
        ...
        return [*command, '--reasoning-format', 'deepseek', '--verbosity','4','--log-file', str(private / f'server-{self.tag}.log')]
