"""Replay exact232 clean PCM through product segmentation and installed ASR."""
import hashlib
...
baseline_path = ROOT / "artifacts/comprobaciones/C03/astra-source225-snapshot/src/baxy_mind/voice.py"
spec = importlib.util.spec_from_file_location("baxy_mind.voice_baseline235", baseline_path)
voice = importlib.util.module_from_spec(spec)
sys.modules[spec.name] = voice
spec.loader.exec_module(voice)

OUT = ROOT / "artifacts/comprobaciones/C03/astra-baseline235"
PRIVATE = Path(os.environ["LOCALAPPDATA"]) / "BAXY/C03-baseline235-private"
...
prior = json.loads((ROOT / "artifacts/comprobaciones/C03/astra-voice232/RESULTS.json").read_text(encoding="utf-8"))
source = Path(os.environ["LOCALAPPDATA"]) / "BAXY/C03-voice232-private/tap232.npz"
...
save(OUT / "PREREG.json", {
    "method": "All1316 exact232 cleaned frames through real product capture segmentation and fresh SileroVad. Recorded speaking state; echo guard receives the recorded delayed raw microphone/reference pair, as in Speex. Inert device/output/ducker only, no physical effects. Decode actual queued segments with installed baxy.2. Compare actual VAD and guard to221; keep mismatches honest.",
    "hypothesis": "Replay both physical232 interruptions on baseline source225, before provisional admission, to separate acoustic failures from the new state change. Compare VAD, actual guard and speaking timeline against232; no threshold or decoder change.",
    "sourceSha256": sha(baseline_path), "scriptSha256": sha(Path(__file__)),
...
def recorded_guard(_microphone, _reference):
    i = stream.reads - 1
    result = original_guard(near[i - 1] * 32768 if i else np.zeros(512), reference[i - 1] if i else np.zeros(4512))
...
np.savez(PRIVATE / "segments235.npz", **signals)
