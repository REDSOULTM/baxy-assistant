...
client=ObservedProcess([observed['cmdline'][0],'-u','-X','utf8',str(root/'scratchpad/c03-sidecar168-entry.py')],environment=env,cwd=root)
...
try:
    ...
    names=['messages.jsonl','stderr.log','stacks168.log','catalog.json','tts-state.jsonl']
    indexes=[]
    for name in names:
        p=private/name
        with p.open('rb') as f:
