...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen8-cpu-budget'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen8-cpu-budget.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen8-cpu-budget/profile/sitecustomize.py',
         'scratchpad/c03-qwen8-cpu-budget.py']
...
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          ...
          'profile': {'model': str(model), 'sha256': sha(model), 'ngl': 20,
                      'kv': 'q8_0', 'hostCompositionBudgetProfile': 'existing CPU; host NGL=0, isolated sidecar NGL=20', 'sampling': {'temperature': 0.7, 'top_p': 0.8, 'top_k': 20, 'min_p': 0},
...
env = os.environ.copy()
env.update(BAXY_MIND_LLM_GGUF=str(model), BAXY_MIND_NGL='0', BAXY_MIND_KV_CACHE_TYPE='q8_0',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen8-cpu-budget'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen8-cpu-budget.turns.jsonl')]
