...
WHEEL = FOLDER / "wheels" / "pywebrtc_audio-0.2.0-cp312-cp312-win_amd64.whl"
WHEEL_SHA = "718f8681940df85abb3535790721b3fc735a564a0e7fbb82f5ed747401816e69"
...
class EchoCanceller:
    def __init__(self):
        ...
        # Linear native:64block framing; bridge adds128; final parity retained.
        self._mic_delay = np.zeros(192, np.float32)
    ...
    def process(self, microphone, reference):
        ...
        while len(self._near) >= 160:
            ...
            self._final = np.r_[self._final, clean]
            self._out = np.r_[self._out, self._native.last_linear_frame()]
            self._near = self._near[160:]
            self._far = self._far[160:]
        assert len(self._out) >= 512
        self.last_final, self._final = self._final[:512].copy(), self._final[512:]
        ...
        pair = np.r_[self._previous_reference, ref[-512:]][:-192][-4512:]
