...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen2507-role-sampling'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen2507-role-sampling.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen2507-role-sampling/profile/sitecustomize.py',
         'scratchpad/c03-qwen2507-role-sampling.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Remove the diagnostic global sampling override and observe the existing per-role runtime payloads, including deterministic decisions. Same 12-turn panel, original instructions/guards, with raw conversation audit before veto. No model promotion or additional decoding.',
          'expected': 'Useful faithful replies without global sampling overriding classifiers. Record every actual role sampler and raw public draft; inspect whether remaining language failures are genuine monolingual answers or false rejections.',
          'profile': {'model': str(model), 'sha256': sha(model), 'ngl': 99,
                      'kv': 'q8_0', 'sampling': 'Existing role payloads unchanged; effective values in sampling.jsonl',
                      'source': 'src/baxy_mind/llm.py; no sampling override',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen2507-role-sampling'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen2507-role-sampling.turns.jsonl')]
