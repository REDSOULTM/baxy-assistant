...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen2507-explanation'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen2507-explanation.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen2507-explanation/profile/sitecustomize.py',
         'scratchpad/c03-qwen2507-explanation.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Complete explanation requests now use the existing knowledge classifier for generic explain/explica/explicame, retaining the original question and existing catalog checks. Previously only definitions, hardware and why/difference frames qualified; the integrated decider asked an unnecessary encryption type question. Same model, sampling, six controls, GPU profile and budgets as qwen2507-product. No promotion.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen2507-explanation'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen2507-explanation.turns.jsonl')]
