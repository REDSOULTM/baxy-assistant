...
out = ROOT / 'artifacts/comprobaciones/C03/astra-compound-shell48'
...
files = ['tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-compound-shell48.turns.jsonl',
         'scratchpad/c03-compound-shell48.py']
files.append('src/Baxy.App/UserMessagePolicy.cs')
files.append('src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.dll')
files.append('artifacts/comprobaciones/C03/astra-compound-shell48/CASES.json')
...
prereg['method'] = 'Source47 plus removal of C# clock/audio substring shortcut and fixed two-step plan. Same triple request that droppedCPU in wire-context47; known ES/EN/mixed pair controls and independent prohibition. Registered runtime, no hook/PythonPath/model/sampler overrides. Read-only conductor, not UI or voice acceptance. Candidate uses existing literal reader and generic mission path, with current progress composition. No new classifier/regex. Prior context differs from baseline13, so compare requested operations, verified facts and completeness, not prose identity or latency as a controlled improvement.'
...
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-compound-shell48'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
        ...
        while process.poll() is None:
            if not released:
                ...
                if time.monotonic()-started > 50:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-compound-shell48.turns.jsonl').read_bytes()
