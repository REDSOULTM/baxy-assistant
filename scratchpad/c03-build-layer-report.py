...
def read(folder,name):return json.loads((BASE/folder/name).read_text(encoding='utf-8-sig'))
def rows(folder):return [json.loads(x) for x in (BASE/folder/'replies.jsonl').read_text(encoding='utf-8').splitlines()]
parts=['''# Qué degrada a BAXY: prueba del modelo y de sus capas
...
''']
for prefix,label in [('astra-layer-ablation-v2','Conversación por capas'),('astra-compositor-ablation','Compositor y validadores'),('astra-layer-wrapper-fixed','Reproducción tras corregir prefijo')]:
    for model in ['registered','qwen-base']:
        folder=BASE/f'{prefix}-{model}'
        parts += [f'- {label}, {model}: '+link(folder/'RESPUESTAS.md','entradas, respuestas y borradores')+'; '+link(folder/'PREREG.json','prompts/condiciones/hashes')+'.\n']
parts+=['\n## Recursos medidos\n','| Corrida | Segundos | Pico GPU MiB |\n|---|---:|---:|']
for prefix in ['astra-layer-ablation-v2','astra-compositor-ablation','astra-layer-wrapper-fixed']:
    for model in ['registered','qwen-base']:
        name=f'{prefix}-{model}';r=read(name,'RESULT.json')
        parts+=[f"| {name} | {r['seconds']} | {r['gpuPeakMiB']:.2f} |"]
...
integrated=BASE/'astra-layer-product-registered'
if (integrated/'paired.json').exists():
    paired=read(integrated.name,'paired.json');r=read(integrated.name,'RESULT.json')
    parts+=['## Aplicación integrada con Granite registrado\n',
            f"{len(paired)}turnos; {sum(x.get('terminal')=='published_final' for x in paired)}finales publicados. Publicar no equivale a responder bien. PicoGPU{r['gpuPeakMiB']:.2f}MiB; registro intacto={r['registrationUnchanged']}.\n",
            ...
            link(integrated/'RESPUESTAS.md','Las respuestas integradas y su evaluación')+' · '+link(integrated/'sampling.jsonl','Prompts y respuestas de todos los roles LLM')+' · '+link(integrated/'RESULT.json','Cierre del proceso')+'\n']
else:parts+=['\nAplicación integrada todavía en curso; actualizar esta sección al recoger su proceso.\n']
parts += ['''## Actualización: correcciones verificadas y límites pendientes
...
''']
for name in ['astra-presentation-guards-fixed-registered', 'astra-presentation-guards-fixed-qwen-base', 'astra-presentation-product-qwen', 'astra-clarification-error-qwen', 'astra-unsupported-evidence-qwen-base', 'astra-unsupported-cause-qwen-base', 'astra-clarification-error-guarded-qwen']:
    folder=BASE/name
    parts += ['- '+name+': '+link(folder/'RESPUESTAS.md','entradas y respuestas literales')+'; '+link(folder/'RESULT.json','resultado del proceso')+'.']
report=BASE/'DIAGNOSTICO_POR_CAPAS_C03.md';report.write_text('\n'.join(parts),encoding='utf-8')
(BASE/'DIAGNOSTICO_POR_CAPAS_C03.manifest.json').write_text(json.dumps({'reportSha256':hashlib.sha256(report.read_bytes()).hexdigest(),'directResponses':294,'acceptance':False},indent=2),encoding='utf-8')
print(str(report))
