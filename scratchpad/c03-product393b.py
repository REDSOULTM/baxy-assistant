...
out = root/'artifacts/comprobaciones/C03/astra-stored-product393b'
private = Path(os.environ['LOCALAPPDATA'])/'BAXY/C03-stored-product393b-private'
...
prereg = {
    ...
    'method':'Six explicitly synthetic development turns through the real conductor in a new isolated private profile. Source393 adds explicit stored-name reads to the existing sealed private route; source383 preserves native conversation drafts under guards. No model/prompt/sampler change or injection. This is not fresh human acceptance, a graphical UI or physical voice measurement.',
...
for name in list(env):
    if name.startswith(('BAXY_MIND_','BAXY_VOICE_','BAXY_C03_','BAXY_FIELD_')) or name in {'PYTHONPATH','BAXY_DATA_DIR','BAXY_ASSET_DESCRIPTOR','BAXY_APP_TRACE'}:
        env.pop(name,None)
env.update(BAXY_MIND_LLM_GGUF=str(model), BAXY_MIND_PYTHONPATH=str(root/'scratchpad/c03-owner393b-hook')+os.pathsep+str(root/'src'), BAXY_VOICE_WAKE_ON_START='0',
...
command=['py','main.py','--conductor','--profile',str(private.parent/'C03-stored-profile393b'),
