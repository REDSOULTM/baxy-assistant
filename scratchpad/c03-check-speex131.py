...
out = root / 'artifacts/comprobaciones/C03/astra-speex131'
...
loop = read(base / 'C03-voice127-private/loopback.wav')
...
for name in ['physical_echo', 'near_only', 'physical_echo_plus_synthetic_near']:
    raw = read(base / f'C03-speex129-private/{name}-raw.wav')
    processed = read(base / f'C03-speex130-private/{name}-aec_residual.wav')
    for method, signal, delay in [('raw_with_existing_guard', raw, 0), ('aec_with_existing_guard', processed, 512)]:
        vad.reset()
        floor, count = .002, 0
        barge = []
        candidates = []
        for first in range(0, signal.size-511, 512):
        ...
        row = {'case': name, 'method': method, 'bargeSequencesSeconds': barge,
...
(out / 'LIMITS.json').write_text(json.dumps({
    'input': 'Existing captures127 and labelled synthetic near controls129; local CPU only',
