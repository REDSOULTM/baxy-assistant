...
OUT=ROOT/'artifacts/comprobaciones/C03/astra-runtime-qwen-registered/manual-ui'
...
(OUT/'PREREG.json').write_text(json.dumps({'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'command':['py','main.py'],'registrationSha256':digest,'method':'Normal real-window launch, registered model/KV/voice/Python defaults. Manual Windows skill input and inspection. First cancel the pre-existing memory recovery rather than authorizing erasure; then request clock/audio and ES/EN/mixed conversation. Development only, not fresh acceptance. Existing audit sinks only; resource sampler attaches exclusively to the newly launched exact Baxy.exe.'},indent=2),encoding='utf-8')
