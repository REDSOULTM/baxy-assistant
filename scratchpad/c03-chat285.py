...
out = root/'artifacts/comprobaciones/C03/astra-chat285'
...
runtime=Capture()
try:
    runtime.chat(row['body'],history,conversation_kind='knowledge',response_language='es')
except Captured:
    pass
...
prereg={'utc':datetime.now(timezone.utc).isoformat(),'request':row['body'],
    'method':'Native first payload of actual chat API, owner collection index5 and preceding visible history. 2x2: retain/remove only stage-tool instruction; retain/remove prior dialogue. Fifth identity-only baseline, same sampler/limits. No source changes, no tool execution. Captures first raw reply before publication guards.',
...
for name,payload in variants:
    ...
    try:
        ...
        print(json.dumps({'variant':name,'answer':result['choices'][0]['message'],'finish':result['choices'][0]['finish_reason']},ensure_ascii=False),flush=True)
