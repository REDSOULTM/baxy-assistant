"""Bounded physical product voice session: RAW, installed AEC3 two-view capture, VAD, Piper and ASR."""
...
OUT = ROOT / "artifacts/comprobaciones/C03/astra-voice259"
PRIVATE = Path(os.environ["LOCALAPPDATA"]) / "BAXY/C03-voice259-private"
...
def main():
    ...
    assert voice.EchoCanceller.__module__ == "baxy_mind.webrtc_aec"
    ...
    save(OUT / "PREREG.json", {
        ...
        "sourceFiles": {str(p): sha(ROOT / p) for p in [
            ...
            "src/baxy_mind/webrtc_aec.py", "src/baxy_mind/voice_output.py", "src/baxy_mind/piper_tts.py"]},
        "probeSha256": sha(helper), "lockSha256": sha(ROOT / "pylock.runtime-win-x64.toml"), "nativePackage": "pywebrtc-audio0.2.0+baxy.1",
    })
    capacity = 2000
    ...
    def observe_process(self, microphone, history):
        ...
        confirmation[i] = result.confirmation
    ...
    def observe_vad(self, frame):
        ...
        if count and threading.get_ident() == owner:
            observations[count - 1, 4 if self is engine._vad else 6] = result
        return result
    ...
    with _com_apartment():
        ...
        try:
            ...
            print("Physical RAW/installed two-view AEC3 direct session ready; four fixed TTS controls.", flush=True)
            ...
            np.savez(PRIVATE / "tap259.npz", microphone=near[:count], reference=reference[:count],
                clean=clean[:count], confirmation=confirmation[:count], observations=observations[:count])
            np.savez(PRIVATE / "generated259.npz", **{f"audio{i}": item[0] for i, item in enumerate(generated)})
            save(PRIVATE / "generated259.json", [item[1] for item in generated])
