...
PRIVATE = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-inventory-projection758-private'
SOURCE = PRIVATE.parent / 'C03-compose753-private'
OUT = ROOT / 'artifacts/comprobaciones/C03/INVENTORY_PROJECTION758'
...
plan = {'utc': datetime.now(timezone.utc).isoformat(), 'calls': 2, 'command': command,
        ...
        'change': 'Same identity projection in both arms; append the unchanged writing clause from754 only to the second system prompt', 'all_other_payload_fields_equal': True, 'window_count_preserved': len(windows), 'identity_sequence_preserved': True, 'inventory_metadata_preserved': True,
        ...
        'scope': '757 projection completed20 entries but omitted partial-page scope. This paired probe holds that projection constant and adds the unchanged754 page-writing clause only to the second arm. Inspect all identities and page scope under fixed256output tokens. Offline observation15s does not change any product deadline or the4s success reference. Server cwd remains binary folder rather than product sidecar folder; not a complete product timing replica or adoption.',
