"""Capture the actual HTTP payload after runtime template serialization."""
import io
import json
import os
...
if destination := os.environ.get('C03_WIRE_TRACE_DIR'):
    ...
    _c03_original_post = llm.post_chat_completion

    def _c03_traced_post(payload, *args, **kwargs):
        started = time.monotonic()
        row = {'payload': payload}
        try:
            result = _c03_original_post(payload, *args, **kwargs)
            row['response'] = result
            return result
    ...
    llm.post_chat_completion = _c03_traced_post
