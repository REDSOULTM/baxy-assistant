"""Evaluate all frozen236 outputs through actual product segmentation and ASR."""
...
OUT = BASE / "astra-product237"
PRIVATE = Path(os.environ["LOCALAPPDATA"]) / "BAXY/C03-product237-private"
...
assert read(BASE / "astra-dsp236/COMPLETE.json")["outputs"] == 20
inputs = read(BASE / "astra-dsp236/RESULTS.json")
assert importlib.metadata.version("sherpa-onnx") == "1.13.4+baxy.2"
assert sha(ROOT / "src/baxy_mind/voice.py") == read(BASE / "astra-source230-snapshot/FILES.json")["src/baxy_mind/voice.py"]
OUT.mkdir(exist_ok=False)
PRIVATE.mkdir(exist_ok=False)
save(OUT / "PREREG.json", {
    "utc": datetime.now(timezone.utc).isoformat(), "scriptSha256": sha(Path(__file__)),
    "method": "All20 full cleaned outputs236 through actual source230 capture segmentation and fresh-reset SileroVad. Use frozen speaking mask and actual backend-returned-pair guard. Same installed baxy.2 recognizer/greedy acknowledgement, no expected text hints, gains or thresholds. Decode every queued segment; additionally decode all16 fixed human windows to separate waveform recognition from endpointing. No device/ducker/TTS effects.",
    "criterion": "Speex must reproduce prior231 human segments/cancellations and233 physical232 failure, with221 no segments. Compare DTLN128 on those same timelines for both human content and echo. Report every literal, not merely zero interruptions.",
...
for row in inputs:
    ...
    if row["backend"] == "speex":
        if row["human"] is not None:
            condition = "raw_speex" if row["case"].endswith("raw") else "near_speex"
            prior = next(r for r in read(BASE / "astra-human231/RESULTS.json") if r["human"] == row["human"] and r["condition"] == condition)["variants"][1]
            assert segments == prior["segments"], row["case"]
            assert cancellations == prior["cancellations"], row["case"]
        elif row["case"] == "echo221":
            assert segments == [] and cancellations == []
        else:
            prior = read(BASE / "astra-replay233/RESULTS.json")
            assert cancellations == prior["cancelFrames"]
            assert [(s["firstSample"] // 512, s["lastSample"] // 512, s["text"]) for s in segments] == [
                (s["firstFrame"], s["endFrameExclusive"], s["text"]) for s in prior["segments"]]
...
save(OUT / "COMPLETE.json", {"timelines": len(results), "fixedHumanWindows": 16,
    "segments": sum(len(r["segments"]) for r in results), "speexPriorParities": 10, "exitCode": 0})
