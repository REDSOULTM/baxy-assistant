...
if os.environ.get('BAXY_C03_EARLY_READ412') == 'early-read':
    _catalog412 = json.loads((Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-read-recovery413-private/catalog.json').read_text(encoding='utf-8'))
    ...
    def _early_read412(self, text, candidates, *, history=None, evidence=None):
        ...
        if (getattr(self, '_native_tool_policy_enabled', False)
            ...
            second = _prior_decide412(self, text, candidates[:4], history=history, evidence=evidence)
