...
OUT = BASE / 'CLOCK_VALUES784'
PRIVATE = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-clock-values784-private'
...
validation = read(BASE / 'NAMED_CLOCK783/VALIDATION.json')
...
pins = read(BASE / 'NAMED_CLOCK783/SOURCE_PINS.json')
...
prereg = {'utc': utc(), 'head': subprocess.check_output(['git', 'rev-parse', 'HEAD'], text=True).strip(),
    'candidate': 'NAMED_CLOCK783, validated but not adopted yet', 'cases': len(cases),
    ...
    'template_source': 'Identical50 cases, IDs, observed values, questions and criteria from780; only validator source783 changed. Original situation came from779.',
