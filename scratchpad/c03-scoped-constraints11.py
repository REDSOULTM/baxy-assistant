...
out = ROOT / 'artifacts/comprobaciones/C03/astra-scoped-constraints11'
...
files = ['tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-scoped-constraints11.turns.jsonl',
         'scratchpad/c03-scoped-constraints11.py']
files.append('src/Baxy.App/UserMessagePolicy.cs')
files.append('src/Baxy.App/bin/Release/net10.0-windows10.0.19041.0/Baxy.dll')
files.append('artifacts/comprobaciones/C03/astra-scoped-constraints11/CASES.json')
...
prereg={'kind':'qwen-base-integrated-diagnostic-not-acceptance','utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
 'method':'Eleven consumed definition/follow-up and labeled synthetic scope/recovery controls. Native AUTO receives the same literal current request and bounded history as reference data. Catalog recovery uses contextual AUTO rather than the old weak per-operation identity classifier. The App shortcut for negative substrings is removed, so complete requests cross turn.decide. Compound conservation counts positive effects; a negative clause does not globally ban a separate positive clause. Standalone negatives, explicit no-tools, correction, device and missing-effect guards remain. Same eleven ordered inputs as reference-context11. Registered runtime, no model/sampling/KV/PYTHONPATH overrides; no intended mutations, fault injection, UI or fresh acceptance.',
...
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-scoped-constraints11'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
        ...
        while process.poll() is None:
            if not released:
                ...
                if time.monotonic()-started > 50:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-scoped-constraints11.turns.jsonl').read_bytes()
