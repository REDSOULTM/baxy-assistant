...
out=ROOT/f'artifacts/comprobaciones/C03/astra-presentation-guards-fixed-{key}';assert not out.exists();out.mkdir()
def sha(p):
...
cases=[c for c in cases if c['route'] in {'welcome','confirmation','progress'}]
for c in cases:
...
prereg={'kind':'postfix-presentation-guards-nine-case-diagnostic-not-acceptance','cases':cases,'stages':['guarded'],
