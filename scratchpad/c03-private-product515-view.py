...
out = root/'artifacts/comprobaciones/C03/astra-private-product515-view'
private = Path(os.environ['LOCALAPPDATA'])/'BAXY/C03-private-product515-view-private'
...
assert not (private.parent/'C03-private-profile515-view').exists()
prereg = {
    'utc':datetime.now(timezone.utc).isoformat(),
    'production_verification': '515 view: source512 actual product, registered runtime, same9cases; see astra-memory-view515/PREREG.json. Only treatment mutates the writer view, never replies or source.',
...
for name in list(env):
    if name.startswith(('BAXY_MIND_','BAXY_VOICE_','BAXY_C03_','BAXY_FIELD_')) or name in {'PYTHONPATH','BAXY_DATA_DIR','BAXY_ASSET_DESCRIPTOR','BAXY_APP_TRACE'}:
        env.pop(name,None)
env.update(BAXY_MIND_LLM_GGUF=str(model), BAXY_MIND_LLAMA_SERVER=config['llama_server'], BAXY_MIND_PYTHONPATH=str(root/'scratchpad/c03-owner515-view-hook')+os.pathsep+str(root/'src'), BAXY_VOICE_WAKE_ON_START='0',
...
command=['py','main.py','--conductor','--profile',str(private.parent/'C03-private-profile515-view'),
