...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen2507-corpus-warm'
...
files = ['tests/data/turn_evidence_runtime.v1.jsonl', 'src/baxy_mind/data/turn_evidence_abstention_policy.v1.json', 'src/baxy_mind/planner.py', 'src/Baxy.Providers.Windows/Windows/WindowsWindowControlProvider.cs', 'src/Baxy.Providers.Windows/Windows/WindowControlContracts.cs', 'src/Baxy.Core/Operations/WindowControlHandlers.cs', 'src/Baxy.Kernel/Operations/ProductCatalog.cs', 'src/Baxy.Core/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish/baxy-core.exe', 'src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen2507-corpus-warm.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen2507-corpus-warm/profile/sitecustomize.py',
         'scratchpad/c03-qwen2507-corpus-warm.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Missing calibrated runtime corpus caused TurnEvidenceService policy incompatibility. Inherit the exact corpus and verified existing vector cache from prior BAXY, with unchanged calibration, model and sampler. Retain original boolean audio projection after failed categorical experiment. Observe service/catalog readiness in the actual product and all 21 existing development replies.',
...
command=[str(app),'--conductor','--profile='+str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen2507-corpus-warm'), '--timeout-ms=120000','--commit='+commit,'--capture='+str(out)]
        ...
        while process.poll() is None:
            if not released:
                ...
                if {'planner_catalog','skill_registry'} <= ready_components or time.monotonic()-started > 150:
                    commands=(ROOT/'artifacts/comprobaciones/C03/astra-qwen2507-corpus-warm.turns.jsonl').read_bytes()
