...
OUT = BASE / "astra-dsp238"
PRIVATE_ROOT = Path(os.environ["LOCALAPPDATA"]) / "BAXY"
PRIVATE = PRIVATE_ROOT / "C03-dsp238-private"
...
save(OUT / "PREREG.json", {
    ...
    "method": "Ten identical236 timelines with DTLN512 through the same adapter182; compare to frozen Speex236/237 and DTLN128236/237 without rerunning them. Exact physical221/232 plus eight constructed human RAW213 timelines, no new gain/delay/model search. Save full cleaned frames and actual returned-pair guard results. Speex baselines already proved exact in236 and remain frozen; this run computes only512. No ASR until outputs frozen.",
    "newEvidenceJustifyingComparison": "DTLN128236/237 removes both echo232 cancellations but drops human clauses/words. Evaluate the existing author challenge model512 on exactly the same inputs, changing capacity only; no gain or threshold sweep.",
    "next": "All10 new512 outputs through source230 real segmentation/Silero and installed baxy.2 without hints; compare full literals and interruption decisions. Four human fixed windows additionally isolate segmentation from ASR if needed.",
    ...
    "dtlnDownloads": read(BASE / "astra-dtln180/DOWNLOADS.json"),
    "cases": [{k: v for k, v in case.items() if not isinstance(v, np.ndarray)} for case in cases],
})
results = []
for case in cases:
    for name, factory in [("dtln512", lambda: Dtln(512))]:
        engine = factory()
        frames = len(case["mic"])
...
save(OUT / "COMPLETE.json", {"cases": len(cases), "outputs": len(results), "speexExactParities": 0, "baseline236Reused": True, "exitCode": 0})
