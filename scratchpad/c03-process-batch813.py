"""Frozen 50-case process-read category through the registered product with sealed process counts812.
...
PRIVATE = Path(os.environ['LOCALAPPDATA']) / 'BAXY/C03-process-batch813-private'
PROFILE = PRIVATE.parent / 'C03-process-profile813'
...
plan = read(plan_path)
# Full804 is the baseline for cumulative Python802 plus C#804, not Full812.
# Python-only counts812 requires its own owners and Fast before product measurement.
validation_root = ROOT / 'artifacts/comprobaciones/C03/PROCESS_COUNTS812'
...
pins812 = read(validation_root / 'SOURCE_PINS.json')
assert len(pins812) == 31, 'counts812 source pin set'
assert all(sha(ROOT / path) == digest for path, digest in pins812.items())
...
prereg = {
    'utc': utc(), 'method': 'Registered50 process-read category with sealed candidate812; nine historical requests and41 variants. Build prepared before sealing DLL; model selection is closed.',
    ...
    'validation': 'Full804 retry1 exit0 is the baseline for Python802 plus C#804, verified against SOURCE_PINS804; this is not Full812. PROCESS_COUNTS812 owners and Fast exit0 are required before launch. Original Full804 seal failure preserved. New real product confirmation; final cumulative Full/adoption remains governed by the goal. No native model comparison or automated coverage credit.',
    ...
    'source812_pins_sha256': sha(validation_root / 'SOURCE_PINS.json'),
    ...
    'source812_pins': pins812, 'sources': sources, 'runner_sha256': sha(__file__),
...
outcome = {'utc': utc(), 'exit_code': code, 'manifest_unchanged': sha(manifest) == prereg['manifest_sha256'],
           ...
           'source812_unchanged': all(sha(ROOT / path) == digest for path, digest in pins812.items()),
