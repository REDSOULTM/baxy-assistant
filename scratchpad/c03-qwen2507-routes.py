...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen2507-routes'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen2507-routes.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen2507-routes/profile/sitecustomize.py',
         'scratchpad/c03-qwen2507-routes.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'Broader inherited 18-turn route panel with the existing Instruct-2507 candidate, unchanged source restored after ineffective semantic language fallback. Tests actual status, multi-operation, clarification, unsupported request, mixed language and file confirmation/cancellation. Development only.',
          'expected': 'Useful faithful responses across routes, no invented observations or unintended effects; synthetic deletion fixture must remain after cancellation.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen2507-routes'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen2507-routes.turns.jsonl')]
