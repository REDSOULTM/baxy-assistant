...
prereg = {'method': 'Native prototype: same six packets82 and sampler/model; replace only state=in progress '
...
try:
    ...
    for case in cases:
        for variant in ('request_interpretation',):
            payload = copy.deepcopy(case['payload'])
            if variant == 'request_interpretation':
