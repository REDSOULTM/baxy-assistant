...
from webrtc_linear250 import EchoCanceller

BASE = ROOT / "artifacts/comprobaciones/C03"
OUT = BASE / "astra-linear250"
PRIVATE_ROOT = Path(os.environ["LOCALAPPDATA"]) / "BAXY"
PRIVATE = PRIVATE_ROOT / "C03-linear250-private"
...
save("DSP_PREREG.json", {"scriptSha256": sha(Path(__file__)), "adapterSha256": sha(ROOT / "scratchpad/webrtc_linear250.py"), "cases": cases, "method": "Exactly248mic/ref and speaking masks. Instrumented176 final must equal248 on every block; save linear stage only, same bridge128 plus verified native64 gives raw pair192. No source edits, gain/config variants or ASR until11outputs frozen.", "next": "Real source242 segmentation/Silero and baxy.2 final ASR for11timelines plus8fixedhumanwindows; compare239 literal outputs; no echo-only acceptance."})
final_baseline = {r["case"]: r for r in read(BASE / "astra-webrtc248/RESULTS.json")}
results = []
for case in cases:
    ...
    baseline_row = final_baseline[case["id"]]
    assert sha(Path(baseline_row["privatePath"])) == baseline_row["sha256"]
    baseline_final = np.load(baseline_row["privatePath"])["clean"]
    ...
    try:
        for i in range(len(mic)):
            ...
            assert np.array_equal(engine.last_final, baseline_final[i]), (case["id"], i, "instrumented final differs248")
            clean[i] = frame
            guards[i] = _looks_like_echo(near, history)
    finally:
        engine.close()
    target = PRIVATE / f'{case["id"]}-aec3-linear.npz'
    np.savez(target, clean=clean, guards=guards, speaking=speaking, dspMs=elapsed)
    row = {"case": case["id"], "backend": "aec3-linear", "human": case["human"], "referenceText": case["referenceText"], "frames": len(mic), "engineSha256": engine.sha256, "privatePath": str(target), "sha256": sha(target), "dspMs": {"mean": float(np.mean(elapsed)), "p99": float(np.quantile(elapsed, .99)), "max": max(elapsed)}}
