...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen2507-product'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen2507-product.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen2507-product/profile/sitecustomize.py',
         'scratchpad/c03-qwen2507-product.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'The inherited non-thinking 4B Instruct-2507 checkpoint can fit on GPU and may answer within existing deadlines unlike the 8B partial-CPU candidate. R82 measured 178/180 contracts with a repeated arguments-06 failure; that failure remains a required regression before any promotion. This is the first integrated C03 comparison of that checkpoint on this repaired source, with the same six controls, sampling and budgets, using ngl99/q8 and the 4096 MiB process-tree stop.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen2507-product'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen2507-product.turns.jsonl')]
