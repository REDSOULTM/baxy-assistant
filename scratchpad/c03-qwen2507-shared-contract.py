...
out = ROOT / 'artifacts/comprobaciones/C03/astra-qwen2507-shared-contract'
...
files = ['src/baxy_mind/llm.py', 'src/baxy_mind/__main__.py',
         ...
         'artifacts/comprobaciones/C03/astra-qwen2507-shared-contract.turns.jsonl',
         'artifacts/comprobaciones/C03/astra-qwen2507-shared-contract/profile/sitecustomize.py',
         'scratchpad/c03-qwen2507-shared-contract.py']
manifest_hash = sha(manifest_path)
prereg = {'kind': 'integrated-development-diagnostic-not-promotion',
          'utc': datetime.datetime.now(datetime.timezone.utc).isoformat(),
          'hypothesis': 'One fair recheck of existing Qwen3-4B-Instruct-2507 after shared dialogue-state, knowledge, noun, uncertainty, confirmation and ordered-result repairs. Same21 requests and .7/.8/k20 sampler, original profile without system packing. No new download, prompt variation or verifier change.',
          'expected': 'Compare usefulness, faithfulness and language against current8B; not publication count. Empty own C03WindowFixture PID27028 remains open from prior run, where only window.resolve occurred after second close request. No user documents in this fixture; authorize only this named test process. Development only.',
...
command = ['powershell.exe', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
           ...
           str(Path(env['LOCALAPPDATA'])/'BAXY/comprobaciones-c03-qwen2507-shared-contract'),
           '-Capture', str(out), '-TurnsFile',
           str(ROOT/'artifacts/comprobaciones/C03/astra-qwen2507-shared-contract.turns.jsonl')]
