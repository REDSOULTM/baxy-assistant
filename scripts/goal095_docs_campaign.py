"""Single-campaign cursor for Goal 09.5.2 docs checkpoints.

Wraps the 09.5.1 v1 queue in place: 25 lots become recoverable units of
...
CAMPAIGN_ID = "docs"
KIND = "docs"
OWNER_PROMPT = "documentacion/sprints/09.5.2_LEER_DOCUMENTACION_LOTE.md"
CAMPAIGN_CONTINUE = "campaign:docs"
...
def campaign_path(repo: Path) -> Path:
    return repo / "artifacts" / "goal095" / "campaigns" / "docs.json"
...
def next_human_prompt(queue_ledger: dict[str, Any]) -> str | None:
    docs = counts_for(queue_ledger)
    if docs["pending"] or docs["claimed"]:
        return None
    code_open = any(
    ...
    return CODE_PROMPT if code_open else EVIDENCE_PROMPT
...
def write_campaign(
    ...
    payload = {
        ...
        "migrated_from": "artifacts/goal095/queue v1; 25 docs lots",
...
def _rewrite_docs001_next_prompt(repo: Path) -> None:
    path = repo / "artifacts" / "goal095" / "ledger" / "docs-001-carter.json"
    if not path.is_file():
        return
    ledger = load_json(path)
    if ledger.get("next_prompt") == OWNER_PROMPT:
        ledger["next_prompt"] = CAMPAIGN_CONTINUE
...
def wrap_v1(repo: Path) -> dict[str, Any]:
    ...
    _rewrite_docs001_next_prompt(repo)
    claimed = [
        item["batch_id"]
        for item in queue_ledger.get("batches") or []
        if item.get("kind") == KIND and item.get("status") == "claimed"
