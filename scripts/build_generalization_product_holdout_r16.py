"""Build and preregister BAXY's execution-inert R16 blind product holdout.

R16 independently confirms the addressed-envelope and complete message/task
composition repairs found after R15. It preserves the 700-case distribution
while changing entities, stacked wrappers, and all 94 generic composition
orders. No normalized request may overlap R7-R15.
"""
...
from scripts import build_generalization_product_holdout_r15 as r15  # noqa: E402
...
OUTPUT = REPO / "artifacts/holdout/generalization_product_holdout_v16.jsonl"
PREREGISTRATION = (
    REPO / "artifacts/holdout/generalization_product_holdout_v16.preregistration.json"
)
MIND_OUTPUT = REPO / "artifacts/holdout/generalization_product_holdout_r16_mind.json"
MIND_AUDIT = REPO / "artifacts/holdout/generalization_product_holdout_r16_mind.raw.jsonl"
MEMORY_TRX = REPO / "artifacts/holdout/generalization_product_holdout_r16_memory.trx"
PRODUCT_OUTPUT = REPO / "artifacts/holdout/generalization_product_holdout_r16_product.json"
PRIOR_CORPORA = (*r15.PRIOR_CORPORA, r15.OUTPUT)
POLICY_SOURCES = r15.POLICY_SOURCES
MEASUREMENT_SOURCES = (
    REPO / "experiments/mind_router_spike/probe_generalization_product_holdout_r16.py",
    REPO / "experiments/mind_router_spike/analyze_generalization_product_holdout_r16.py",
...
BUILDER_DEPENDENCIES = (
    *r15.BUILDER_DEPENDENCIES,
    REPO / "scripts/build_generalization_product_holdout_r15.py",
)
REFERENCES = r15.REFERENCES


_REPLACEMENTS = (
    ("Blender", "Obsidian"),
    ("7-Zip", "HandBrake"),
    ("Carpeta Aurora", "Carpeta Bruma"),
    ("Cedar Folder", "Birch Folder"),
    ("pases del acuario", "pases del observatorio"),
    ("aquarium passes", "observatory passes"),
    ("Paloma", "Nadia"),
    ("Iñaki", "Óscar"),
    ("Dakota", "Sage"),
    ("Hayden", "Ellis"),
    ("Finley", "Remy"),
    ("Cameron", "Kerry"),
    ("The Bear", "Slow Horses"),
    ("Shogun", "Ripley"),
    ("mariposas monarca", "ballenas azules"),
    ("monarch butterflies", "blue whales"),
    ("cafeteras portátiles de viaje", "linternas compactas de campamento"),
    ("portable travel espresso makers", "compact camping lanterns"),
    (
    ...
    ("ramen", "curry"),
    ("noche nevada", "amanecer con niebla"),
    ("snowy night", "foggy dawn"),
    ("Clara", "Elena"),
    ("Mauricio", "Rodrigo"),
    ("Florencia", "Daniela"),
    ("Sebastián", "Martín"),
...
def _request_utterance(value: Utterance, index: int) -> Utterance:
    prefixes = {
        "es": (
            "Oye, por favor, cuando tengas un minuto, ",
            "Oye, a ver, una solicitud rápida: ",
        ),
        "en": (
            "One thing: please, when you have a minute, ",
            "Please, one small request: ",
        ),
        "spanglish": (
            "Oye, porfa, when you have a minute, please ",
            "One thing, porfa: one small request: ",
        ),
    }
...
def _conversation_utterance(value: Utterance, index: int) -> Utterance:
    markers = {
        "es": ("A ver, una pregunta rápida: ", "Oye, una consulta pequeña: "),
        "en": ("Please, one quick question: ", "One thing: a small question: "),
        "spanglish": ("Oye, una pregunta rápida: ", "One thing: a small question: "),
    }
    return Utterance(
        value.language,
...
def _generated_compositions() -> tuple[builder.Composition, ...]:
    ...
    for index in range(94):
        ...
        for attempt in range(len(operations) * len(steps)):
            step = steps[(index * 23 + attempt // len(operations) + 17) % len(steps)]
            cursor = (index * 41 + attempt * 29 + 31) % len(operations)
            candidate: list[str] = []
            ...
            if sequence not in prior_sequences and sequence not in current:
                ...
                break
        if selected is None:
            raise RuntimeError(f"R16 could not derive novel composition {index}")
        parts = [COMPOSITION_PARTS[item][language] for item in selected]
        base = r7._composition_text(language, parts, index + 409)
        rows.append(
            builder.Composition(
                selected,
...
SPECIAL_COMPOSITIONS = (
    builder.Composition(
        ...
        _request_utterance(
            Utterance(
                "en",
                "Capture the screen and read every visible word from that exact image",
            ),
            0,
        ),
    ),
    builder.Composition(
        ...
        _request_utterance(
            Utterance(
                "spanglish",
                "Capture el desktop y describe everything visible in that exact image",
            ),
            1,
        ),
    ),
    builder.Composition(
        ...
        _request_utterance(
            Utterance(
                "es",
                "Captura el escritorio, describe esa imagen y lee cada palabra visible de la misma captura",
            ),
            2,
        ),
    ),
    builder.Composition(
        ...
        _request_utterance(
            Utterance(
                "es",
                "Dile a Uma por WhatsApp que llego en cincuenta minutos y luego lista mis tareas abiertas",
            ),
            3,
        ),
    ),
    ...
    builder.Composition(
        ...
        _request_utterance(
            Utterance(
                "es",
                "Enséñame los eventos de mañana, captura esa vista, describe lo que aparece y lee el texto del portapapeles",
            ),
            5,
        ),
    ),
...
def configure() -> None:
    ...
    builder.CAMPAIGN = "r16"
    builder.BUILDER_SOURCE = Path(__file__).resolve()
    builder.ROW_SCHEMA = "baxy.generalization-product-holdout.v16"
    builder.PREREGISTRATION_SCHEMA = "baxy.generalization-product-preregistration.v16"
    builder.EXPECTED_POPULATION = 700
    builder.MINIMUM_CASES = 700
    builder.METHOD_DESCRIPTION = (
        "r7_distribution_with_fresh_entities_new_bounded_stacked_envelopes_"
        "che_addressing_and_generic_orders_absent_from_r7_r15"
    )
