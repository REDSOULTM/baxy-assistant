...
| Pieza | Borde | Qué decide el cambio | Elegido hoy | Goal | Fecha |
|---|---|---|---|---|---|
| **Wake word** | Proceso | Corpus **aislado** «Baxy»/«Baxy.» de voces holdout (Sabina es-MX + Zira en-US, rates −3/0/+3): FRR y máximo de score. FAR: Poisson unilateral 95 % sobre horas de película/serie que no le hablan a BAXY; el umbral **no se retoca** al abrir. Listón de promoción del umbral: FRR ≤5 %, FAR superior ≤0,1/h. Procedimiento: `scripts/evaluate_wake_corpus.py` y `scripts/measure_goal09_far.py` | `baxy.onnx` SHA `9b1ae5db…`, umbral **0,5**, manifiesto `baxy-wakeword-v1.json` SHA `fefb9517…` (R281). Holdout 2026-08-24: **12/12** (FRR 0). FAR **2,06 h** de series (Impractical Jokers): **6 disparos**, 2,91/h, superior 5,74/h. **No se promociona el umbral** ni `calibration.approved`. Identidad exige escucha permanente: `wake_on_start` true vía costura `BAXY_VOICE_WAKE_ALLOW_UNCALIBRATED=1`; el interruptor visible (`VoiceListenCommand`) apaga. El hash se publica aunque la escucha esté apagada. `artifacts/goal09/wake_far.json` | 01 → 09 | 2026-08-24 |
