...
| Planificador de misiones | Contrato | Un plan es una lista de operaciones del catálogo: cada paso se verifica antes del siguiente; cero huérfanos; un compuesto incompleto no corre como subconjunto silencioso. Campaña ciega `experiments/mind_router_spike/run_compound_execution_current_tree_r6.py` (disjunta de R4/R5) más `tests/test_compound_missions.py` sobre el planner y el veto de conservación de producción. El candidato se rechaza si baja del 90 % de misiones completas, si deja un paso sin verificar o si ejecuta un subconjunto y declara éxito | Lista de pasos en `planner.py` + veto `apply_compound_effect_conservation_veto`. Sin motor de workflows. La cláusula dependiente conserva la cabeza gobernante (reparo R4). Medido en unidad: abrir+ir, halt de «envía un mensaje», `termina con`. Holdout R6 abierto una vez: **6/6 misiones, 22/22 pasos, 0 huérfanos, 0 ambiguos**. Sello `1ad6ecd2…` | 07 | 2026-08-23 |

### La pila de voz

| Pieza | Borde | Qué decide el cambio | Elegido hoy | Goal | Fecha |
|---|---|---|---|---|---|
| **Wake word** | Proceso | Corpus **aislado** «Baxy»/«Baxy.» de voces holdout (Sabina es-MX + Zira en-US, rates −3/0/+3): FRR y máximo de score. FAR: Poisson unilateral 95 % sobre horas de película/serie que no le hablan a BAXY; el umbral **no se retoca** al abrir. Listón de promoción del umbral: FRR ≤5 %, FAR superior ≤0,1/h. Procedimiento: `scripts/evaluate_wake_corpus.py` y `scripts/measure_goal09_far.py` | `baxy.onnx` SHA `9b1ae5db…`, umbral **0,5**, manifiesto `baxy-wakeword-v1.json` SHA `fefb9517…` (R281). Holdout 2026-08-24: **12/12** (FRR 0). FAR **2,00 h** de series (Impractical Jokers), corrida terminada (`partial: false`): **5 disparos**, 2,50/h, superior 5,26/h. **No se promociona el umbral** ni `calibration.approved`. Identidad exige escucha permanente: `wake_on_start` true vía costura `BAXY_VOICE_WAKE_ALLOW_UNCALIBRATED=1`; el interruptor visible (`VoiceListenCommand`) apaga. El hash se publica aunque la escucha esté apagada. `artifacts/goal09/wake_far.json` | 01 → 09 | 2026-08-24 |
| **STT** | Proceso | WER / aciertos por locución en es, en y spanglish sobre WAV reales o SAPI holdout, RTF, y que una transcripción vacía o dudosa **pregunta** (`_transcript_is_doubtful`) en vez de ejecutar. CPU. `VoiceEngine.transcribe_pcm` | Parakeet TDT 0.6b v3 int8, SHA agregado `5a70e086…`. Holdout SAPI 2026-08-24 **3/3** (`artifacts/goal09/stt_holdout.json`): «open notepad please» → `Open notepad please`; «buenas noches» → `Buenas noches.`; codeswitch «abre notepad please» → `Abre notepad, please.`. p50 decode **0,20 s**. EOU live (ingest habla + silencio → `recognized`) p50 **0,21 s**, máx 0,31 s. «BAXY» aislado y «Spotify» propio a menudo fallan; dudo no ejecuta. Qwen3-ASR GGUF en disco **no** se adoptó | 01 → 09 | 2026-08-24 |
...
| Automatización de apps (UIA) | Contrato | Cascada UIA → OCR → visión sobre la ventana en primer plano, **sin nombres de app**. Pruebas `VisibleClickCascade*` en `ExternalAdaptersTests.cs`: si UIA acierta no se llama OCR/visión; si UIA no expone, OCR; visión sólo al final. Más la misión viva «Abre Steam y ve a la biblioteca». Un candidato se mide con esos dos números y con el postread (seleccionado, desaparecido, superficie cambiada o ToggleState) | `DesktopClickVisible.ps1` busca por **NameProperty** (no recorre todo el árbol), reintenta 400 ms si la ventana enfocada no es el host de replay, y si no hay match busca otras ventanas visibles excepto testhost/Baxy. Cero Steam/Spotify/Discord en la cascada. Postread acepta ToggleState. Un stdout vacío ya no se marca como efecto ambiguo. 2026-08-26 | 01 → 10 | 2026-08-26 |
| OCR | Proceso | El escalón OCR de Click X: fracción de controles que UIA no expone y OCR localiza de forma única, más falsos positivos de etiqueta corta. `ocr.read` de catálogo sigue siendo otra pieza (`CaptureVisionAdapter`). Gana el motor que lee español sin `spa.traineddata` de Tesseract | **Windows.Media.Ocr** (`WindowsVisibleOcrLocator.cs`) con idiomas del perfil de usuario. Tesseract 5.4.0 sigue en `ocr.read` y **sigue sin `spa.traineddata`**; no es el escalón de Click X | 01 → 07 | 2026-08-23 |
| Motor de visión | Proceso | Último recurso de Click X: se invoca sólo si UIA y OCR fallan (la prueba de cascada lo cuenta). Un proveedor que reciba más de 2 llamadas por misión o que afirme un click sin postread se rechaza. Si no hay endpoint, el escalón se salta y el fallo es honesto | HTTPS OpenAI-compatible (`BAXY_VISION_ENDPOINT` + `BAXY_VISION_MODEL`) en `WindowsVisibleVisionLocator.cs`. En esta máquina no está configurado; Steam no lo necesitó | 07 | 2026-08-23 |

### El producto alrededor

| Pieza | Borde | Qué decide el cambio | Elegido hoy | Goal | Fecha |
|---|---|---|---|---|---|
| Almacén de memoria | Contrato | Tres números sobre el mismo data root: (1) `Save` con la memoria apagada falla cerrado (`MemoryDisabledException`) y no escribe; (2) `Configure(enabled=true)` es durable e idempotente — un segundo `Configure` con el mismo id es replay, no un segundo alta; (3) el panel y las operaciones `memory.save` / `correct` / `forget` ven, editan y borran el mismo registro. `LocalMemoryStoreTests` + `MemoryAppFlowTests`. Un candidato que recuerde con la memoria apagada, o que no sobreviva un reinicio del proceso, se rechaza | `LocalMemoryStore` JSON + sobre `memory.store.v1` (Windows DPAPI). Apagada por defecto. Goal 10 r48: «acordate que mi color favorito es el azul» → «Guardé el dato en la memoria local.» con `memory.save` verificado | 10 | 2026-08-26 |
...
| Capa visual (bandeja / WebView) | Contrato | Tres observaciones de producto vivo, no de captura: (1) X oculta a la bandeja, no cierra el proceso; (2) segunda instancia muestra la primera, no abre otra; (3) el input `message input` queda `enabled` y acepta un turno. Sonda `scripts/goal10_launch_probe.ps1` + UIA `goal10_uia_dump.ps1`. Un candidato que mate el proceso al cerrar la ventana, o que deje el input muerto, se rechaza | WinForms `NotifyIcon` con `UseWindowsForms=true` (sin `System.Drawing` global: CS0104). Field/WebView es chrome de depuración, no la ventana que lee una persona. Medido 2026-08-24: UIA 18 nodos, input habilitado, turno «qué hora es» 1,06 s | 10 | 2026-08-26 |
| Instalación y arranque | Contrato | Tres números de arranque en frío, no soak: (1) `HKCU\...\Run\BAXY` apunta a `"Baxy.exe" --tray` y sobrevive un reboot de Windows; (2) `baxy-core.exe` se queda vivo; (3) la mente publica cliente listo y el primer turno produce texto. Sonda `scripts/goal10_launch_probe.ps1`. Un candidato que no reaparezca tras reboot, o que deje `startup.ready` con la mente muerta, se rechaza | Arranque por `HKCU` Run, no servicio. `Baxy.Setup` (13 k líneas) no es la vía de uso diario. Medido 2026-08-24: reboot 10:08, `Baxy.exe --tray` 10:09, handshake 10:10 | 10 | 2026-08-26 |
