...
| **Reconocedor determinista** | Contrato | **Cuatro números, y el cuarto es el que decide si vale la pena**: cuántas filas reclama, cuántas acierta, cuántas peticiones fuera de catálogo reclama por error, y **cuántas de esas mismas filas sirve el camino por modelo cuando se le quita**. Sin el cuarto no se sabe si 12.310 líneas de reglas están aportando o estorbando. La tercera sigue siendo la que veta: un reconocedor que reclama algo que no sabe hacer produce un efecto no pedido | 12.310 líneas de reglas en `effect_intent.py`, **con declinación desde el goal 03B**: antes de publicar lo que resolvió, el verificador de identidad de operación confirma que es el efecto pedido, y cuando no lo confirma la fila cae al camino por modelo. Su reparto por causa baja de **12 a 7 filas de 124**, y su p50 sube de 0,007 s a **0,16 s** por ese decodificado. Sigue ganando a su alternativa: enviar sus 38 filas enteras al modelo daba 24 contra 26. Sus errores restantes son desvíos silenciosos a una hermana: «dejame el sonido a la mitad» resuelve `audio.status` en vez de `audio.volume` | 03 → 03B | 2026-08-19 |
...
| **Puerta de alcance (abstención)** | Contrato | Dos números que se leen juntos y nunca uno solo: **cuántas propuestas legítimas conserva** y **cuántas peticiones fuera de catálogo rechaza**, sobre el corpus fresco. Desde el goal 03B hay un tercero que cambia cómo se leen los dos primeros: **qué hace con lo que retira**. Retirar ya no es negar la capacidad — es retenerla hasta que la persona confirme la invocación exacta — así que el sobreveto se paga en preguntas, no en respuestas perdidas. Un candidato que suba el rechazo bajando la conservación se sigue rechazando | La puerta léxica curada de `effect_intent.py`, con **veredicto ternario**: cuando refuta y el verificador de identidad reconoce el efecto, la operación se ofrece para confirmar. Medida con la puerta y sin ella sobre el corpus fresco: comprensión **85/124 contra 84/124** —o sea, quitarla ya no compra nada—, abstención honesta **28/36 contra 7/36** y decisiones que ejecutarían un efecto no pedido **4/36 contra 23/36**. En el goal 03 el trato era 21 puntos contra 20 efectos; ahora es ruido contra 19. **Se queda, y ya no es el techo** | 03 → 03B | 2026-08-19 |
