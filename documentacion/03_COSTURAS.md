...
| Pieza | Borde | Qué decide el cambio | Elegido hoy | Goal | Fecha |
|---|---|---|---|---|---|
| **Wake word** | Proceso | Corpus **aislado** «Baxy»/«Baxy.» de voces holdout (Sabina es-MX + Zira en-US, rates −3/0/+3): FRR y máximo de score. FAR: Poisson unilateral 95 % sobre horas de película/serie que no le hablan a BAXY; el umbral **no se retoca** al abrir. Listón de promoción: FRR ≤5 %, FAR superior ≤0,1/h. Procedimiento: `scripts/evaluate_wake_corpus.py` y `scripts/measure_goal09_far.py` | `baxy.onnx` SHA `9b1ae5db…`, umbral **0,5**. Holdout 2026-08-24: **12/12** (FRR 0). FAR **2,02 h** de series (Impractical Jokers): **6 disparos**, 3,0/h, superior 5,85/h. **No se promociona** `wake_on_start`: el listón 0,1/h no se cumple y no se baja el umbral. `artifacts/goal09/wake_far.json` | 01 → 09 | 2026-08-24 |
