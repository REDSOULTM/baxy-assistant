...
| **Reconocedor determinista** | Contrato | **Cuatro números, y el cuarto es el que decide si vale la pena**: cuántas filas reclama, cuántas acierta, cuántas peticiones fuera de catálogo reclama por error, y **cuántas de esas mismas filas sirve el camino por modelo cuando se le quita**. Sin el cuarto no se sabe si 12.310 líneas de reglas están aportando o estorbando. La tercera sigue siendo la que veta: un reconocedor que reclama algo que no sabe hacer produce un efecto no pedido | 12.310 líneas de reglas en `effect_intent.py`. Reclama **38 de 124** filas y sirve **26**; no reclama **ninguna** de las 36 fuera de catálogo, y responde en **p50 0,007 s** contra ~2 s del modelo. **Se conserva porque se midió contra su alternativa**: enviando sus 38 filas al camino por modelo, el modelo sirve 24 —recupera 3 y pierde 5—, así que gana por dos filas. Sus errores son desvíos silenciosos a una hermana: «dejame el sonido a la mitad» resuelve `audio.status` en vez de `audio.volume`. Su techo, declinando perfectamente cuando duda, son 29 de 38 | 03 | 2026-08-17 |
