"""Evaluate the precision-revised ServiceNow semantic STT fusion candidate."""
...
SCHEMA = "baxy.servicenow-codeswitch-semantic-fusion-development.v3"


def _load_module(name: str, path: Path):
    specification = importlib.util.spec_from_file_location(name, path)
    if specification is None or specification.loader is None:
        raise RuntimeError(f"semantic_fusion_v3_import_failed:{name}")
...
def evaluate(arguments: argparse.Namespace) -> dict[str, object]:
    ...
    def load_v1_with_precision_fusion(root: Path):
        ...
        def load_module(name: str, path: Path):
            if name == "baxy_semantic_stt_fusion_development":
                path = (
                    root
                    / "experiments/stt_quality/semantic_stt_fusion_v3.py"
                )
    ...
    v2._load_v1 = load_v1_with_precision_fusion
    v2.SCHEMA = SCHEMA
    v2.__file__ = str(evaluator_path)
    artifact = v2.evaluate(arguments)

    fusion_path = (
        repository_root / "experiments/stt_quality/semantic_stt_fusion_v3.py"
    )
    predecessor_path = (
        repository_root
        / "artifacts/development/servicenow_codeswitch_semantic_fusion_development_v2.json"
    )
    artifact["evaluator"] = {
        "path": evaluator_path.relative_to(repository_root).as_posix(),
        "sha256": v2._load_v1(repository_root).sha256(evaluator_path),
    }
    artifact["fusion"] = {
        "path": fusion_path.relative_to(repository_root).as_posix(),
        "sha256": v2._load_v1(repository_root).sha256(fusion_path),
    }
    artifact["predecessor"] = {
        "path": predecessor_path.relative_to(repository_root).as_posix(),
        "sha256": v2._load_v1(repository_root).sha256(predecessor_path),
        "status": "passed_with_known_false_clarification_pressure",
        "reason": "ordinary_lowercase_morphology_was_emitted_as_entity_ambiguity",
    }
    artifact["precisionRevision"] = {
        "policy": "lexical_compounds_require_named-entity_capitalization",
...
def main() -> int:
    ...
    repository_root = arguments.repository_root.resolve(strict=True)
    base = _load_module(
        "baxy_servicenow_semantic_fusion_v1_hash",
        repository_root
