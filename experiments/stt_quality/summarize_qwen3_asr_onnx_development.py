"""Publish the development-only Qwen3-ASR ONNX candidate verdict."""
...
REPORT_SCHEMA = "baxy.stt-qwen3-asr-onnx-development.v1"
VERDICT_SCHEMA = "baxy.stt-qwen3-asr-onnx-development-verdict.v1"
EXPECTED_WAKE_TREE_SHA256 = (
    "756a5b311b20ed833da182595766c9d577cca89f20d7a4355eb972576f0f90c0"
)
REPORTS = {
    "audioArenaQwen": ROOT
    / "artifacts/development/stt_audio_arena_qwen3_asr_onnx_cpu_development_v1.json",
    "minds14Qwen": ROOT
    / "artifacts/development/stt_minds14_qwen3_asr_onnx_cpu_development_v1.json",
...
def _read(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"qwen3_verdict_report_invalid:{path}")
    return value
...
def build_verdict(repository_root: Path = ROOT) -> dict[str, object]:
    ...
    audio = reports["audioArenaQwen"]
    minds = reports["minds14Qwen"]
    parakeet = reports["minds14Parakeet"]
    nemotron = reports["minds14Nemotron"]
    for key, report in (("audio_arena", audio), ("minds14", minds)):
        if (
            report.get("schema") != REPORT_SCHEMA
            or report.get("source") != key
            or report.get("partition") != "development"
            or report.get("engine") != "qwen3_asr_onnx_cpu"
            ...
            raise RuntimeError(f"qwen3_verdict_candidate_report_invalid:{key}")

    qwen_rows = {str(row["caseId"]): row for row in minds["cases"]}
    parakeet_rows = {str(row["caseId"]): row for row in parakeet["cases"]}
    nemotron_rows = {str(row["caseId"]): row for row in nemotron["cases"]}
    identities = set(qwen_rows)
    if (
        len(identities) != 84
        or identities != set(parakeet_rows)
        or identities != set(nemotron_rows)
    ):
        raise RuntimeError("qwen3_verdict_minds_population_mismatch")
    unique_qwen_rescues = sorted(
        identity
        for identity in identities
        if _intent_correct(qwen_rows[identity])
        and not _intent_correct(nemotron_rows[identity])
    )
    qwen_confirms_nemotron_rescues = sorted(
        identity
        for identity in identities
        if not _intent_correct(parakeet_rows[identity])
        and _intent_correct(nemotron_rows[identity])
        and _intent_correct(qwen_rows[identity])
    )
    ...
    decision = {
        ...
        "reasons": [
            ...
            "audio_arena_latency_p95_above_2_seconds",
            "minds14_semantic_preservation_below_0_99",
            "minds14_finalization_latency_p95_above_1_5_seconds",
            "no_unique_minds14_intent_rescue_beyond_nemotron",
        ],
        "nextArchitecture": (
            "smaller multilingual ASR with a non-generative or bounded decoder"
        ),
    }
    return {
        ...
        "results": {
            ...
            "minds14": {
                ...
                "uniqueIntentRescuesBeyondNemotron": unique_qwen_rescues,
                "confirmedNemotronRescues": qwen_confirms_nemotron_rescues,
            },
        },
        "decision": decision,
...
def main() -> int:
    ...
    if output.exists():
        raise RuntimeError("qwen3_verdict_output_exists")
