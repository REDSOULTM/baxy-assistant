"""Run one corrected read-only recogniser measurement after R265 preflight failure."""
...
OUTPUT = REPO / "artifacts/audit/fresh_independent_cut_b_recogniser_r266.json"
...
def validate_inputs() -> None:
    required = (CORPUS, R264_PREREGISTRATION, MEASUREMENT_PREREGISTRATION, ALIASES)
    if any(not path.is_file() for path in required):
        raise RuntimeError("R266 requires sealed R264 inputs, preregistration, and aliases")
    r264 = json.loads(R264_PREREGISTRATION.read_text(encoding="utf-8"))
    r266 = json.loads(MEASUREMENT_PREREGISTRATION.read_text(encoding="utf-8"))
    if r264["identities"]["corpus_sha256"] != sha256(CORPUS):
        raise RuntimeError("R266 R264 corpus identity changed")
    if r264["constraints"]["recogniser_measured"] is not False:
        raise RuntimeError("R266 requires R264 before recogniser measurement")
    if r266["scoring"]["runner_sha256"] != sha256(Path(__file__)):
        raise RuntimeError("R266 runner changed after preregistration")
    if r266["source"]["r264_corpus_sha256"] != sha256(CORPUS):
        raise RuntimeError("R266 preregistration names a different R264 corpus")
    if r266["predecessor"]["r265_output_created"] is not False:
        raise RuntimeError("R266 requires the failed R265 run to have no output")
...
def build() -> dict[str, object]:
    ...
    rows = load_rows()
    if len(rows) != 114:
        raise RuntimeError("R266 expects 114 sealed R264 rows")
    ...
    return {
        "schema": "baxy.fresh-independent-cut-b-recogniser-r266.v1",
        "authority": "single_read_only_recogniser_measurement_after_r264_and_r266_preregistration",
        "verdict": (
        ...
        "identities": {
            ...
            "r266_preregistration_sha256": sha256(MEASUREMENT_PREREGISTRATION),
...
def main() -> int:
    if OUTPUT.exists():
        raise RuntimeError(f"R266 output exists; refusing a second recogniser measurement: {OUTPUT}")
    OUTPUT.write_bytes(
