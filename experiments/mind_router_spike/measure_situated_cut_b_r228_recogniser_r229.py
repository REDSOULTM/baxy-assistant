"""Open sealed R228 once against the deterministic recogniser only."""
...
PREREGISTRATION = REPO / "artifacts/holdout/situated_cut_b_r228.preregistration.json"
ALIASES = REPO / "src/baxy_mind/data/catalog_operation_aliases.v1.json"
OUTPUT = REPO / "artifacts/audit/situated_cut_b_r228_recogniser_reach_r229.json"
...
def measure(corpus_path: Path = CORPUS) -> dict[str, Any]:
    ...
    return {
        "schema": "baxy.situated-cut-b.r228-recogniser-reach-r229.v1",
        "authority": "opened_once_read_only_recogniser_baseline_not_a_model_or_product_result",
        "identities": {
        ...
        "outcomes": outcomes,
        "next_step": (
            "If reach is below one half, retain R228 unchanged and preregister "
            "a separate retrieval/model-path candidate. Otherwise reject R228 "
            "without editing or reusing it."
        ),
    }
...
def main() -> None:
    if OUTPUT.exists():
        raise FileExistsError("r229_is_one_shot_and_already_opened")
    OUTPUT.write_bytes(
        (
