...
OUTPUT = REPO / "artifacts" / "fixes" / "p_catalog_prefix_20260731.json"
...
def _sidecar_main(variant: str, telemetry: str) -> int:
    ...
    def build(text, operation_names, candidate_text, prior_messages):
        payload = original_build(
            text, operation_names, candidate_text, prior_messages)
        if variant != "catalog_prefix":
            return payload
        # The candidate catalog is emitted last today, after the history and
        # after the current message, so two turns with an identical shortlist
        # can still never share a prefix: everything before it changed. Moving
        # it to the end of the system turn puts it inside the stable prefix
        # whenever the shortlist repeats. When the shortlist does change, the
        # system prefix is invalidated too, which is the risk this measures.
        messages = payload["messages"]
        head = messages[0]
        if head.get("role") != "system":
            raise RuntimeError("el primer mensaje primario ya no es de sistema")
        tail = messages[-1]
        marker = "\n\nOperaciones candidatas:\n"
        content = tail.get("content") or ""
        if marker not in content:
            raise RuntimeError("el mensaje final no contiene el catálogo esperado")
        current, catalog = content.split(marker, 1)
        payload["messages"] = [
            {
                "role": "system",
                "content": head["content"] + marker.lstrip("\n") + catalog,
            },
            *messages[1:-1],
            {"role": tail["role"], "content": current},
        ]
        return payload
...
def run() -> dict[str, Any]:
    ...
    telemetry = OUTPUT.with_name("p_catalog_prefix_telemetry_20260731.jsonl")
    if telemetry.exists():
        telemetry.unlink()
    telemetry.touch()
    # ABBA over whole variant sessions so neither arm owns the warm state.
    for order in (("baseline", "catalog_prefix"), ("catalog_prefix", "baseline")):
    ...
    for case_id, _ in CASES:
        ...
        for variant in ("baseline", "catalog_prefix"):
        ...
        introduced = [
            value for value in sides["catalog_prefix"] if value not in sides["baseline"]
        ]
        if introduced:
            no_new_decisions = False
        if len(sides["baseline"]) > 1 or len(sides["catalog_prefix"]) > 1:
            nondeterministic.append(case_id)
        per_case[case_id] = {
            "baseline_outcomes": [json.loads(value) for value in sides["baseline"]],
            "catalog_prefix_outcomes": [json.loads(value) for value in sides["catalog_prefix"]],
            "introduced_by_candidate": [json.loads(value) for value in introduced],
            "same_outcome_set": sides["baseline"] == sides["catalog_prefix"],
            "deterministic_in_both": (
                len(sides["baseline"]) == 1 and len(sides["catalog_prefix"]) == 1),
        }
    ...
    baseline, candidate = summarize("baseline"), summarize("catalog_prefix")
    policy_baseline = policy_stats("baseline")
    policy_candidate = policy_stats("catalog_prefix")
    policy_delta = None
    ...
    report = {
        ...
        "policy_catalog_prefix": policy_candidate,
...
def main() -> int:
    ...
    print(json.dumps({
        ...
        "policy_catalog_prefix": report["policy_catalog_prefix"],
