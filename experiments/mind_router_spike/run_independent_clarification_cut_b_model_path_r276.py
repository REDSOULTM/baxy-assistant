"""Open R270 once through the registered no-dispatch BAXY model path."""

from __future__ import annotations

import collections
import hashlib
import importlib
import json
import sys
import time
from pathlib import Path
from typing import Any


REPO = Path(__file__).resolve().parents[2]
for path in (REPO, REPO / "src", REPO / "scripts"):
    if str(path) not in sys.path:
        sys.path.insert(0, str(path))

from scripts.baxy_runtime_config import (  # noqa: E402
    DEFAULT_RUNTIME_MANIFEST,
    public_runtime_identity,
    resolve_runtime,
)
from scripts.measure_mind_budget import (  # noqa: E402
    PROFILE_LIMITS,
    JsonLineProcess,
    current_core_catalog_snapshot,
    discover_core,
    sidecar_environment,
)
from experiments.mind_router_spike import (  # noqa: E402
    score_independent_clarification_cut_b_model_path_r276 as scoring,
)


CORPUS = REPO / "artifacts/development/independent_clarification_cut_b_r270.jsonl"
R270_PREREGISTRATION = (
    REPO
    / "artifacts/development/independent_clarification_cut_b_r270.preregistration.json"
)
R274_RESULT = (
    REPO / "artifacts/audit/independent_clarification_cut_b_entry_path_r274.json"
)
R275_PREREGISTRATION = (
    REPO
    / "artifacts/development/independent_clarification_cut_b_r275.model-path.preregistration.json"
)
SNAPSHOT = REPO / "artifacts/development/current_core_catalog_snapshot_r219.json"
RESULT = REPO / "artifacts/audit/independent_clarification_cut_b_model_path_r276.json"
TELEMETRY = (
    REPO / "artifacts/audit/independent_clarification_cut_b_model_path_r276.telemetry.jsonl"
)
TURN_AUDIT = (
    REPO / "artifacts/audit/independent_clarification_cut_b_model_path_r276.turn-audit.jsonl"
)
RAW_REPLIES = (
    REPO / "artifacts/audit/independent_clarification_cut_b_model_path_r276.raw-replies.jsonl"
)
RECEIPT = (
    REPO / "artifacts/audit/independent_clarification_cut_b_model_path_r276.consumed.json"
)


def sha256(path: Path) -> str:
    return hashlib.sha256(path.read_bytes()).hexdigest()


def _write(path: Path, value: object, *, lines: bool = False) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("xb") as handle:
        if lines:
            for row in value:  # type: ignore[union-attr]
                handle.write(
                    (json.dumps(row, ensure_ascii=False, sort_keys=True) + "\n").encode(
                        "utf-8"
                    )
                )
        else:
            handle.write(
                (json.dumps(value, ensure_ascii=False, indent=2) + "\n").encode("utf-8")
            )


def _refuse_reuse() -> None:
    if any(path.exists() for path in (RESULT, TELEMETRY, TURN_AUDIT, RAW_REPLIES, RECEIPT)):
        raise RuntimeError("R276 has already been opened")


def _load_rows() -> list[dict[str, Any]]:
    return [
        json.loads(line)
        for line in CORPUS.read_text(encoding="utf-8").splitlines()
        if line.strip()
    ]


def _recogniser() -> Any:
    return importlib.import_module("baxy_mind.effect_intent")


def _entry_path(text: str, available: tuple[str, ...], resolver: Any) -> str:
    if resolver.resolve_explicit_clarification_intent(text, available) is not None:
        return "explicit_clarification"
    if resolver.resolve_explicit_effects(text, available, (), ()) is not None:
        return "explicit_effect"
    return "model_decision_candidate_after_two_effect_gates"


def validate_inputs() -> dict[str, Any]:
    required = (CORPUS, R270_PREREGISTRATION, R274_RESULT, R275_PREREGISTRATION, SNAPSHOT)
    if any(not path.is_file() for path in required):
        raise RuntimeError("R276 requires sealed R270/R274/R275 inputs and R219")
    preregistration = json.loads(R275_PREREGISTRATION.read_text(encoding="utf-8"))
    r270 = json.loads(R270_PREREGISTRATION.read_text(encoding="utf-8"))
    r274 = json.loads(R274_RESULT.read_text(encoding="utf-8"))
    if r270["identities"]["corpus_sha256"] != sha256(CORPUS):
        raise RuntimeError("R276 R270 corpus identity changed")
    if r274["identities"]["r270_corpus_sha256"] != sha256(CORPUS):
        raise RuntimeError("R276 R274 source identity changed")
    identities = preregistration["identities"]
    expected = {
        "corpus_sha256": sha256(CORPUS),
        "r270_preregistration_sha256": sha256(R270_PREREGISTRATION),
        "r274_result_sha256": sha256(R274_RESULT),
        "snapshot_sha256": sha256(SNAPSHOT),
        "runtime_manifest_sha256": sha256(DEFAULT_RUNTIME_MANIFEST),
        "runner_sha256": sha256(Path(__file__)),
        "scorer_sha256": sha256(scoring.__file__ and Path(scoring.__file__)),
    }
    if preregistration.get("schema") != "baxy.independent-clarification-cut-b.model-path-r275-preregistration.v1":
        raise RuntimeError("R275 preregistration schema changed")
    if any(identities.get(key) != value for key, value in expected.items()):
        raise RuntimeError("R276 preregistration identity changed")
    return preregistration


def _environment(runtime: Any) -> dict[str, str]:
    environment = sidecar_environment(
        runtime,
        gpu_layers=runtime.gpu_layers,
        llm_http_timeout=PROFILE_LIMITS["gpu"]["llm_http"],
    )
    for name in (
        "BAXY_MIND_TURN_AUDIT_PATH",
        "BAXY_MIND_RAW_REPLY_AUDIT_PATH",
        "BAXY_MIND_MESSAGE_COMPOSE_AUDIT_PATH",
        "BAXY_MIND_INVALID_REPLY_DIR",
    ):
        environment.pop(name, None)
    environment["BAXY_MIND_TURN_AUDIT_PATH"] = str(TURN_AUDIT.resolve())
    environment["BAXY_MIND_RAW_REPLY_AUDIT_PATH"] = str(RAW_REPLIES.resolve())
    environment["BAXY_VOICE_STREAMING_STT"] = "off"
    return environment


def _audits(expected_ids: set[str]) -> dict[str, dict[str, Any]]:
    grouped: dict[str, list[dict[str, Any]]] = collections.defaultdict(list)
    for line in TURN_AUDIT.read_text(encoding="utf-8").splitlines():
        if line.strip():
            row = json.loads(line)
            grouped[str(row.get("request_id") or "")].append(row)
    if set(grouped) != expected_ids:
        raise RuntimeError(f"R276 audit incomplete: {len(grouped)}/{len(expected_ids)}")
    selected: dict[str, dict[str, Any]] = {}
    for request_id, records in grouped.items():
        raw = [row for row in records if row.get("phase") == "raw_attempt"]
        final = [row for row in records if row.get("phase") in {"final", "recovery"}]
        if not final:
            raise RuntimeError(f"R276 final audit missing: {request_id}")
        audit = dict(final[-1])
        audit["candidate_operations"] = list(
            (raw[-1] if raw else audit).get("candidate_operations") or []
        )
        audit["raw_decision"] = (raw[-1].get("raw_decision") if raw else None)
        selected[request_id] = audit
    return selected


def _raw_replies() -> dict[str, list[str]]:
    grouped: dict[str, list[str]] = collections.defaultdict(list)
    if RAW_REPLIES.is_file():
        for line in RAW_REPLIES.read_text(encoding="utf-8").splitlines():
            if line.strip():
                row = json.loads(line)
                grouped[str(row.get("request_sha256") or "")].append(
                    str(row.get("raw_reply") or "")
                )
    return dict(grouped)


def _proposal(value: object) -> dict[str, Any]:
    if not isinstance(value, dict):
        return {"available": False, "effect_operations": []}
    return {
        "available": True,
        "mode": value.get("mode"),
        "operation": value.get("operation"),
        "effect_operations": list(value.get("effect_operations") or []),
        "effect_verification": value.get("effect_verification"),
    }


def measure() -> tuple[list[dict[str, Any]], dict[str, Any], dict[str, Any]]:
    preregistration = validate_inputs()
    rows = _load_rows()
    if len(rows) != 93:
        raise RuntimeError("R276 expects the sealed 93-row R270 corpus")
    snapshot = json.loads(SNAPSHOT.read_text(encoding="utf-8"))["catalogue"]
    available = tuple(sorted(str(item["name"]) for item in snapshot["capabilities"]))
    resolver = _recogniser()
    entry_paths = {str(row["case_id"]): _entry_path(str(row["text"]), available, resolver) for row in rows}
    if list(entry_paths.values()).count("model_decision_candidate_after_two_effect_gates") != 79:
        raise RuntimeError("R276 entry-path count differs from sealed R274")
    runtime = resolve_runtime(manifest_path=DEFAULT_RUNTIME_MANIFEST)
    capabilities, applications, games = current_core_catalog_snapshot(discover_core(None))
    encoded = json.dumps(capabilities, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
    if len(capabilities) != 174 or hashlib.sha256(encoded).hexdigest() != snapshot["capabilities_sha256"]:
        raise RuntimeError("current Core catalogue differs from authenticated R219")
    client = JsonLineProcess(
        [str(runtime.python), "-u", "-X", "utf8", "-m", "baxy_mind"],
        environment=_environment(runtime),
        cwd=REPO,
    )
    replies: dict[str, dict[str, Any]] = {}
    seconds: dict[str, float] = {}
    request_ids: dict[str, str] = {}
    try:
        if client.next_message(PROFILE_LIMITS["gpu"]["handshake"]).get("type") != "hello":
            raise RuntimeError("R276 sidecar did not greet")
        ready = client.request(
            {
                "type": "catalog.configure",
                "id": "r276-catalog",
                "capabilities": capabilities,
                "applicationCatalog": applications,
                "gameCatalog": games,
            },
            PROFILE_LIMITS["gpu"]["handshake"],
        )
        if ready.get("type") != "catalog.ready":
            raise RuntimeError("R276 catalogue rejected")
        for row in rows:
            case_id = str(row["case_id"])
            request_hash = hashlib.sha256(str(row["text"]).encode("utf-8")).hexdigest()
            request_id = f"r276-{request_hash[:24]}"
            request_ids[case_id] = request_id
            started = time.perf_counter()
            replies[case_id] = client.request(
                {
                    "type": "turn.decide",
                    "id": request_id,
                    "text": row["text"],
                    "history": [],
                    "uiLanguage": "en" if row["language"] == "en" else "es",
                },
                PROFILE_LIMITS["gpu"]["turn.decide"],
            )
            seconds[case_id] = round(time.perf_counter() - started, 6)
    finally:
        client.close(graceful_message=None, timeout=PROFILE_LIMITS["gpu"]["shutdown"])
    audits, raw_replies = _audits(set(request_ids.values())), _raw_replies()
    telemetry: list[dict[str, Any]] = []
    for row in rows:
        case_id = str(row["case_id"])
        request_hash = hashlib.sha256(str(row["text"]).encode("utf-8")).hexdigest()
        reply, audit = replies[case_id], audits[request_ids[case_id]]
        telemetry.append(
            {
                "request_sha256": request_hash,
                "family": row["family"],
                "language": row["language"],
                "intended_operations": list(row["intended_operations"]),
                "missing_facts": list(row["missing_facts"]),
                "entry_path": entry_paths[case_id],
                "seconds": seconds[case_id],
                "candidate_operations": list(audit.get("candidate_operations") or []),
                "raw_proposal": _proposal(audit.get("raw_decision")),
                "stages": list(audit.get("stages") or []),
                "kind": reply.get("kind"),
                "operation": reply.get("operation"),
                "intent_operations": list(reply.get("intentOperations") or []),
                "effect_operations": list(reply.get("effectOperations") or []),
                "question": str(reply.get("question") or ""),
                "reply": str(reply.get("reply") or ""),
                "raw_visible_proposals": raw_replies.get(request_hash, []),
                "provider_dispatch_enabled": False,
                "external_effect_executed": False,
            }
        )
    return telemetry, preregistration, public_runtime_identity(runtime)


def run_once() -> dict[str, Any]:
    _refuse_reuse()
    telemetry, preregistration, runtime = measure()
    result = scoring.build_result(telemetry, preregistration)
    result.update(
        {
            "execution": {
                "mind_sidecar_started": True,
                "providers_enabled": False,
                "effects_executed": 0,
                "opened_v9": False,
                "voice_stt_wake_exercised": False,
            },
            "runtime": runtime,
            "identities": preregistration["identities"],
        }
    )
    _write(TELEMETRY, telemetry, lines=True)
    _write(RESULT, result)
    _write(
        RECEIPT,
        {
            "schema": "baxy.independent-clarification-cut-b.model-path-r276-consumption.v1",
            "measurement_status": "consumed",
            "opened_exactly_once": True,
            "retry_allowed": False,
            "status": result["status"],
            "effects_executed": 0,
            "providers_enabled": False,
            "hashes": {
                "corpus_sha256": sha256(CORPUS),
                "preregistration_sha256": sha256(R275_PREREGISTRATION),
                "result_sha256": sha256(RESULT),
                "telemetry_sha256": sha256(TELEMETRY),
                "turn_audit_sha256": sha256(TURN_AUDIT),
                "runner_sha256": sha256(Path(__file__)),
            },
        },
    )
    return result


def main() -> int:
    result = run_once()
    print(json.dumps({"status": result["status"], "failed_thresholds": result["failed_thresholds"]}, ensure_ascii=False))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
