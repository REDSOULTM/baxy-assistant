"""Open the sealed compound execution R3 campaign exactly once."""
...
from scripts import build_compound_execution_holdout_r3 as campaign  # noqa: E402
...
def verify_preregistration() -> None:
    ...
    if (
        ...
        raise RuntimeError("compound execution R3 preregistration changed")
    if campaign.OUTPUT.exists():
        raise RuntimeError("compound execution R3 has already been opened")
...
def main() -> int:
    ...
    if (
        ...
        raise RuntimeError("compound execution R3 did not meet its sealed threshold")
    return 0
