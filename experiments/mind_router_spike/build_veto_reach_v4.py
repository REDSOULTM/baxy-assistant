...
OUTPUT = REPO / "artifacts/holdout/veto_reach_v4.json"
PREREGISTRATION = REPO / "artifacts/holdout/veto_reach_v4.preregistration.json"
RUNNER = REPO / "experiments/mind_router_spike/run_veto_reach_v4.py"
HARNESS = REPO / (
...
CATALOGUE_CONTROLS: tuple[tuple[str, str, str], ...] = (
    ("v4-cat-01", "es", "sigue en pie el amarre a la red de casa"),
    ("v4-cat-02", "en", "does the tie to the home network still hold"),
    ("v4-cat-03", "spanglish", "sigue en pie el amarre a la home network"),
    ("v4-cat-04", "es", "que tal lleva la maquina la carga de hoy"),
    ("v4-cat-05", "en", "how is the box holding up under today load"),
    ("v4-cat-06", "es", "que chismes van prendidos a este cacharro"),
    ("v4-cat-07", "en", "which widgets sit attached to this rig"),
    ("v4-cat-08", "es", "atrapame lo que se ve en la lamina"),
    ("v4-cat-09", "en", "catch what is shown on the pane for me"),
    ("v4-cat-10", "es", "que llevo en el bolsillo para soltar despues"),
    ("v4-cat-11", "en", "what sits in my pocket to release later"),
    ("v4-cat-12", "es", "por donde brota el sonido y con que empuje"),
    ("v4-cat-13", "en", "where does the sound spring from and how hard"),
    ("v4-cat-14", "es", "quien trajina alla al fondo sin asomarse"),
    ("v4-cat-15", "en", "who bustles back there without showing up"),
)
...
INTENDED: dict[str, str] = {
    "v4-cat-01": "wifi.status",
    "v4-cat-02": "wifi.status",
    "v4-cat-03": "wifi.status",
    "v4-cat-04": "system.status",
    "v4-cat-05": "system.status",
    "v4-cat-06": "peripheral.list",
    "v4-cat-07": "peripheral.list",
    "v4-cat-08": "capture.screenshot",
    "v4-cat-09": "capture.screenshot",
    "v4-cat-10": "clipboard.read.text",
    "v4-cat-11": "clipboard.read.text",
    "v4-cat-12": "audio.status",
    "v4-cat-13": "audio.status",
    "v4-cat-14": "system.process.list",
    "v4-cat-15": "system.process.list",
}
...
REQUESTS: tuple[tuple[str, str, str], ...] = (
    ("v4-req-01", "es", "Cuece las alcachofas para la cena."),
    ("v4-req-02", "es", "Vierte el aceite en la aceitera."),
    ("v4-req-03", "es", "Tiende las sabanas en el tendedero."),
    ("v4-req-04", "es", "Mueve el sofa hacia la ventana."),
    ("v4-req-05", "es", "Cuelga el cuadro sobre la chimenea."),
    ("v4-req-06", "es", "Enciende la chimenea del salon."),
    ("v4-req-07", "es", "Riega los geranios del balcon."),
    ("v4-req-08", "es", "Calienta la sopa que quedo de ayer."),
    ("v4-req-09", "es", "Tuerce el alambre para el gancho."),
    ("v4-req-10", "es", "Muele la pimienta para el guiso."),
    ("v4-req-11", "es", "Suelta la cuerda del columpio."),
    ("v4-req-12", "es", "Cierra el grifo del jardin."),
    ("v4-req-13", "es", "Sienta al nino en la trona."),
    ("v4-req-14", "es", "Empieza la masa del pan manana."),
    ("v4-req-15", "es", "Prueba la salsa antes de servirla."),
    ("v4-req-16", "spanglish", "Cuece las artichokes para la cena."),
    ("v4-req-17", "spanglish", "Tiende las sheets en el tendedero."),
    ("v4-req-18", "spanglish", "Riega los geraniums del balcon."),
    ("v4-req-19", "en", "Boil the artichokes for dinner."),
    ("v4-req-20", "en", "Hang the picture over the fireplace."),
)

CONTROLS: tuple[tuple[str, str, str], ...] = (
    ("v4-ctl-01", "es", "Que opinas de cocinar a fuego lento?"),
    ("v4-ctl-02", "en", "Tell me something odd about clotheslines."),
    ("v4-ctl-03", "es", "Cocer las alcachofas nos llevo media hora."),
    ("v4-ctl-04", "en", "Hanging that picture took two of us."),
)
...
def validate_population() -> None:
    if len(CATALOGUE_CONTROLS) != 15 or len(REQUESTS) != 20 or len(CONTROLS) != 4:
        raise RuntimeError("veto-reach V4 size changed")
    surfaces = {
    ...
    if len(surfaces) != len(REQUESTS) + len(CONTROLS) + len(CATALOGUE_CONTROLS):
        raise RuntimeError("veto-reach V4 has duplicate surfaces")
    if {language for _, language, _ in CATALOGUE_CONTROLS} != {"es", "en", "spanglish"}:
        raise RuntimeError("veto-reach V4 language coverage changed")
    ...
    if reused:
        raise RuntimeError(f"veto-reach V4 reuses prior surfaces: {reused}")
    ...
    for case_id, _, text in CATALOGUE_CONTROLS:
        ...
        if intended not in available:
            raise RuntimeError(f"veto-reach V4 names an absent operation: {case_id}")
        ...
        if resolved is not None and tuple(resolved.operations) == (intended,):
            ...
            raise RuntimeError(
                f"veto-reach V4 control resolves deterministically: {case_id}"
            )


def build() -> dict[str, Any]:
    if PREREGISTRATION.exists() or OUTPUT.exists():
        raise RuntimeError("refusing to overwrite veto-reach V4 state")
    validate_population()
    runtime_manifest = (
    ...
    manifest = {
        "schema": "baxy.veto-reach.preregistration.v4",
        "blind_holdout": True,
