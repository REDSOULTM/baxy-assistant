"""Analyze the sealed memory-status R1 TRX exactly once."""
...
from scripts import build_memory_status_holdout_r1 as campaign  # noqa: E402
...
TEST_PREFIX = "RoutesBlindMemoryStatusR1Requests("
...
def main() -> int:
    if campaign.RESULT.exists():
        raise RuntimeError("refusing to overwrite memory R1 result")
    ...
    if any(sha256(path) != expected for path, expected in expected_hashes.items()):
        raise RuntimeError("memory R1 source hashes changed after preregistration")
    corpus = [
        json.loads(line)
    ...
    for result in results:
        ...
        if not name.startswith(TEST_PREFIX) or not name.endswith(")"):
            raise RuntimeError(f"unexpected memory R1 test: {name}")
    ...
    if observed_ids != expected_ids or len(results) != campaign.EXPECTED_CASES:
        raise RuntimeError("memory R1 TRX is not bound to the sealed population")
    failed = campaign.EXPECTED_CASES - passed
    accuracy = passed / campaign.EXPECTED_CASES
    lower = (
        math.pow(0.05, 1.0 / campaign.EXPECTED_CASES)
        if failed == 0
        else None
    )
    threshold_passed = (
    ...
    report = {
        "schema": "baxy.memory-status-holdout-result.v1",
