"""Analyze the sealed memory-status R3 TRX exactly once."""
...
from scripts import build_memory_status_holdout_r3 as campaign  # noqa: E402
...
TEST_PREFIX = "RoutesBlindMemoryStatusR3Requests("
...
def main() -> int:
    if campaign.RESULT.exists():
        raise RuntimeError("refusing to overwrite memory R3 result")
    ...
    if any(sha256(path) != expected for path, expected in expected_hashes.items()):
        raise RuntimeError("memory R3 source hashes changed after preregistration")
    corpus = [
        json.loads(line)
    ...
    for result in results:
        ...
        if not name.startswith(TEST_PREFIX) or not name.endswith(")"):
            raise RuntimeError(f"unexpected memory R3 test: {name}")
    ...
    if observed_ids != expected_ids or len(results) != campaign.EXPECTED_CASES:
        raise RuntimeError("memory R3 TRX is not bound to the sealed population")
    ...
    report = {
        "schema": "baxy.memory-status-holdout-result.v3",
