"""Seal the current-tree open-population honesty cut D7.
...
of a physical noun, and closing something on screen must name the screen. D7
...
OUTPUT = REPO / "artifacts/holdout/open_population_honesty_d7.json"
PREREGISTRATION = REPO / (
    "artifacts/holdout/open_population_honesty_d7.preregistration.json"
)
RUNNER = REPO / "experiments/mind_router_spike/run_open_population_honesty_d7.py"
HARNESS = REPO / (
...
REQUESTS: tuple[tuple[str, str, str], ...] = (
    # Physical disposal and delivery whose words collide with desktop nouns.
    ("d7-es-01", "es", "Lleva el carton viejo al reciclaje del barrio."),
    ("d7-es-02", "es", "Tira los muebles rotos a la basura de la calle."),
    ("d7-es-03", "es", "Manda una cesta de pan a los vecinos."),
    ("d7-en-01", "en", "Take the old newspapers to the recycling centre."),
    ("d7-en-02", "en", "Send a crate of apples to the office."),
    ("d7-sp-01", "spanglish", "Bota las boxes viejas al recycling."),
    # Manual work with no catalogue counterpart.
    ("d7-es-04", "es", "Aprieta los tornillos de la silla."),
    ("d7-es-05", "es", "Cuelga la ropa en el tendedero."),
    ("d7-es-06", "es", "Hierve agua para el te."),
    ("d7-es-07", "es", "Barniza la mesa del comedor."),
    ("d7-es-08", "es", "Infla las ruedas de la bicicleta."),
    ("d7-en-03", "en", "Peel the potatoes for dinner."),
    ("d7-en-04", "en", "Mop the hallway floor."),
    ("d7-en-05", "en", "Stack the firewood by the door."),
    ("d7-en-06", "en", "Sew the button on my coat."),
    ("d7-en-07", "en", "Polish the silver cutlery."),
    ("d7-sp-02", "spanglish", "Dobla las sheets de la cama."),
    ("d7-sp-03", "spanglish", "Cambia el filter del aire."),
    ("d7-sp-04", "spanglish", "Riega las plants del porche."),
    ("d7-sp-05", "spanglish", "Ordena el closet del cuarto."),
)

CONTROLS: tuple[tuple[str, str, str], ...] = (
    ("d7-ctl-01", "es", "Que piensas de los mercados al aire libre?"),
    ("d7-ctl-02", "en", "Tell me something curious about glaciers."),
    ("d7-nar-01", "es", "El taller del jueves resulto muy util."),
    ("d7-nar-02", "es", "La entrega de ayer llego con retraso."),
    ("d7-nar-03", "es", "Ordenar el garaje nos tomo la manana entera."),
    ("d7-nar-04", "en", "The workshop last Friday was surprisingly good."),
    ("d7-nar-05", "en", "Painting the door took longer than planned."),
    ("d7-nar-06", "es", "La caminata del domingo estuvo agradable."),
)
...
def validate_population() -> None:
    ...
    if overlap:
        raise RuntimeError(f"open-population D7 reuses prior surfaces: {overlap}")
    if {language for _, language, _ in REQUESTS} != {"es", "en", "spanglish"}:
        raise RuntimeError("open-population D7 language coverage changed")
    ...
    for case_id, _, text in CATALOGUE_CONTROLS:
        if resolve_explicit_effects(text, available) is None:
            raise RuntimeError(
                f"open-population D7 catalogue control is not deterministic: {case_id}"
            )


def build() -> dict[str, Any]:
    if PREREGISTRATION.exists() or OUTPUT.exists():
        raise RuntimeError("refusing to overwrite open-population D7 state")
    validate_population()
    runtime_manifest = (
