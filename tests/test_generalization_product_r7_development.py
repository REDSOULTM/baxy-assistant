...
CORPUS = REPO / "artifacts/holdout/generalization_product_holdout_v7.jsonl"
...
def test_every_opened_r7_action_has_a_deterministic_contract_oracle(
    row: dict[str, object],
) -> None:
    text = str(row["text"])
...
def test_every_opened_r7_clarification_preserves_zero_effects(
    row: dict[str, object],
) -> None:
...
def test_every_opened_r7_conversation_has_a_stable_zero_effect_turn_contract(
    row: dict[str, object],
) -> None:
    text = str(row["text"])
