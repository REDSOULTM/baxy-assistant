...
CORPUS = REPO / "artifacts/holdout/generalization_product_holdout_v9.jsonl"
...
APPLICATIONS = ("Steam", "Calculator", "GIMP")

assert len(CORPUS_ROWS) == 700
assert len(MIND_ACTION_ROWS) == 560
assert len(MIND_CLARIFICATION_ROWS) == 20
assert len(MIND_CONVERSATION_ROWS) == 108
...
def test_every_opened_r9_action_has_a_deterministic_contract_oracle(
    row: dict[str, object],
) -> None:
    text = str(row["text"])
...
def test_every_opened_r9_clarification_preserves_zero_effects(
    row: dict[str, object],
) -> None:
...
def test_every_opened_r9_conversation_has_a_stable_zero_effect_turn_contract(
    row: dict[str, object],
) -> None:
    text = str(row["text"])
