...
def test_a_capability_answer_names_a_few_of_the_served_catalog() -> None:
    families = served_capability_families(
        [
            "app.open", "audio.status", "system.time", "note.create",
            "task.create", "web.search", "media.play.exact", "game.launch",
        ]
    )
...
def test_a_follow_up_does_not_replay_the_previous_turn() -> None:
    """Hipótesis retirada: el turno anterior hacía que el modelo lo continuara.
    ...
    assert "latencia de red" not in messages[-1]["content"]
